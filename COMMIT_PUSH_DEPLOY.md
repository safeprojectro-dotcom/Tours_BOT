...
(Use a **reachable** `DATABASE_URL` for that environment — often the Railway **public** Postgres URL and a driver form such as **`postgresql+psycopg://...`** if running Alembic from a local shell.) **Do not** treat Mini App, private bot, or API fixes as the next step until **`alembic current`** on that database matches **`alembic heads`** (including **`20260416_06`**, **`20260417_07`**, **`20260418_08`**, **`20260419_09`**, **`20260421_10`**, **`20260422_11`**, **`20260423_12`** as applicable when V2 Track **2** / **3** / **3.1** (showcase polish) / **4** / **5a** / **5b.1** code is deployed). Then redeploy if needed and smoke **`/health`**, catalog, and any tour-loading routes.
