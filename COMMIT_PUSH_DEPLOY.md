...
**V2 Track 5d (Mini App My Requests / RFQ status hub):** **no** new Alembic revision. Adds Flet routes **`/my-requests`** and **`/my-requests/{id}`** — **read-only** composition of existing **`GET /mini-app/custom-requests`**, **`GET /mini-app/custom-requests/{id}`**, **`GET .../booking-bridge/preparation`**, **`GET /mini-app/bookings`**, and **`GET .../payment-eligibility`** (when hold on linked tour exists). **No** new backend routes or booking/payment semantics. Smoke: list loads; detail shows labels; CTA matches bridge + bookings truth. **Stabilization review** recorded in **`docs/OPEN_QUESTIONS_AND_TECH_DEBT.md`** §32 (closure subsection). **Record:** **`docs/CHAT_HANDOFF.md`** (Track **5d** bullet).

**V2 Track 5e (Bridge supersede / cancel lifecycle):** **no** new Alembic revision (uses existing **`superseded`** / **`cancelled`** on **`custom_request_booking_bridge_status`**). Admin **`POST /admin/custom-requests/{id}/booking-bridge/close`** and **`POST .../booking-bridge/replace`**; customer **`GET /mini-app/custom-requests/{id}`** adds **`latest_booking_bridge_status`**, **`latest_booking_bridge_tour_code`**. Bridge **`preparation`** / **`reservations`** / **`payment-eligibility`** **404** when no active bridge. Smoke: close → preparation **404**; replace → new active bridge on admin detail; optional: terminal bridge + hold → hub **Continue to payment** still routes via existing **`order_id`** / **`payment-entry`**. **Stabilization review:** **`docs/OPEN_QUESTIONS_AND_TECH_DEBT.md`** §33; **`docs/CHAT_HANDOFF.md`** (Track **5e** bullet).

**V2 Track 5f v1 (Customer multi-quote summary / aggregate visibility):** **no** new Alembic revision. Extends **`GET /mini-app/custom-requests/{id}`** with **`proposed_response_count`** (**`proposed`** only), **`offers_received_hint`** (English MVP), **`selected_offer_summary`** (allowlisted snippet for admin-selected **`proposed`** response — **no** supplier identity). **My Requests** detail shows read-only copy only — **no** new CTAs or bridge/payment changes. Smoke: open request with **0** / **N** proposals → count + hint; **`supplier_selected`** → snippet present when selection valid. **Stabilization review:** **`docs/OPEN_QUESTIONS_AND_TECH_DEBT.md`** §34; **`docs/CHAT_HANDOFF.md`** (Track **5f v1** bullet).

**V2 Track 5g.1 (Commercial mode classification — read-side only):** **no** new Alembic revision. Adds **`commercial_mode`** to **`GET /mini-app/tours/{tour_code}`** (from **`tours.sales_mode`**: **`per_seat` → `supplier_route_per_seat`**, **`full_bus` → `supplier_route_full_bus`**) and to **`GET /mini-app/custom-requests/{id}`** (always **`custom_bus_rental_request`** for customer RFQ detail). **No** booking, payment, bridge, RFQ execution, or CTA logic changes. Smoke: tour detail JSON includes **`commercial_mode`** aligned with **`tour.sales_mode`**; custom-request detail includes **`custom_bus_rental_request`** alongside existing **5f v1** fields. **Stabilization review:** **`docs/OPEN_QUESTIONS_AND_TECH_DEBT.md`** §35; **`docs/CHAT_HANDOFF.md`** (Track **5g.1** bullet).
