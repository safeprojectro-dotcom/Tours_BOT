...
from app.workers.predeparture_reminder_outbox import run_once
from tests.unit.base import FoundationDBTestCase


class PredepartureReminderOutboxServiceTests(FoundationDBTestCase):
    def test_enqueue_due_reminders_persists_due_predeparture_reminder_once(self) -> None:
        now = datetime(2026, 4, 1, 8, 0, tzinfo=UTC)
        user = self.create_user(preferred_language="ro")
        tour = self.create_tour(
            code="PRE-OUT-1",
            status=TourStatus.OPEN_FOR_SALE,
            departure_datetime=now + timedelta(hours=14),
        )
        self.create_translation(tour, language_code="ro", title="Belgrad Predeparture Outbox")
        ...
        self.create_payment(order, external_payment_id="pre-outbox", status=PaymentStatus.PAID)
    ...
    def test_worker_run_once_delegates_to_outbox_service(self) -> None:
        ...
        with patch("app.workers.predeparture_reminder_outbox.PredepartureReminderOutboxService") as service_cls:
