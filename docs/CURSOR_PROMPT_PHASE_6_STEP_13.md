...
- narrow admin write slices for tours, boarding points, and tour translations exist
...
FIRST NARROW BOARDING POINT TRANSLATIONS CREATE/UPDATE-ONLY SLICE
...
или
   - PATCH /admin/boarding-points/{boarding_point_id}/translations/{language_code}
Выбери один вариант и используй его последовательно.

2. Разрешить создать или обновить перевод для одной boarding point и одного language_code за запрос.

3. Поля перевода сделать максимально узкими и практичными:
   - city
   - address
   - notes
Не добавляй лишние поля, которых нет в текущем boarding point контексте.
...
4. Validation rules in service layer:
   ...
   - create/update semantics must be explicit and narrow
   - blank handling must be safe and explicit
   - do not change tour linkage or boarding point core fields through this endpoint

5. Return one narrow admin-facing response shape:
   - either updated boarding point translation object
...
Требования к тестам:
- focused tests only
- покрыть protected access for chosen boarding point translation endpoint
- покрыть successful create/update
- покрыть boarding point not found
