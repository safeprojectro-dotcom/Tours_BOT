...
| **5f v1** — Customer multi-quote summary / aggregate visibility | **Completed (implementation + stabilization reviewed)** — **`proposed_response_count`**, **`offers_received_hint`**, **`selected_offer_summary`** on **`GET .../custom-requests/{id}`**; My Requests detail read-only copy; **no** migration; §34 closure | **Track 5g.1** (below) |
| **5g.1** — Commercial mode classification (read-side) | **Completed (implementation + stabilization reviewed)** — **`CustomerCommercialMode`**; **`GET /mini-app/tours/{code}`** **`commercial_mode`** from **`tour.sales_mode`** (**`supplier_route_per_seat`** / **`supplier_route_full_bus`**); **`GET /mini-app/custom-requests/{id}`** **`commercial_mode`** **`custom_bus_rental_request`**; **no** migration; **no** execution/policy change; §35 closure | **5f** v2+ compare / localized hints; RFQ **notifications** / bot deep links; Track **5g** presentation/copy follow-on; **Phase 7.1 Step 6** / Track **0** baseline if paused |
...
**Separate** from the **V2** table rows above. **Authoritative** product sequence: [`docs/SUPPLIER_OFFER_TO_TOUR_BUSINESS_PLAN.md`](SUPPLIER_OFFER_TO_TOUR_BUSINESS_PLAN.md) (**B1–B13**). **As of 2026 (B8.4 docs sync):** **B9** / **B10**–**B10.5** complete (explicit bridge, draft, activate-for-catalog, full_bus package + boarding). **B8** recurring draft generation + **B8.2** activation policy + **B8.3** duplicate-active guard at B10.2 **implemented** / **documented** — **Layer** **A** booking semantics **unchanged**. **B10.6** (bot as router) **postponed**. **B7.3A** **media** **policy** **accepted** **(docs** **;** no **download**/**storage**/**render** **yet** **).** **B7.3** **implementation** **and** **B11** **not** **in** **this** **V2** **file**. **Product** **next** **—** **explicit** **choice** **(B7.3B** **/** **storage,** **B11,** **B10.6,** **B12/B13,** **etc.** **)** per [`docs/SUPPLIER_OFFER_TO_TOUR_BUSINESS_PLAN.md`](SUPPLIER_OFFER_TO_TOUR_BUSINESS_PLAN.md) §5. **Handoff:** [`docs/CHAT_HANDOFF.md`](CHAT_HANDOFF.md), [`docs/B10_X_SYNC_CHECKPOINT_2026.md`](B10_X_SYNC_CHECKPOINT_2026.md), [`docs/OPEN_QUESTIONS_AND_TECH_DEBT.md`](OPEN_QUESTIONS_AND_TECH_DEBT.md) (B7.3 / B8 / B10.6 / B11). **Do** **not** **conflate** **this** **B-line** **with** **V2** **RFQ** **/** **marketplace** **track** **numbers** **above.**
...
**Next safe track:** **5f v2+** (explicit) anonymized compare / localized hints, RFQ **notifications** / bot deep links, optional Track **5g** presentation/copy use of **`commercial_mode`**, or **Phase 7.1 Step 6** / Track **0** baseline if product pauses marketplace execution. (**Tracks 5b.3a–5f v1** through §34 and **Track 5g.1** §35 — **completed** for scoped slices; payment session creation remains **only** via existing **`payment-entry`**.)
