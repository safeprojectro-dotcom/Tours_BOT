...
**B9** (bridge **design**), **B10** (bridge **implementation**), **B10.1–B10.5** (draft, activate-for-catalog, **full_bus** Mini App semantics, **Layer A** package total, boarding fallback) are **implemented** and **smoke-accepted**. **Production path:** **Supplier offer #8 → Tour #4 → `open_for_sale` → Reserve bus → preparation → reservation → payment entry / mock complete → My bookings**. **Mini App** remains **execution truth**. **B8** (recurring draft `Tour` generation from template offer + audit; **B8.2** activation policy; **B8.3** duplicate-active guard at B10.2) is **implemented** / **stabilized** in-repo — see [`docs/CHAT_HANDOFF.md`](CHAT_HANDOFF.md), [`docs/OPEN_QUESTIONS_AND_TECH_DEBT.md`](OPEN_QUESTIONS_AND_TECH_DEBT.md), [`docs/HANDOFF_B8_3_DUPLICATE_ACTIVE_TOUR_ACTIVATION_GUARD.md`](HANDOFF_B8_3_DUPLICATE_ACTIVE_TOUR_ACTIVATION_GUARD.md). **Second** **vehicle** same **date** under **one** **offer** = **ops** / **future** **policy** (not automatic product behavior). **B10.6** (bot as router, not duplicate catalog) is **postponed**. **B7.3A** **media** **policy** **accepted** **(docs** **;** see [`docs/CHAT_HANDOFF.md`](CHAT_HANDOFF.md) **,** [`docs/OPEN_QUESTIONS_AND_TECH_DEBT.md`](OPEN_QUESTIONS_AND_TECH_DEBT.md) **,** [`docs/SUPPLIER_OFFER_PHOTO_MODERATION_CARD_GENERATION_DESIGN.md`](SUPPLIER_OFFER_PHOTO_MODERATION_CARD_GENERATION_DESIGN.md) **B7.3A** **).** **B7.3** **implementation** **(download,** **object** **storage,** **pixels,** **Telegram** **media** **send** **)** **remains** **unimplemented** **until** **an** **explicit** **slice** **(e.g. ** **B7.3B** **stub** **/** **storage** **).** **No** **automatic** **publish** **from** **media** **approval.** **B11** **—** **`/start supoffer_<id>`** **→** **Mini** **App** **`/tours/{code}`** **when** **active** **execution** **link** **+** **`OPEN_FOR_SALE`** **+** **catalog** **visibility** **(read-only** **`resolve_sup_offer_start_mini_app_routing`**) **`;`** **polish** **/** **edge** **cases** **per** [`docs/OPEN_QUESTIONS_AND_TECH_DEBT.md`](OPEN_QUESTIONS_AND_TECH_DEBT.md) **. Canonical** **B10** **checkpoint:** [`B10_X_SYNC_CHECKPOINT_2026.md`](B10_X_SYNC_CHECKPOINT_2026.md). **Docs** **sync** **(conversion** **chain** **for** **ops):** [`docs/CURSOR_PROMPT_SUPPLIER_OFFER_CONVERSION_DOCS_SYNC_C2B9B.md`](CURSOR_PROMPT_SUPPLIER_OFFER_CONVERSION_DOCS_SYNC_C2B9B.md) **/** **[`docs/SUPPLIER_OFFER_TO_TOUR_BRIDGE_READINESS_C2B9A.md`](SUPPLIER_OFFER_TO_TOUR_BRIDGE_READINESS_C2B9A.md)** **(C2B9A** **audit** **)** **.**

**Conversion chain (implementation reality, C2B9B):** Supplier offer **→** AI/admin packaging (`generate_packaging_draft`, `approve_packaging_for_publish`) **→** moderation **+** publish readiness (`GET …/review-package`, `conversion_closure`) **→** Telegram showcase publish **when** `publish_showcase_channel` **enabled** **(C2B8B)** **or** HTTP publish **→** **explicit** Tour bridge (**`POST …/tour-bridge`** **or** Telegram **Link tour** when **`create_tour_bridge` enabled** — **C2B10T-A**) **→** activate Tour for catalog when allowed (`activate_tour_for_catalog`, B8.3 guard; **Admin** **API** **or** Telegram **List for sale / În catalog** when **`activate_tour_for_catalog.enabled`** — **C2B10T-B**) **→** create/activate execution link (`create_execution_link`; **HTTP** **`POST …/execution-link`** **or** Telegram **Booking link / Link rezervări** when **`create_execution_link.enabled`** — **C2B10T-C** **;** published-offer link wizard **/** other HTTP when manual tour selection is needed) **→** **B11** `/start supoffer_<id>` **→** exact Mini App tour **only** when active execution link **+** `OPEN_FOR_SALE` **+** catalog visibility **→** Layer A reservation/payment. **Ops** order **for** bridge **vs.** channel publish **may** **vary**, **but** **execution** **link** **normally** **expects** **`published`** **lifecycle**.
...
**Admin operator workflow Slice C1 / C1.1 / C2A:** Telegram **private admin offer card** displays **`operator_workflow`** text sourced via **`SupplierOfferReviewPackageService.review_package()`** — **display-only** (**C1.1** compact formatting **;** full fields on **`GET …/review-package`**). **C2A:** optional buttons **`review_package_refresh`** and **`get_showcase_preview`** only (read-only callbacks — **no** POST). **C2B8B** adds **separate** Telegram **showcase** **`publish_showcase_channel`** ( **Publică / Publish** **) **—** propose/confirm**, **double** **`review-package`** **re-read**, **not** part of C2A read-only set **.**

**Slice C2B1 implemented:** Telegram workflow adds **`approve_packaging_for_publish`** ( **`operator_workflow.actions`**, **`enabled`** only **)** **:** confirmation flow **;** confirm **re-reads `GET …/review-package`** **;** executes **only** **`SupplierOfferPackagingReviewService.approve`** if still enabled (**`accept_warnings`** when **`needs_admin_review`**) **;** **`reviewed_by`** **`telegram:{admin_id}`** **;** cancel **no** mutation **;** refreshed card after success.

**Slice C2B2 implemented:** Telegram adds **`generate_packaging_draft`** with mandatory confirmation **;** button only when action **`enabled`** **;** confirm **re-reads `GET …/review-package`** and executes **only** **`SupplierOfferPackagingService.generate_and_persist`** if still enabled **;** **does not** auto-approve packaging **(** **C2B1** separate **)** **;** cancel **no** mutation **;** refreshed card after success/error handling **.** Legacy **Aprobă / Respinge** unchanged **;** **no** **`approve_offer_moderation`** workflow button **;** **`create_tour_bridge`** **on** **Telegram** **—** **C2B10T-A** **(Link tour** **)** when **`enabled`** **`;`** **`activate_tour_for_catalog`** **—** **C2B10T-B** **(List for sale / În catalog** **)** when **`enabled`** **`;`** **`create_execution_link`** **—** **C2B10T-C** **(Booking link** **/** **Link rezervări** **)** when **`enabled`** **`;`** **HTTP** **/** **published-offer** **wizard** **remain** **for** **flows** **that** **need** **manual** **tour** **input** **`;`** **no** batch **.** **Showcase** **publish** **:** **C2B8B** **.**

**Telegram Button UX Policy (accepted, docs-only):** Card is **not** a developer console **`;`** labels **1–2 words**, action/outcome oriented **`;`** **no** codes/endpoints/enums in button text **`;`** disabled workflow actions **hidden** **`;`** safe reads — **no** confirmation **`;`** safe mutations — **confirmation + re-read `review-package`** **`;`** preferred order **:** observe/read → packaging → **Link tour** ( **C2B10T-A** **)** **→** **List for sale** ( **C2B10T-B** **)** **→** legacy moderation → **Publish** ( **C2B8B** **)** **→** **Booking link** ( **C2B10T-C** **)** **`;`** **`publish_showcase_channel`** **—** **C2B8B** **(** **double** **re-read** **)** **`;`** **`create_tour_bridge`** **—** **C2B10T-A** **(** **double** **re-read** **)** **`;`** **`activate_tour_for_catalog`** **—** **C2B10T-B** **(** **double** **re-read** **)** **`;`** **`create_execution_link`** **on** **card** **—** **C2B10T-C** **(** **double** **re-read** **)** **`;`** **HTTP** **/** **wizard** **still** **available** **elsewhere** **.** Label direction **:** **`review_package_refresh`** → **Actualizează / Refresh** **`;`** **`get_showcase_preview`** → **Preview** **`;`** **`generate_packaging_draft`** → **Generează text** or **Pregătește** **`;`** **`approve_packaging_for_publish`** → **Aprobă text / Aprobă pachet** **`;`** **`create_tour_bridge`** → **Link tour / Leagă tur** **`;`** **`activate_tour_for_catalog`** → **List for sale / În catalog** **`;`** **`create_execution_link`** → **Booking link / Link rezervări** **`;`** legacy moderation → **Aprobă oferta**, **Respinge oferta** **;** showcase publish → **Publică / Publish** **.**

**Slice C2B3 implemented:** Labels and **`_detail_keyboard`** row order only **`;`** subsequent slices add **packaging** **/** **media** **/** **showcase** **/** **bridge** **callbacks** **per** **C2B1–C2B8B** **,** **C2B10T-A** **,** **C2B10T-B** **,** **C2B10T-C** **`;`** legacy moderation approve **`/`** reject disambiguated **`;`** alternative execution-link entry — **published-offer** **UI** **/** **HTTP** **/** **wizard** **.**
...
**Slice C2B5 implemented:** **`GET …/review-package`** adds **`cover_media_quality_review`** (**read-only** deterministic warnings for showcase cover vs B7.1 **`media_review`**) **`;`** merged into **`warnings`** **`/`** **`operator_workflow`** **`;`** **`publish_showcase_channel`** readiness respects hard subset (**C2B8A** **`;`** **`POST …/publish`** unchanged **).**

**Slice C2B8A implemented:** **`operator_workflow.publish_showcase_channel`** **`disabled`** until **`approved_for_publish`** packaging **`+`** cleared hard **`cover_media_quality_review`** codes (**photo-mode-sensitive**) **`;`** **`POST …/publish`** unchanged **.**

**Slice C2B6 implemented:** Telegram **`request_cover_photo_replacement`** (**Cere poză** **`/`** **Request photo**) **`→`** **`media/request-replacement`** semantics via **`SupplierOfferMediaReviewService.request_replacement`** **`;`** **no** cover PATCH**,** **`review-package`** **`publish_showcase_channel`** reflects C2B8A gates **`;`** **`POST …/publish`** unchanged **.**
**Slice C2B7.1 implemented:** **`PUT /admin/supplier-offers/{offer_id}/cover`** sets **`SupplierOffer.cover_media_reference`** only ( **`telegram_photo:{file_id}`**, HTTPS URL**,** or **`null`**) **`;`** central admin only **`;`** **no** publish**,** **no** **`media_review`** mutation**,** **no** file upload **.**
...
| **B11** | Telegram deep link routing | **`supoffer_<id>`** **→** safe **Mini** **App** **`/tours/{code}`** **when** **active** **execution** **link** **+** **`OPEN_FOR_SALE`** **+** **catalog** **visibility** **(** **`resolve_sup_offer_start_mini_app_routing`** **)** **`;`** **else** **generic** **intro** **/** **landing** **.** |
