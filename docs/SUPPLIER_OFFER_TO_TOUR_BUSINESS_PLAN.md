...
**Slice C2B — design accepted (planned implementation C2B1):** next Telegram mutation slice is **packaging-only** — **`approve_packaging_for_publish`** from **`operator_workflow.actions`**, only when **`enabled`**, **mandatory confirmation**, **re-read `GET …/review-package` before execution**, **no duplicate `approve_offer_moderation` button** (legacy **Aprobă / Respinge** remain). **Postponed:** **`publish_showcase_channel`**, **`activate_tour_for_catalog`**, **`create_execution_link`**, **`create_tour_bridge`** **;** **no** batch workflow button. Every **new** mutating workflow button policy: confirmation **+** fresh **`review-package`** gate **+** **`enabled`** check **+** **no** hidden chains.
