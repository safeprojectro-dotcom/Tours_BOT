...
### Status
open (Steps **4–15** group + private **`grp_*`** + **`grp_followup`** persistence + **chain tests** + **admin visibility** + **`assign-operator`** + **read-side work-state labels** + **`mark-in-work`** + **`resolve-group-followup`** + **`group_followup_resolution_label`** + **`group_followup_queue_state`** / **`group_followup_queue`** filter for **`group_followup_start`** **shipped**; **Step 16** **private** resolved-followup confirmation forward path — **broad** claim / **notifications** / **full operator workflow** / **`per_seat` / `full_bus`** still **not** done here — see **`docs/CHAT_HANDOFF.md` Next Safe Step**)