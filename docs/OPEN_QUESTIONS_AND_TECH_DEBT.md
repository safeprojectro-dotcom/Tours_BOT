...
**Slice B + Slice C1 + C1.1 implemented:** read-model on **`GET …/review-package`** **;** Telegram admin offer card appends **display-only** **`operator_workflow`** (**C1.1** compact block **;** **`review-package`** remains source of truth) (see **`ADMIN_OPERATOR_WORKFLOW.md`**).
...
- **Slice C1 / C1.1:** Telegram **private admin offer card** shows the same **`operator_workflow`** projection (**display-only**); **C1.1** tightens Telegram copy (compact lines; **`review-package`** remains source of truth); **no** POST execution from the chat formatter **;** **no** publish **/** bridge **/** activate **/** execution-link buttons added. **Possible Slice C2:** Telegram inline buttons **from** **`operator_workflow.actions`**, safe/low-risk first **+** explicit confirmation **— do not start automatically.**
