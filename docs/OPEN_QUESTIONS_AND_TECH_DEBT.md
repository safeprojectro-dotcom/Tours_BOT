...
- **B15 — Admin Publishing Console (B15A design + B15B read-only API):** **B15A** — **[`docs/B15_ADMIN_PUBLISHING_CONSOLE_DESIGN.md`](B15_ADMIN_PUBLISHING_CONSOLE_DESIGN.md)** (model, rollout **B15B–G**). **B15B** — **[`docs/B15B_READ_ONLY_ADMIN_PUBLISHING_CONSOLE.md`](B15B_READ_ONLY_ADMIN_PUBLISHING_CONSOLE.md)** **`GET /admin/publishing-console`** (candidate cards; **no** publish/mutation). **Still future:** **publish / schedule / auto-draft / auto-send** from the console, RFQ/service cards, scheduler (**B15D–G** per design). **Exact-tour CTA** for supplier-offer channel posts remains **B15C** (not B15B). **Product decisions pending/accepted:** console **UX** surface beyond HTTP (Telegram card / web); **auto-draft** vs **limited auto-publish** trust model (**B15F/G**).
