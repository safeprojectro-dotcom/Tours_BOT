...
- **Temporary hold expiry persistence (B14E):** Smoke **orders #52/#53** on **Tour #6** stayed **`awaiting_payment` / `active` / `active_temporary_hold`** with **`seats_available = 7`** after **`reservation_expires_at`** passed when checked via **admin read** — **B14E** traces **`ReservationExpiryService`**, **lazy** vs **worker** vs **HTTP commit** behavior, and admin lifecycle (**no clock check on `reservation_expires_at`**). **Fix/remediation not claimed** — **[`docs/B14E_TEMP_HOLD_EXPIRY_RELEASE_DIAGNOSTIC.md`](B14E_TEMP_HOLD_EXPIRY_RELEASE_DIAGNOSTIC.md)**; follow-up **B14F/B14G** as needed.
