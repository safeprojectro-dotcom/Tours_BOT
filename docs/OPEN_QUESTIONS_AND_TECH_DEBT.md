...
- **Layer A / Mini App — reservation preparation vs catalog open_for_sale (B13G follow-up, 2026-05-10; B14A diagnostic, 2026-05-09):** **Production:** **Tour #6** (**`tour_code`** **`B10-SO12-04fb1f`**, supplier offer **#12**) — Mini App **catalog** lists the offer; supplier-offer landing and **exact** tour detail show **bookable** semantics (**seats**, **price**); **reservation preparation** returns **tour is not available for reservation preparation** (or equivalent). **This is separate from B13:** showcase publish audit, bridge, catalog activation, and **execution link #5** **all verified OK** for this smoke. **B14A code read:** prep **`404`** when **`PrivateReservationPreparationService.get_preparable_tour`** returns **`None`** — for **`per_seat`**, empty **`boarding_points`** yields **`None`** while **`MiniAppTourDetailService`** may still render detail (**see** **[`docs/B14A_CHANNEL_LINK_AND_RESERVATION_READINESS_DIAGNOSTIC.md`](B14A_CHANNEL_LINK_AND_RESERVATION_READINESS_DIAGNOSTIC.md)** §4). **Next:** confirm **Tour #6** **`boarding_points`** in admin/DB; then **B14C** data fix or UX/docs alignment if product wants prep-blocking rules to match visible “bookable” copy.
