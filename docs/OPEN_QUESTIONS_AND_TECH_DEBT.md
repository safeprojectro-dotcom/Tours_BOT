...
- **Phase 6 / Step 29** (narrow mutation): **`POST /admin/orders/{order_id}/move`** (**`app/services/admin_order_move_write.py`**) — move when Step **28**-style readiness passes; **no** payment-row writes, **no** reconciliation semantics change. **Broad** move workflow/history/audit is **not** implemented — **read-side move/history visibility** is a **forward** narrow slice (**Phase 6 / Step 30**, `docs/CHAT_HANDOFF.md`). **Payment**-row admin **mutations** (refund/capture/cancel-payment) remain **postponed** until explicit product approval.
...
### Status
open (read-side **partially mitigated** for Phase 6 Step 1 list API, Steps 16–17 order-detail hints/preview, Step 18 handoff queue reads, Steps **19–22** narrow handoff mutations, Steps **23–26** narrow order mutations, Step **27** **`ready_for_departure_paid`** lifecycle read refinement, Step **28** move-readiness hints, Step **29** narrow move **mutation**; **section 1** remains open for semantics and non-projected consumers; **move audit/history** read slice and broader admin order workflow / full lifecycle redesign **postponed** — see **`docs/CHAT_HANDOFF.md` Next Safe Step**)
