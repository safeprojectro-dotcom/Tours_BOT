...
- **Phase 7 / Step 6** adds **narrow private `/start`** handling in **`app/bot/handlers/private_entry.py`**: **`match_group_cta_start_payload`**; **`start_grp_private_intro`** / **`start_grp_followup_intro`** in **`app/bot/messages.py`**; then catalog — **no** handoff persistence. Tests: **`test_group_private_cta`** (matcher), **`test_private_start_grp_messages`**.
- **Forward (documented):** **`docs/CHAT_HANDOFF.md` Next Safe Step** — **Phase 7 / Step 7** — **narrow runtime handoff persistence foundation** (payload / category → **persisted** `handoff` row — **not** full operator workflow). Optional **later** slices: **i18n** polish, group **rate limits** — **not** Step **7** by default.
- **Still postponed** until explicit future slices (beyond Step **6** scope): **runtime-created** handoff rows from the **`grp_*`** chain (**Step 7** targets the **foundation** slice), **operator workflow engine**, **full** group assistant; **live** validation **may** still depend on **Railway** / deploy health — **product scope** unchanged. Optional later: **i18n** acks, **rate limits**, **structured logging** — product-scoped.

### Status
open (Steps **4–6** group + private **`grp_*`** slices **shipped**; persistence / operator workflow / full assistant **not** done — see **`docs/CHAT_HANDOFF.md` Next Safe Step**)