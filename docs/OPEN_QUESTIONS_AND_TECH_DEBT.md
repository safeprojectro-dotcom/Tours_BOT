...
- **Continuity note:** **group** chat path (Steps **3–4**) remains **reply-only** (no DB handoff from group messages); **private** **`grp_followup`** chain persists intent (Step **7**), is **test-covered** (Step **8**), is **operator-visible in admin reads** (Steps **9–11**), can be **narrow-assigned** via **`assign-operator`** (Step **10**), **taken in-work** via **`mark-in-work`** (Step **12**) after assignment, with **read-side triage labels** (Step **11**). **Phase 7 / Step 13** next — **narrow close/resolve** for **`in_review`** **`group_followup_start`** — **broad** claim / **internal ops** takeover / **notifications** still **out of scope** unless scoped — see **`docs/CHAT_HANDOFF.md` Next Safe Step**.
- **Forward (documented):** **`docs/CHAT_HANDOFF.md` Next Safe Step** — **Phase 7 / Step 13** — **narrow operator close/resolve** mutation for **`in_review`** **`group_followup_start`** handoffs. Optional **later** slices: **i18n** polish, group **rate limits**. **Future track** **`per_seat` / `full_bus`**: **separate** — **not** part of these narrow admin handoff slices.
...
### Status
open (Steps **4–12** group + private **`grp_*`** + **`grp_followup`** persistence + **chain tests** + **admin visibility** + **`assign-operator`** + **read-side work-state labels** + **`mark-in-work`** for **`group_followup_start`** **shipped**; **Step 13** narrow **close/resolve** forward path — **broad** claim / **notifications** / **full operator workflow** / **`per_seat` / `full_bus`** still **not** done here — see **`docs/CHAT_HANDOFF.md` Next Safe Step**)