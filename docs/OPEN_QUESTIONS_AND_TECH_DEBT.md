...
- **Continuity note:** **group** chat path (Steps **3–4**) remains **reply-only** (no DB handoff from group messages); **private** **`grp_followup`** chain persists intent (Step **7**), is **test-covered** (Step **8**), and is **operator-visible in admin reads** (Step **9**); **Phase 7 / Step 10** next — **narrow assignment** for **`group_followup_start`** **only** — **broad** workflow / **internal ops** claim still **out of scope** unless scoped — see **`docs/CHAT_HANDOFF.md` Next Safe Step**.
- **Forward (documented):** **`docs/CHAT_HANDOFF.md` Next Safe Step** — **Phase 7 / Step 10** — **narrow operator assignment** for **`group_followup_start`** handoffs **only**. Optional **later** slices: **i18n** polish, group **rate limits**.
...
### Status
open (Steps **4–9** group + private **`grp_*`** + **`grp_followup`** persistence + **chain tests** + **admin visibility for `group_followup_start`** **shipped**; **Step 10** narrow assignment forward path — **full operator workflow** still **not** done — see **`docs/CHAT_HANDOFF.md` Next Safe Step**)