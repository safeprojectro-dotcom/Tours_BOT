...
- **Phase 6 / Step 26** adds **`POST /admin/orders/{order_id}/mark-ready-for-departure`**: **confirmed + paid + active cancellation** only, **`tour.departure_datetime` strictly in the future** (UTC); **`booking_status` → `ready_for_departure` only**; **no** seat mutation, **no** payment-row mutation. **`admin_order_lifecycle`** projection for **`ready_for_departure`** was **intentionally not** expanded in this slice (may still show **`other`**) — **Phase 6 / Step 27** in `docs/CHAT_HANDOFF.md` addresses **read-side** refinement. **Broader** order workflow (move, refund, broad editor) remains **postponed**.
...
### Status
open (read-side **partially mitigated** for Phase 6 Step 1 list API, Steps 16–17 order-detail hints/preview, Step 18 handoff queue reads, Steps **19–22** narrow handoff mutations, Steps **23–26** narrow order mutations (cancel + duplicate + no-show + ready-for-departure); **`ready_for_departure` lifecycle projection** refinement **postponed** to Step **27**; **section 1** remains open for semantics and non-projected consumers; broader admin order workflow **postponed**)
