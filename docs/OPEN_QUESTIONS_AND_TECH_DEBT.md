...
### Status
open (Steps **4–17** + **final consolidation** polish: group + private **`grp_*`** + **`grp_followup`** persistence + **chain tests** + **admin visibility** + **`assign-operator`** + **read-side work-state labels** + **`mark-in-work`** + **`resolve-group-followup`** + **`group_followup_resolution_label`** + **`group_followup_queue_state`** / **`group_followup_queue`** filter + **private** **`start_grp_followup_resolved_intro`** + **`group_followup_private_intro_key`** / **`start_grp_followup_readiness_*`** + **aligned** private/admin wording for **`group_followup_start`** **shipped**; **forward** — **Phase 7 review / closure** — **operator chat** / **handoff push notifications** / **full operator workflow** / **`per_seat` / `full_bus`** still **not** in this chain — see **`docs/CHAT_HANDOFF.md` Next Safe Step**)