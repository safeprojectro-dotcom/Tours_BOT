...
- **Phase 6 / Step 19** adds **`POST /admin/handoffs/{handoff_id}/mark-in-review`** (`open`→`in_review`, idempotent `in_review`, `closed` rejected). **Phase 6 / Step 20** adds **`POST /admin/handoffs/{handoff_id}/close`** (`in_review`→`closed`, idempotent `closed`, `open` rejected; unexpected statuses rejected narrowly). Admin **`close`** intentionally **depends** on moving **`open` → `in_review`** first — not a shortcut from **`open`**. Handoff **status** progression under **`/admin/*`** remains **intentionally narrow** (no general state machine). **Admin** **claim/assign**, **reopen**, and broader operator **workflow** remain **postponed** (next planned narrow slice: **Phase 6 / Step 21** per `docs/CHAT_HANDOFF.md`; internal **ops** JSON remains a **separate** surface from **`/admin/*`**).
...
### Status
open (read-side **partially mitigated** for Phase 6 Step 1 list API, Steps 16–17 order-detail hints/preview, Step 18 handoff queue reads, and Steps **19–20** narrow handoff mutations; **section 1** remains open for semantics and non-projected consumers)
