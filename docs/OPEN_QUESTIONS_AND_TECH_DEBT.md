...
- **Phase 7 / Step 5** adds **`app/services/group_private_cta.py`** + **`app/schemas/group_private_cta.py`**: **`https://t.me/<bot>?start=grp_private`** (generic) or **`…grp_followup`** (after handoff-category line); appended to group replies — **no** private **`/start`** branching on **`grp_*`** in this step, **no** persistence. Tests: **`tests/unit/test_group_private_cta.py`**.
- **Forward (documented):** **`docs/CHAT_HANDOFF.md` Next Safe Step** — **Phase 7 / Step 6** = **narrow private `/start` branching for `grp_private` / `grp_followup`** (implementation when scheduled). **Private** branching for **`grp_*`** remains **postponed** until that step.
- **Still postponed** until explicit future slices (beyond Step **6** scope): **real handoff persistence** from group runtime, **operator workflow engine**, **full** group assistant; optional later: **i18n** acks, **rate limits**, **structured logging** — product-scoped.

### Status
open (Steps **4–5** group runtime slices **shipped**; persistence / operator workflow / full assistant **not** done — see **`docs/CHAT_HANDOFF.md` Next Safe Step**)