...
- **Temporary hold expiry persistence (B14E / B14F):** Smoke **orders #52/#53** on **Tour #6** exposed **lazy expiry** without **`commit`** on several **GET** paths (and **`payment-entry`** **`rollback`** undoing expiry). **B14F (code):** **`lazy_expire_due_reservations_commit_if_any`** in **`app/services/reservation_expiry.py`** + call sites — see **[`docs/B14E_TEMP_HOLD_EXPIRY_RELEASE_DIAGNOSTIC.md`](B14E_TEMP_HOLD_EXPIRY_RELEASE_DIAGNOSTIC.md)** §10 and **[`docs/HANDOFF_B14F_TEMP_HOLD_EXPIRY_PERSISTENCE_FIX_TO_NEXT_STEP.md`](HANDOFF_B14F_TEMP_HOLD_EXPIRY_PERSISTENCE_FIX_TO_NEXT_STEP.md)**. **Post-deploy:** confirm stale prod rows (**#52/#53** or others) via committing path or **`run_reservation_expiry_once`**; **handoff #86** on **#53** still operator-owned. **Background worker** remains useful for hygiene without customer/mini-app traffic.
