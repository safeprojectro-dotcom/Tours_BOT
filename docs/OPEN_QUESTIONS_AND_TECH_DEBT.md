...
- **Continuity note:** **group** chat path (Steps **3–4**) remains **reply-only** (no DB handoff from group messages); **private** **`grp_followup`** chain persists intent (Step **7**) and is **covered by tests** (Step **8**) — **narrow operator visibility** (Step **9**) next; **automatic assignment** / **full workflow** still **postponed** — see **`docs/CHAT_HANDOFF.md` Next Safe Step**.
- **Forward (documented):** **`docs/CHAT_HANDOFF.md` Next Safe Step** — **Phase 7 / Step 9** — **narrow operator visibility/readiness** for **`group_followup_start`** handoffs (**read-side**, **not** assignment/claim). Optional **later** slices: **i18n** polish, group **rate limits**.
- **Still postponed:** **automatic operator assignment** from **`grp_followup`** entry, **operator workflow engine**, **full** group assistant; **live** Telegram validation **may** still depend on **Railway** / bot deploy health — **product scope** unchanged. Optional later: **i18n** acks, **rate limits**, **structured logging** — product-scoped.

### Status
open (Steps **4–8** group + private **`grp_*`** + **`grp_followup`** persistence + **focused chain tests** **shipped**; **Step 9** operator visibility forward path — **full operator workflow / assignment** still **not** done — see **`docs/CHAT_HANDOFF.md` Next Safe Step**)