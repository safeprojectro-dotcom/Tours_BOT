...
- **Continuity note:** **group** chat path (Steps **3–4**) remains **reply-only** (no DB handoff from group messages); **private** **`grp_followup`** chain persists intent (Step **7**), is **test-covered** (Step **8**), is **operator-visible in admin reads** (Step **9**), and can be **narrow-assigned** via **`assign-operator`** (Step **10**). **Phase 7 / Step 11** next — **read-side operator work-state visibility** for **assigned** **`group_followup_start`** — **broad** claim / **internal ops** takeover / **notifications** still **out of scope** unless scoped — see **`docs/CHAT_HANDOFF.md` Next Safe Step**.
- **Forward (documented):** **`docs/CHAT_HANDOFF.md` Next Safe Step** — **Phase 7 / Step 11** — **read-side operator work-state visibility** for **assigned** **`group_followup_start`** handoffs. Optional **later** slices: **i18n** polish, group **rate limits**.
...
### Status
open (Steps **4–10** group + private **`grp_*`** + **`grp_followup`** persistence + **chain tests** + **admin visibility** + **narrow `assign-operator`** for **`group_followup_start`** **shipped**; **Step 11** read-side **post-assignment** visibility forward path — **broad** claim / **notifications** / **full operator workflow** still **not** done — see **`docs/CHAT_HANDOFF.md` Next Safe Step**)