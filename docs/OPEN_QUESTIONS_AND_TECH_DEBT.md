...
- **Phase 6 / Step 23** adds **`POST /admin/orders/{order_id}/mark-cancelled-by-operator`**: **operator cancellation** for **active temporary holds** only (aligned with admin “active hold” lifecycle); **paid** orders and other disallowed state combinations are **rejected**; **no** payment-row mutation, **no** refund/reconciliation change. **Broader** order corrections (duplicate marking, move/merge, paid cancellations, broad status editor) remain **postponed**; next forward admin slice: **Phase 6 / Step 24** per `docs/CHAT_HANDOFF.md`.
...
### Status
open (read-side **partially mitigated** for Phase 6 Step 1 list API, Steps 16–17 order-detail hints/preview, Step 18 handoff queue reads, Steps **19–22** narrow handoff mutations, and Step **23** narrow operator cancel; **section 1** remains open for semantics and non-projected consumers; broader admin order workflow **postponed**)
