...
- **Continuity note:** **group** chat path (Steps **3–4**) remains **reply-only** (no DB handoff from group messages); **private** **`grp_followup`** chain now persists intent — **operator assignment** / **full workflow** still **postponed** — see **`docs/CHAT_HANDOFF.md` Next Safe Step**.
- **Forward (documented):** **`docs/CHAT_HANDOFF.md` Next Safe Step** — **Phase 7 / Step 8** — **focused runtime/bot-flow test coverage** for the **`grp_followup`** chain. Optional **later** slices: **i18n** polish, group **rate limits**.
...
### Status
open (Steps **4–7** group + private **`grp_*`** + **`grp_followup`** persistence **shipped**; **Step 8** test-coverage forward path — **operator workflow / full assistant** still **not** done — see **`docs/CHAT_HANDOFF.md` Next Safe Step**)