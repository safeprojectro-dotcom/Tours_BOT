...
- **Layer A / Mini App — reservation preparation vs catalog open_for_sale (B13G / B14A / B14C):** **Production Tour #6** (**`B10-SO12-04fb1f`**) observed **prep 404** before B14C (**empty `boarding_points`** on a **per_seat** tour while detail loaded) — see **[`docs/B14A_CHANNEL_LINK_AND_RESERVATION_READINESS_DIAGNOSTIC.md`](B14A_CHANNEL_LINK_AND_RESERVATION_READINESS_DIAGNOSTIC.md)**. **B14C (implemented):** **new** bridge-created tours materialize **`boarding_points`** from **`supplier_offers.boarding_places_text`** (**`supplier_offer_tour_bridge_service`**) — **does not** alter **existing** tours or production data by migration. **Existing Tour #6** (and any tour bridged **before** deploy) may still need **manual** boarding-point remediation or admin ops — then **`GET /mini-app/tours/{code}/preparation`** smoke; forward prompt e.g. **B14D** production-safe remediation / Mini App reservation smoke.
