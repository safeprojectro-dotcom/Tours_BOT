...
**Slice B + Slice C1 + C1.1 + C2A implemented:** read-model on **`GET …/review-package`** **;** Telegram admin offer card appends **display-only** **`operator_workflow`** (**C1.1** compact block **;** **`review-package`** remains source of truth **;** **C2A** optional read-only buttons **`review_package_refresh`** / **`get_showcase_preview`**) (see **`ADMIN_OPERATOR_WORKFLOW.md`**).

**Slice C2B — design accepted (implementation slice C2B1 planned):** packaging-only mutation — **`approve_packaging_for_publish`** from **`operator_workflow.actions`** only when **`enabled`** **;** mandatory **two-step confirmation** **;** **re-read `GET …/review-package` before execution** **;** execute **only if action still enabled** **;** **no hidden chained actions** **;** **no batch workflow** button. Legacy **Aprobă / Respinge** remain moderation **;** **do not** add **`approve_offer_moderation`** via **`operator_workflow`** until legacy consolidation. **Postponed for Telegram workflow buttons:** **`publish_showcase_channel`**, **`activate_tour_for_catalog`**, **`create_execution_link`**, **`create_tour_bridge`**.
...
- **Slice C1 / C1.1 / C2A:** Telegram **private admin offer card** shows the same **`operator_workflow`** projection (**display-only**); **C1.1** tightens Telegram copy (compact lines **;** **`review-package`** remains source of truth); **C2A** adds inline callbacks only for **`review_package_refresh`** and **`get_showcase_preview`** (**no** POST **/** **no** publish send); **no** publish **/** bridge **/** activate **/** execution-link mutating buttons yet. **C2B design + planned C2B1:** see paragraph above **;** broader mutations remain **product-gated**.
