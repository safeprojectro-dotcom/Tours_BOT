...
- **Layer A / Mini App — reservation preparation vs catalog open_for_sale (B13G follow-up, 2026-05-10):** **Production:** **Tour #6** (**`tour_code`** **`B10-SO12-04fb1f`**, supplier offer **#12**) — Mini App **catalog** lists the offer; supplier-offer landing and **exact** tour detail show **bookable** semantics (**seats**, **price**); **reservation preparation** returns **tour is not available for reservation preparation** (or equivalent). **This is separate from B13:** showcase publish audit, bridge, catalog activation, and **execution link #5** **all verified OK** for this smoke. **Next (scoped ticket):** diagnose **`MiniAppReservationPreparationService`** (or equivalent) **readiness** — e.g. sales window / deadline, seat policy, tour fields, API response contract — **not** a B13 retry/resend or execution-link mutation.
