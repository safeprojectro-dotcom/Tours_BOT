...
- **Continuity note:** **group** chat path (Steps **3–4**) remains **reply-only** (no DB handoff from group messages); **private** **`grp_followup`** chain persists intent (Step **7**), is **test-covered** (Step **8**), is **operator-visible in admin reads** (Steps **9–11**), can be **narrow-assigned** via **`assign-operator`** (Step **10**), **taken in-work** via **`mark-in-work`** (Step **12**) after assignment, **resolved** via **`resolve-group-followup`** (Steps **13–14**), with **read-side triage labels** (Step **11**) and **resolved** label (Steps **13–14**). **Phase 7 / Step 15** next — **narrow post-resolution operator visibility / queue refinement** — **broad** claim / **internal ops** takeover / **notifications** still **out of scope** unless scoped — see **`docs/CHAT_HANDOFF.md` Next Safe Step**.
- **Forward (documented):** **`docs/CHAT_HANDOFF.md` Next Safe Step** — **Phase 7 / Step 15** — **narrow post-resolution operator visibility / queue refinement** for **`group_followup_start`**. Optional **later** slices: **i18n** polish, group **rate limits**. **Future track** **`per_seat` / `full_bus`**: **separate** — **not** part of these narrow admin handoff slices.
...
### Status
open (Steps **4–14** group + private **`grp_*`** + **`grp_followup`** persistence + **chain tests** + **admin visibility** + **`assign-operator`** + **read-side work-state labels** + **`mark-in-work`** + **`resolve-group-followup`** + **`group_followup_resolution_label`** for **`group_followup_start`** **shipped**; **Step 15** **post-resolution** queue/visibility refinement forward path — **broad** claim / **notifications** / **full operator workflow** / **`per_seat` / `full_bus`** still **not** done here — see **`docs/CHAT_HANDOFF.md` Next Safe Step**)