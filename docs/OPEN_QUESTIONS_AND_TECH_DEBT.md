...
- **Explicit admin chain** is **test-proven** in **`tests/unit/test_supplier_offer_catalog_conversion_closure.py`** **:** bridge **`POST .../tour-bridge`**, **`POST .../activate-for-catalog`**, **`MiniAppCatalogService.list_catalog`** sees **`OPEN_FOR_SALE`** Tour **before** execution link exists **;** **`POST .../execution-link`** **;** showcase **`POST .../publish`** **(** **B15C:** execution link **before** publish**)** **;** landing **`MiniAppSupplierOfferLandingService`**, B11 **`resolve_sup_offer_start_mini_app_routing`**, review-package **`conversion_closure`** all **`true`**, **`next_missing_step`** **`null`** **.**
...
**Slice B + Slice C1 + C1.1 + C2A + C2B1 + C2B2 implemented:** read-model on **`GET …/review-package`** **;** Telegram admin offer card appends **`operator_workflow`** (**C1.1** compact block **;** **`review-package`** source of truth **;** **C2A** read-only buttons **`review_package_refresh`** / **`get_showcase_preview`** **;** **C2B1** packaging approve **`approve_packaging_for_publish`** **;** **C2B2** **`generate_packaging_draft`** — see bullets below) (see **`ADMIN_OPERATOR_WORKFLOW.md`** **;** **Telegram Button UX Policy:** section below).

**Slice C2B1 implemented:** Telegram workflow **`approve_packaging_for_publish`** from **`operator_workflow.actions`** only when **`enabled`** **;** mandatory **two-step confirmation** **;** confirm **re-reads `GET …/review-package`** **;** executes **only `SupplierOfferPackagingReviewService.approve`** if action **still enabled** (**`accept_warnings`** for **`needs_admin_review`**) **;** **`reviewed_by`** **`telegram:{admin_id}`** **;** cancel **no** mutation **;** refreshed card after success.

**Slice C2B2 implemented (docs sync):** Telegram **`generate_packaging_draft`** button only when **`enabled`** in **`operator_workflow.actions`** **;** first tap — confirmation only **;** confirm **re-reads `GET …/review-package`** and runs **only `SupplierOfferPackagingService.generate_and_persist`** if action **still enabled** **;** **does not** auto-approve packaging (**C2B1 remains separate**) **;** cancel — no mutation **;** refreshed card after success/error handling **.** Legacy **Aprobă / Respinge** unchanged **;** **no** **`approve_offer_moderation`** workflow button **;** **no** publish **/** bridge **/** catalog activation **/** execution-link workflow buttons **;** **no** batch workflow **.** **Possible next:** manual verification on a new real offer **;** legacy moderation consolidation **;** **`create_tour_bridge`** later in a conversion-action slice **;** conversion/public actions remain postponed **.**

- **`GET …/review-package`** exposes read-only **`operator_workflow`**: **`state`**, **`primary_next_action`**, **`actions[]`** (**`code`**, **`label`**, **`enabled`**, **`danger_level`**, **`requires_confirmation`**, **`method`**, **`endpoint`**, **`disabled_reason`**), **`blocking_reasons`**, **`warnings`**. **`danger_level`** values: **`safe_read`**, **`safe_mutation`**, **`conversion_enabling`**, **`public_dangerous`** (showcase **`POST …/publish`** is always **`public_dangerous`** **`;`** **`publish_showcase_channel`** **`enabled`** requires **`showcase_preview.can_publish_now`**, **`approved_for_publish`** packaging**,** **`approved`** lifecycle**,** **and** **no** C2B8A hard **`cover_media_quality_review`** gate **`—`** **`POST …/publish`** handler unchanged **).** **No** action execution from **`GET`**; **no** Telegram buttons **/** web UI **/** batch endpoints.
- **Slice C1 / C1.1 / C2A / C2B1 / C2B2:** Telegram **private admin offer card** shows **`operator_workflow`** (**C1.1** compact copy **;** **C2A** read-only callbacks **;** **C2B1** packaging approve **;** **C2B2** packaging draft generate per paragraph above **;** **bridge / catalog / execution link** — **C2B10T-A** / **C2B10T-B** / **C2B10T-C** when **`enabled`** **;** published-offer link wizard **/** **HTTP** still for manual tour input **).**
...
## Checkpoint Sync — Telegram admin button UX policy (2026)

**Docs-only.** **Accepted:** Telegram admin offer card is **not** a developer console **;** button labels **1–2 words**, human-readable, sequential, safe for mixed experience **;** **no** snake_case action codes **/** endpoints **/** enums **/** HTTP methods in labels **;** disabled **`operator_workflow`** actions **hidden** (not shown as disabled) **;** safe reads — **no** confirmation **;** safe mutations — **two-step confirmation + re-read `GET …/review-package`** before execute **.** **Preferred order (B15C chain):** refresh → preview → generate packaging → approve packaging → **Link tour** (**C2B10T-A**) → **List for sale** (**C2B10T-B**) → **Booking link** (**C2B10T-C**) → **Publish** (**C2B8B**) (**`can_publish_now`** requires an active execution link before channel publish succeeds) → legacy moderation where applicable → Orders **/** Requests → navigation **.** **Telegram strip:** **`_detail_keyboard`** may still render **Publish** left of **Booking link**; follow **`review-package`** / disabled reasons for the real sequence **`;`** **not** strict left-to-right. **`create_execution_link`** on the workflow card — **C2B10T-C** (**Booking link** **/** **Link rezervări**)**;** **HTTP** **`POST …/execution-link`** and published-offer link wizard remain for other flows **.** **Label direction (RO/EN):** **`review_package_refresh`** → **Actualizează / Refresh** **;** **`get_showcase_preview`** → **Preview** **;** **`generate_packaging_draft`** → **Generează text** or **Pregătește** **;** **`approve_packaging_for_publish`** → **Aprobă text / Aprobă pachet** **;** **`create_tour_bridge`** → **Link tour / Leagă tur** **;** **`activate_tour_for_catalog`** → **List for sale / În catalog** **;** **`create_execution_link`** → **Booking link / Link rezervări** **;** legacy moderation → **Aprobă oferta**, **Respinge oferta** (disambiguate from packaging approval)**.**

**Slice C2B3 implemented:** Telegram offer-detail keyboard **`_detail_keyboard`** **`;`** labels and row order aligned with policy **`;`** workflow mutation callbacks include **C2B1–C2B8B**, **C2B10T-A**, **C2B10T-B**, **C2B10T-C**, **C2B6** as shipped **`;`** **`create_execution_link`** workflow button on card — **C2B10T-C** **;** wizard **/** **HTTP** still for manual tour selection **.**
...
### Postponed (tech debt)
- **If** **`ADMIN_API_TOKEN`** **or** **other** **secrets** **were** **exposed** **in** **logs** **/** **chat** **during** **smoke,** **rotate** **after** **smoke.** **B13G production smoke (2026-05-10):** token may have appeared in **screenshots** **/** **chat** — **rotate** **Railway** **`ADMIN_API_TOKEN`** **and** **re-issue** **to** **operators** **as** **needed** **(see** **[`docs/B13G_PRODUCTION_PUBLISH_AUDIT_SMOKE_RUNBOOK.md`](B13G_PRODUCTION_PUBLISH_AUDIT_SMOKE_RUNBOOK.md)** **Run** **log** **—** **2026-05-10** **).** **B13G full conversion smoke (same window, Offer #12 → Tour #6):** same **token-rotation** discipline applies after Mini App / Admin API manual checks.
- **Layer A / Mini App — reservation preparation vs catalog open_for_sale (B13G / B14A / B14C / B14D):** **Production Tour #6** (**`B10-SO12-04fb1f`**) showed **prep 404** when **`boarding_points`** were empty on a **per_seat** tour — **[`docs/B14A_CHANNEL_LINK_AND_RESERVATION_READINESS_DIAGNOSTIC.md`](B14A_CHANNEL_LINK_AND_RESERVATION_READINESS_DIAGNOSTIC.md)**. **B14C (code):** **new** bridge-created tours materialize **`boarding_points`** from **`boarding_places_text`** — **does not** backfill **Tour #6**. **B14D (docs):** operator runbook for **read-only verify** + **`POST /admin/tours/6/boarding-points`** + **`GET /mini-app/tours/B10-SO12-04fb1f/preparation`** smoke — **[`docs/B14D_TOUR6_BOARDING_REMEDIATION_AND_PREP_SMOKE_RUNBOOK.md`](B14D_TOUR6_BOARDING_REMEDIATION_AND_PREP_SMOKE_RUNBOOK.md)**; **Tour #6** remains **pending operator execution** until remediation is run and recorded.
- **Temporary hold expiry persistence (B14E / B14F / B14G):** **B14F** shipped **`lazy_expire_due_reservations_commit_if_any`** (commit **`58968a5`**). **B14G production verification — PASS:** **`GET /mini-app/tours/B10-SO12-04fb1f/preparation`** after deploy — **#52/#53** **`expired_unpaid_hold`**, **`cancelled_no_payment`**, **`unpaid`**, **`reservation_expires_at`** cleared; Tour **#6** **`seats_available`** **7 → 10**. Record: **[`docs/B14G_RECORD_TEMP_HOLD_EXPIRY_FIX_PRODUCTION_RESULT.md`](B14G_RECORD_TEMP_HOLD_EXPIRY_FIX_PRODUCTION_RESULT.md)**; handoff **[`docs/HANDOFF_B14G_TEMP_HOLD_EXPIRY_FIX_PRODUCTION_VERIFICATION_TO_NEXT_STEP.md`](HANDOFF_B14G_TEMP_HOLD_EXPIRY_FIX_PRODUCTION_VERIFICATION_TO_NEXT_STEP.md)**. **Handoff #86** on **#53** still **open** (support). **Background worker** still recommended without Mini App traffic.
- **B15 — Admin Publishing Console:** **Foundation (B15B–B15F) closed — docs:** **[`docs/B15_PUBLISHING_CONSOLE_FOUNDATION_CLOSURE_CHECKPOINT.md`](B15_PUBLISHING_CONSOLE_FOUNDATION_CLOSURE_CHECKPOINT.md)**; handoff **[`docs/HANDOFF_B15_CLOSE_PUBLISHING_CONSOLE_FOUNDATION_CHECKPOINT_TO_NEXT_STEP.md`](HANDOFF_B15_CLOSE_PUBLISHING_CONSOLE_FOUNDATION_CHECKPOINT_TO_NEXT_STEP.md)**. **B15A** — **[`docs/B15_ADMIN_PUBLISHING_CONSOLE_DESIGN.md`](B15_ADMIN_PUBLISHING_CONSOLE_DESIGN.md)**. **B15B** — **[`docs/B15B_READ_ONLY_ADMIN_PUBLISHING_CONSOLE.md`](B15B_READ_ONLY_ADMIN_PUBLISHING_CONSOLE.md)** **`GET /admin/publishing-console`**. **B15C (implemented)** — **[`docs/B15C_SUPPLIER_OFFER_EXACT_TOUR_CTA_GATE.md`](B15C_SUPPLIER_OFFER_EXACT_TOUR_CTA_GATE.md)** — exact-tour **Rezervă** for supplier-offer channel publish; execution link before publish when gates pass. **B15C6 (checkpoint — docs):** **[`docs/B15C_EXACT_CTA_CHAIN_CLOSURE_CHECKPOINT.md`](B15C_EXACT_CTA_CHAIN_CLOSURE_CHECKPOINT.md)** — closure baseline before **B15D**; **[`docs/HANDOFF_B15C6_CLOSE_EXACT_CTA_CHAIN_CHECKPOINT_TO_NEXT_STEP.md`](HANDOFF_B15C6_CLOSE_EXACT_CTA_CHAIN_CHECKPOINT_TO_NEXT_STEP.md)**. **B15C production smoke (PASS — operator):** **[`docs/B15C_PRODUCTION_SMOKE_RESULT.md`](B15C_PRODUCTION_SMOKE_RESULT.md)** — Offer **#13**, Tour **#7** **`B10-SO13-e9f389`**, link **#6**, message **#26**; exact CTA confirmed. **B15C5 production smoke (PASS — operator, direct short-name `t.me/{bot}/{short}?startapp=` CTA):** **[`docs/B15C5_DIRECT_MINI_APP_LINK_PRODUCTION_SMOKE_RESULT.md`](B15C5_DIRECT_MINI_APP_LINK_PRODUCTION_SMOKE_RESULT.md)** — Offer **#15**, tour code **`B10-SO15-460344`**, link **#8**, message **#28**; **Rezervă** → Mini App directly; identity OK; temp hold **#55** (test — allow natural expiry). **B15C2 (implemented — Telegram admin nav):** after **Leagă tur** / **În catalog** / workflow actions, queue FSM pins to the same offer — **[`docs/HANDOFF_B15C2_ADMIN_NAV_STAY_ON_OFFER_AFTER_BRIDGE_TO_NEXT_STEP.md`](docs/HANDOFF_B15C2_ADMIN_NAV_STAY_ON_OFFER_AFTER_BRIDGE_TO_NEXT_STEP.md)**. **Follow-ups (not B15C blockers):** **(1)** Mini App **Telegram identity/session** for **plain HTTPS** channel links was the historical gap (**B15C1**); **B15C5** operator smoke confirms **WebApp** **/** **`startapp`** **/** short-name path; track **`CURSOR_PROMPT_B15C1_MINI_APP_IDENTITY_FROM_CHANNEL_EXACT_TOUR_ENTRY.md`** if new edge cases appear. **(2)** ~~Telegram admin navigation after **Leagă tur**~~ **done (B15C2)**. **(3)** ~~Replace stale copy **“execution links after showcase publish”** (bilingual / RO)~~ **done (B15C3)** — **`CURSOR_PROMPT_B15C3_POST_B15C_COPY_POLISH.md`**. **B15D (implemented):** **[`docs/B15D_ADMIN_PUBLISHING_CONSOLE_RICH_READ_VIEW.md`](B15D_ADMIN_PUBLISHING_CONSOLE_RICH_READ_VIEW.md)** — richer **read-only** console read model (readiness, blockers, exact-tour CTA safety, conversion target, admin paths); **tests** **`tests/unit/test_admin_publishing_console.py`** **—** **8** **passed.** **Handoff:** **[`docs/HANDOFF_B15D_ADMIN_PUBLISHING_CONSOLE_RICH_READ_VIEW_TO_NEXT_STEP.md`](HANDOFF_B15D_ADMIN_PUBLISHING_CONSOLE_RICH_READ_VIEW_TO_NEXT_STEP.md)**. **B15E (implemented):** **[`docs/B15E_ADMIN_PUBLISHING_CONSOLE_ACTION_AFFORDANCES_DESIGN.md`](B15E_ADMIN_PUBLISHING_CONSOLE_ACTION_AFFORDANCES_DESIGN.md)** — additive **`actions[]`** affordances (metadata only); **tests** **8** **passed**; **handoff** **[`docs/HANDOFF_B15E_ADMIN_PUBLISHING_CONSOLE_ACTION_AFFORDANCES_DESIGN_TO_NEXT_STEP.md`](HANDOFF_B15E_ADMIN_PUBLISHING_CONSOLE_ACTION_AFFORDANCES_DESIGN_TO_NEXT_STEP.md)**. **B15F (implemented):** **[`docs/B15F_PUBLISHING_CONSOLE_TEMPLATE_SOURCE_CHANNEL_READ_MODEL.md`](B15F_PUBLISHING_CONSOLE_TEMPLATE_SOURCE_CHANNEL_READ_MODEL.md)** — templates/source/channel/media read metadata plus future **`template_actions`** / **`channel_actions`** hints; **tests** **8** **passed**; **handoff** **[`docs/HANDOFF_B15F_PUBLISHING_CONSOLE_TEMPLATE_SOURCE_CHANNEL_READ_MODEL_TO_NEXT_STEP.md`](HANDOFF_B15F_PUBLISHING_CONSOLE_TEMPLATE_SOURCE_CHANNEL_READ_MODEL_TO_NEXT_STEP.md)**. **B16 Step 1** **+** **B16B** **+** **B16C** **+** **B16D1** **+** **B16D1.1** **+** **B16D1.2** **+** **B16E** **(** **implemented** **):** **[`docs/B16_ADMIN_OPS_VISIBILITY_READ_DASHBOARD.md`](B16_ADMIN_OPS_VISIBILITY_READ_DASHBOARD.md)** — **`GET /admin/ops-dashboard`** **(** **B16B** **filters** **;** **B16C** **`admin_path`** **)** **;** **`GET /admin/supplier-offers/{offer_id}/prepare-conversion-chain/plan`** **(** **B16D1** **read-only** **plan** **preview** **)** **;** **B16D1.1** **additive** **`prepare_conversion_chain_plan_path`** **(** **metadata** **only** **)** **on** **review-package** **,** **plan** **response** **,** **publishing-console** **supplier-offer** **rows** **,** **ops-dashboard** **supplier-offer-related** **publication** **/** **conversion** **/** **attention** **rows** **(** **none** **on** **pure** **tour-promotion** **rows** **)** **;** **B16D1.2** **additive** **`prepare_conversion_chain_plan_status`** **,** **`prepare_conversion_chain_recommended_action`** **,** **`prepare_conversion_chain_blockers_count`** **(** **ineligible** **/** **blocked** **/** **partial** **/** **already_prepared** **)** **same** **supplier-offer** **surfaces** **+** **plan** **endpoint** **(** **no** **misleading** **tour-promotion** **rows** **)** **;** **B16E** **additive** **`audit_events`** **(** **`audit_events_limit`** **,** **`include_sections`** **)** **—** **read-only** **merge** **of** **showcase** **publish** **attempts** **,** **failed** **notification** **outbox** **,** **supplier** **execution** **failed** **/** **blocked** **requests** **+** **failed** **attempts** **;** **B16D2** **(** **design** **gate** **)** **[`docs/B16D2_PREPARE_CONVERSION_CHAIN_EXECUTION_DESIGN_GATE.md`](B16D2_PREPARE_CONVERSION_CHAIN_EXECUTION_DESIGN_GATE.md)** **/** **[`docs/HANDOFF_B16D2_PREPARE_CONVERSION_CHAIN_EXECUTION_DESIGN_GATE_TO_NEXT_STEP.md`](HANDOFF_B16D2_PREPARE_CONVERSION_CHAIN_EXECUTION_DESIGN_GATE_TO_NEXT_STEP.md)** **;** **B16D2A** **(** **implemented** **)** **`admin_guarded_action_attempts`** **/** **`admin_guarded_action_steps`** **,** **`20260609_30_admin_guarded_action_audit.py`** **,** **ORM** **+** **repository** **+** **`AdminGuardedActionAuditService`** **(** **no** **`POST`** **`prepare_conversion_chain`** **;** **audit/idempotency** **only** **)** **;** **tests** **focused** **suite** **32** **passed** **(** **`test_admin_ops_dashboard`** **12** **;** **`test_admin_guarded_action_audit`** **5** **;** **review-package** **+** **publishing** **console** **+** **ops** **dashboard** **+** **plan** **)** **;** **Railway** **/** **prod** **after** **deploy:** **`python -m alembic upgrade head`** **;** **handoff** **[`docs/HANDOFF_B16_ADMIN_OPS_VISIBILITY_READ_DASHBOARD_TO_NEXT_STEP.md`](HANDOFF_B16_ADMIN_OPS_VISIBILITY_READ_DASHBOARD_TO_NEXT_STEP.md)**.  **B16D2B** **(** **implemented** **)** **:** **service-level** **`prepare_conversion_chain`** **execution** **(** **`PrepareConversionChainExecutionService`** **in** **`app/services/prepare_conversion_chain_execution_service.py`** **;** **guarded** **audit** **/** **idempotency** **;** **bridge** **→** **catalog** **→** **execution** **link** **via** **existing** **admin** **services** **only** **)** **;** **no** **`POST`** **endpoint** **yet** **(** **prompt** **:** **[`docs/CURSOR_PROMPT_B16D2B_PREPARE_CONVERSION_CHAIN_SERVICE_EXECUTION.md`](CURSOR_PROMPT_B16D2B_PREPARE_CONVERSION_CHAIN_SERVICE_EXECUTION.md)** **)** **.** **B15O (docs closure — pending commit):** [`docs/B15O_PUBLISHING_CONSOLE_FOUNDATION_CLOSURE.md`](docs/B15O_PUBLISHING_CONSOLE_FOUNDATION_CLOSURE.md) — foundation closed as read-model + guarded internal prepare-chain only. **Implemented:** B16D2C, B16D2D, B15E2. **Remaining product gates:** channel/template editor, scheduled publish, batch approval, Telegram publish automation, auto-publish (B15G design only today), real tour-promotion post generator, durable media storage/rendering, public post edit/delete/unpublish workflow. **Future:** `audit_events` admin UI refinement if needed. **Product:** console UX beyond HTTP; auto-draft vs limited auto-publish (B15F2+/B15G). **B17:** design gate **[`docs/B17_CHANNEL_TEMPLATE_EDITOR_DESIGN_GATE.md`](B17_CHANNEL_TEMPLATE_EDITOR_DESIGN_GATE.md)**. **B17A (done):** read-only **`GET /admin/publishing-console/supplier-offers/{offer_id}/editor`** — **[`docs/HANDOFF_B17A_READ_ONLY_EDITOR_DETAIL_VIEW.md`](HANDOFF_B17A_READ_ONLY_EDITOR_DETAIL_VIEW.md)** (**no** channel/template/draft persistence; **no** publish / Telegram). **B17B (done):** additive **`channel_selection`** / **`template_selection`** response metadata on the same **`GET /admin/publishing-console/supplier-offers/{offer_id}/editor`** — **[`docs/HANDOFF_B17B_CHANNEL_TEMPLATE_SELECTION_METADATA.md`](HANDOFF_B17B_CHANNEL_TEMPLATE_SELECTION_METADATA.md)** (**read-only JSON; not persisted**). **Persistent** channel/template selection, draft edits, **B17C+** publish paths, and **publish automation** remain **future-gated**. **B17Z (docs closure):** **B17** read-only editor foundation formally closed — **[`docs/B17Z_EDITOR_READ_ONLY_FOUNDATION_CLOSURE.md`](B17Z_EDITOR_READ_ONLY_FOUNDATION_CLOSURE.md)** (**no** runtime change in B17Z). **P0 (docs roadmap):** **[`docs/OPERATIONAL_AUTOMATION_ROADMAP.md`](OPERATIONAL_AUTOMATION_ROADMAP.md)** adds the operational automation roadmap; **A1 (docs gate):** **[`docs/A1_ADMIN_AUTOMATION_COCKPIT_DESIGN_GATE.md`](A1_ADMIN_AUTOMATION_COCKPIT_DESIGN_GATE.md)** defines cockpit queues, action taxonomy, next-best-action model, fact-lock, AI boundaries, supplier clarification, marketing review, catalog/conversion, departure ops, customer handoff, and RFQ handling. Future-gated decisions remain open: cockpit persistence vs read-only projection (**A1-Block 1/2:** **`GET /admin/automation-cockpit`** — publishing-console snapshot + commercial/marketing/conversion lanes + optional per-offer detail reads; see **[`docs/HANDOFF_A1_BLOCK1_COCKPIT_READ_ONLY_FOUNDATION.md`](HANDOFF_A1_BLOCK1_COCKPIT_READ_ONLY_FOUNDATION.md)** and **[`docs/HANDOFF_A1_BLOCK2_COMMERCIAL_MARKETING_CONVERSION_QUEUES.md`](HANDOFF_A1_BLOCK2_COMMERCIAL_MARKETING_CONVERSION_QUEUES.md)**), supplier clarification delivery/outbox, permissions, AI3 permissions/tool scope, supplier notification privacy, passenger manifest privacy, action audit format, priority scoring, SLA/due dates, and UI location (Telegram admin buttons vs web admin vs both). **Next product clusters:** **M1** (Marketing Identity & Deep Link Capture design gate); **O1** (Order / Ticket QR & Boarding Validation design gate) when secure operational QR is priority.
...
- **B7.4B** **(docs,** **2026):** **Ingestion** **contract** **before** **bytes** **—** **[`docs/B7_4B_MEDIA_STORAGE_INGESTION_CONTRACT.md`](B7_4B_MEDIA_STORAGE_INGESTION_CONTRACT.md)** **;** **next** **implementation** **slice** **B7.4C** **(** **config** **+** **adapter** **mock** **+** **eligibility** **+** **tests** **)** **per** **[`docs/HANDOFF_MEDIA_STORAGE_INGESTION_CONTRACT_B7_4B_TO_NEXT_STEP.md`](HANDOFF_MEDIA_STORAGE_INGESTION_CONTRACT_B7_4B_TO_NEXT_STEP.md)** **.**
- **B7.4** **open** **decisions** **(** **contract** **records** **options** **;** **stakeholder** **choice** **still** **needed** **):** **(1)** **B7.4E** **—** **whether** **showcase** **publish** **with** **photo** **must** **require** **`publish_safe`** **ready** **/** **durable** **asset** **(** **vs** **remaining** **metadata-only** **for** **some** **modes** **)** **;** **(2)** **HTTPS** **source** **ingestion** **—** **whether** **server-side** **fetch** **is** **allowed** **at** **all** **in** **MVP** **and** **under** **what** **allowlist** **/** **size** **/** **timeout** **limits** **;** **(3)** **object** **storage** **vendor** **/** **account** **(** **S3-compatible** **vs** **specific** **provider** **)** **for** **production** **—** **contract** **assumes** **adapter** **abstraction** **only.**
