...
- **B15 — Admin Publishing Console (B15A design, 2026-05-09):** **[`docs/B15_ADMIN_PUBLISHING_CONSOLE_DESIGN.md`](B15_ADMIN_PUBLISHING_CONSOLE_DESIGN.md)** — separates **Supplier Offer Admin** from **Publishing Console**; defines **Channel Publication**, **Publication Template**, **Conversion Target**, **Safety Policy**, **Draft**, **Attempt/Audit**; automation modes; scheduler/campaign examples; anti-spam; rollout **B15B–G**. **Design created** (docs-only). **Product decisions pending/accepted:** Publishing Console **admin UX** (queue-first layout, advanced/debug toggle); **exact-tour CTA** for supplier-offer posts when conversion chain is green (**B15C**); **auto-draft** vs **limited auto-publish** (approval default, caps, kill switch).
