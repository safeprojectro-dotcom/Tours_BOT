...
**Scope:** **B15B–B15F** plus **B15F2/B15F3** and **B15K** (additive) — safe, **read-only** admin publishing console **foundation** + **template/preview display** (`console_preview`) + **template library** (`template_library`) metadata on console rows; execution-heavy product (auto-publish, console-side mutations, template/channel editors) stays **approval-gated**.  
...
- **Layer A** changes, **Mini App routing** / **B11** deep-link behavior, or **migrations** for these slices (including **B15K** read-model additions).
...
- **Unit:** `tests/unit/test_admin_publishing_console.py` — **8 passed** (covers B15D / B15E / B15F / **B15F2–B15F3** `console_preview` / **B15K** `template_library` additive read model on the same endpoint).
...
2. Open a **new** gated slice (e.g. template/channel **editor** UX, or other console execution beyond existing **B15E2**) with an explicit charter — **B15F2/B15F3** and **B15K** read-model metadata are already closed; **B15E2** already covers prepare-chain execution from the publishing console.
