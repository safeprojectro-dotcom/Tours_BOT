...
**Continuity anchor (read this first):** The **live** approved checkpoint is **Phase 7 / Step 2 completed** — **internal helper/service foundation** for **group trigger evaluation** and **handoff trigger categorization** (**`app/services/group_trigger_evaluation.py`**, **`handoff_trigger_evaluation.py`**, **`assistant_trigger_evaluation.py`**; types **`app/schemas/group_assistant_triggers.py`**; tests **`tests/unit/test_group_assistant_triggers.py`**). **Rules** remain in **`docs/GROUP_ASSISTANT_RULES.md`** (Step 1). **Next planned slice:** **Phase 7 / Step 3** — **first narrow bot integration slice for group trigger evaluation** (see **Next Safe Step**); **not** full group runtime, **not** automatic handoff creation. **Phase 6** (**Steps 1–30**) and **`docs/PHASE_6_REVIEW.md`** remain the **historical closure** of the narrow admin API track. **Not** automatic **Phase 6 / Step 31**, **not** **admin payment** mutation, **not** **operator workflow engine** in one step. **Phase 5** entries under **Completed Steps** below are **historical** (Mini App MVP acceptance); **do not** resume from old **Phase 5 / Step 4** or **Phase 5 / Step 5** “next step” narratives — those are **closed** for MVP. Use **`Next Safe Step`** and this file’s **Current Status** only.

## Current Status
The **latest approved checkpoint** is **Phase 7 / Step 2 completed**. **Step 1** (**`docs/GROUP_ASSISTANT_RULES.md`**) remains the **rules** reference. **Step 2** delivers **internal helper/service foundation** only: **group trigger evaluation** (mention vs approved command vs approved trigger phrase — commands before mention so `/cmd@bot` counts as command); **handoff trigger categorization** (keyword-style, ordered priority; **`low_confidence_signal`** only when passed in); **`evaluate_assistant_trigger_snapshot`** composes both. **No** Telegram **group** handler wiring yet, **no** new HTTP routes, **no** repository changes — **public** booking / payment / waitlist / Mini App **behavior unchanged** (no customer-flow retarget).

**Still not implemented (explicit):** **full** Telegram **group** runtime behavior; **automatic handoff creation / persistence** from group messages; **operator workflow engine**; **public** API or product-flow changes — **out of scope** for Steps 1–2.
...
**Next work:** **Phase 7 / Step 3** — **first narrow bot integration slice for group trigger evaluation** (see **Next Safe Step**). Rules: **`docs/GROUP_ASSISTANT_RULES.md`**; helpers: Step **2** modules. **Admin payment mutations** are **not** the default. Optional future **narrow** admin slices (e.g. persisted move audit) only with **explicit** product approval — **not** assumed.
...
- **Phase 7 / Step 2 (helpers)** — **internal** foundation: **`app/schemas/group_assistant_triggers.py`**, **`app/services/group_trigger_evaluation.py`**, **`handoff_trigger_evaluation.py`**, **`assistant_trigger_evaluation.py`**; **unit tests** `tests/unit/test_group_assistant_triggers.py`; **real** Telegram **group** bot integration **postponed** (Step **3**)
...
### Not Implemented Yet
- **Next (Phase 7):** **Phase 7 / Step 3** — **first narrow bot integration slice for group trigger evaluation** (invoke Step **2** helpers from a **group** message/update path for gating; **or** product-scoped extension of trigger phrase/keyword lists + tests **only**) — see **Next Safe Step**; **not** full group responder runtime, **not** operator engine, **not** automatic handoff **creation** from group unless explicitly scoped.
- **Group / handoff runtime gap:** helper-layer logic exists; **live** **Telegram** hookup and **automatic handoff persistence** remain **future** — **`docs/OPEN_QUESTIONS_AND_TECH_DEBT.md`** §**19**.
...
- **Phase 7–9 (beyond Step 3):** group runtime at scale, content assistant, analytics — per `docs/IMPLEMENTATION_PLAN.md`

## Next Safe Step

**Primary (single outcome):** **Phase 7 / Step 3** — **first narrow bot integration slice for group trigger evaluation** — add a **minimal** path in the **Telegram bot** layer that receives **group** text updates and calls **`evaluate_group_trigger`** (and optionally **`evaluate_assistant_trigger_snapshot`**) for **reply gating** / early classification — **without** building a **full** group conversational runtime, **without** automatic **handoff** row creation, **without** **operator workflow engine**, **without** changing **public** booking/payment/Mini App APIs or customer flows.

- **Preferred:** bot handler + **`evaluate_group_trigger`** (pass **`bot_username`** from config); keep replies **stub/minimal** or no-op beyond gating if product wants smallest diff.
- **Alternative narrow Step 3:** extend **trigger phrase / keyword** lists + unit tests only (still **no** live group integration).
- **Rules + helpers:** **`docs/GROUP_ASSISTANT_RULES.md`**, **`app/services/group_trigger_evaluation.py`**, **`app/services/assistant_trigger_evaluation.py`**.
...
**Staging (optional):** smoke in a **test** Telegram **group** after Step 3 integration lands.

### Goal
Connect **Step 2** helpers to **real** group message flow **incrementally** — **bot integration first**, full product behavior later.
...
**Completed step references:** `docs/CURSOR_PROMPT_PHASE_6_STEP_1.md` … `docs/CURSOR_PROMPT_PHASE_6_STEP_30.md` (historical); **`docs/CURSOR_PROMPT_PHASE_7_STEP_1.md`**, **`docs/CURSOR_PROMPT_PHASE_7_STEP_2.md`** (historical).  
**Plan:** `docs/IMPLEMENTATION_PLAN.md` — Phase 7.

## Recommended Next Prompt
Implement **Phase 7 / Step 3**: **first narrow bot integration slice for group trigger evaluation** — wire **`evaluate_group_trigger`** (and optionally **`evaluate_assistant_trigger_snapshot`**) from a **group** chat handler path; **or** extend phrase/keyword lists + tests only. **Do not** add automatic handoff persistence or full operator engine unless explicitly scoped. Sources: **`docs/GROUP_ASSISTANT_RULES.md`**, **`docs/CHAT_HANDOFF.md`**, `docs/IMPLEMENTATION_PLAN.md`, `docs/OPEN_QUESTIONS_AND_TECH_DEBT.md`, `docs/TESTING_STRATEGY.md`, `docs/TELEGRAM_SETUP.md`, `docs/AI_ASSISTANT_SPEC.md`, `docs/AI_DIALOG_FLOWS.md`.
...
- **Approved checkpoint:** **Phase 7 / Step 2** complete — **internal** helper foundation (**group trigger evaluation** + **handoff trigger categorization** under **`app/services/`**, **`tests/unit/test_group_assistant_triggers.py`**); Step **1** rules: **`docs/GROUP_ASSISTANT_RULES.md`**. **Forward:** **Phase 7 / Step 3** — **first narrow bot integration slice for group trigger evaluation** (see **Next Safe Step**); alternative: phrase-list + tests only. Phase 6 closure: **`docs/PHASE_6_REVIEW.md`**. **Not** old **Phase 5 / Step 4–5** checkpoints; **not** admin payment mutation or **Phase 6 / Step 31** unless explicitly scoped; **not** full group runtime or automatic handoff creation unless scoped.
...
Default forward path is **Phase 7 / Step 3** (narrow **bot** integration for **group** trigger evaluation) — do not use legacy Phase 5 “next step” prompts for new implementation slices.
