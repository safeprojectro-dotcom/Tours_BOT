...
- **Next safe order:**
  1. **Y38–Y42 (docs) + Y43 (persistence) + Y45 (trigger design) —** Supplier gates: **`docs/SUPPLIER_INTERACTION_GATE.md`** through **`docs/SUPPLIER_EXECUTION_PERMISSION_AUDIT_GATE.md`**; **Y45** = **`docs/SUPPLIER_EXECUTION_TRIGGER_DESIGN.md`** (accepted **design**: **first** **trigger** = **admin** **explicit** **only**; **DB** **`supplier_execution_request`** only; **no** supplier **contact**, **no** **attempt** rows in that spec). **Y43** tables exist; read/admin list-detail may ship under separate tickets. **Next** **safe** **in-app** work aligned with Y45: **implement** the **safe** **admin** **trigger** **endpoint** (AUTH + **validation** + **idempotency** + **audit**) **with** **no** **supplier** **messaging** and **no** **execution** **runtime** beyond the **request** row. Still **forbidden** unless a slice **explicitly** allows: RFQ **automation**, booking/order/payment, Mini App, execution **links**, identity **bridge**, customer/supplier **notifications**.
