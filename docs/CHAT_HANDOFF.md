...
**Next work:** **Phase 7 / Step 17** — **narrow private followup history / readiness signal for repeat `grp_followup` entries** (see **Next Safe Step**): **one** small **private read-side** refinement (e.g. clearer copy or signal when the user has **prior** closed **`group_followup_start`** rows and a **new** open row after **`/start grp_followup`**) — **not** operator chat, **not** notification subsystem — **not** admin payment work. **i18n** / **rate limits** remain **separate** optional slices unless bundled by product. **Admin payment mutations** are **not** the default.
...
- **Phase 7 / Step 6 (private `/start` for `grp_*`)** — **`app/bot/handlers/private_entry.py`** + **`match_group_cta_start_payload`**; distinct intros **`start_grp_private_intro`** vs **`start_grp_followup_intro`** (+ Phase **7** / Step **16** **`start_grp_followup_resolved_intro`** when applicable); **tests** `test_group_private_cta`, `test_private_start_grp_messages`
...
- **Phase 7 / Step 16 (private resolved-followup confirmation)** — **`/start grp_followup`** may show **`start_grp_followup_resolved_intro`** when latest **`group_followup_start`** is **`closed`** and **no** **open** row; **`HandoffEntryService`** + **`HandoffRepository`** read helpers only for the predicate
...
### Not Implemented Yet
- **Next (Phase 7):** **Phase 7 / Step 17** — **narrow private followup history / readiness signal for repeat `grp_followup` entries** — see **Next Safe Step** (slightly richer **private** read-side signal for **repeat** visits — e.g. prior closed vs **new** open row — **still** **no** operator chat, **no** notification subsystem). Optional **later** slices: i18n polish, group **rate limits**.
- **Group / handoff:** Steps **1–16** = **rules + helpers + group gating + escalation wording + deep links + private `/start`** + **narrow** **`handoffs`** write on **`/start grp_followup`** (**Step 7**) + **focused tests** (**Step 8**) + **admin read visibility** (**Step 9**) + **narrow `assign-operator`** (**Step 10**) + **read-side work-state labels** (**Step 11**) + **narrow `mark-in-work`** (**Step 12**) + **narrow `resolve-group-followup`** + **`group_followup_resolution_label`** (**Steps 13–14**) + **`group_followup_queue_state`** + **`group_followup_queue`** list filter (**Step 15**) + **private resolved confirmation** on **`grp_followup`** (**Step 16**); **group** chat path (Steps **3–4**) still **no** DB handoff rows — **`docs/OPEN_QUESTIONS_AND_TECH_DEBT.md`** §**19**.
...
- **Longer-term (beyond Phase 7 / Step 17 checkpoint):** optional group assistant at scale, content assistant, analytics — per `docs/IMPLEMENTATION_PLAN.md`

## Next Safe Step

**Primary (single outcome):** **Phase 7 / Step 17** — **narrow private followup history / readiness signal for repeat `grp_followup` entries** (*one* small **private** read-side refinement so repeat **`/start grp_followup`** visits can surface a **clearer “history / readiness” hint** — e.g. when a **new** **open** **`group_followup_start`** exists alongside **prior** **closed** rows — **without** operator↔customer chat, **without** handoff push notifications, **without** new admin API surface unless explicitly scoped).

- **Why:** Steps **7–16** — operator path through **resolve**, admin **queue-state**, and **private** **resolved** acknowledgment exist; the **next smallest increment** is **user-readable context** for **repeat** follow-up entries — still **no** notification subsystem, **no** two-way operator messaging.
- **In scope (typical):** **private** bot copy and/or **narrow** derived read from existing **`handoffs`** rows (**`group_followup_start`** only); exact UX is a **Step 17** design choice; **prefer** **no** schema migration.
- **Out of scope for Step 17 unless rescoped:** **`per_seat` / `full_bus`**, **broad** takeover/reassign, **operator chat**, **customer** notification pipeline, **admin payment** mutations, **Mini App** / **public** booking flow changes.
- **Baseline (Steps 1–16):** **`grp_*`** private **`/start`**, **`group_followup_start`** persistence + dedupe, **`start_grp_followup_resolved_intro`**, admin **assign** / **in-work** / **resolve**, **`group_followup_resolution_label`**, **`group_followup_queue_state`**, **`group_followup_queue`** filter — see **`docs/CHAT_HANDOFF.md`** **Completed Steps**.
...
**Staging (recommended):** smoke **`/start grp_followup`** after **resolve** then again after **new** open row from second **`grp_followup`** — **Railway** / deploy health as usual.

### Safe scope boundaries
- **Preserve** public customer flows and existing APIs unless a future step scopes them.
- **Phase 6 review** (`docs/PHASE_6_REVIEW.md`) remains the admin-track closure record.

**Completed step references:** `docs/CURSOR_PROMPT_PHASE_6_STEP_1.md` … `docs/CURSOR_PROMPT_PHASE_6_STEP_30.md` (historical); **`docs/CURSOR_PROMPT_PHASE_7_STEP_1.md`** … **`docs/CURSOR_PROMPT_PHASE_7_STEP_12.md`** + **`docs/CURSOR_PROMPT_PHASE_7_STEP_13_14_COMBINED.md`** + **`docs/CURSOR_PROMPT_PHASE_7_STEP_15.md`** + **`docs/CURSOR_PROMPT_PHASE_7_STEP_16.md`** (historical).  
**Plan:** `docs/IMPLEMENTATION_PLAN.md` — Phase 7.

## Recommended Next Prompt
Implement **Phase 7 / Step 17** — **narrow private followup history / readiness signal for repeat `grp_followup` entries** (see **Next Safe Step**): **one** small **private** read-side refinement for **repeat** **`grp_followup`** visits — **not** operator chat, **not** handoff notification subsystem, **not** public/Mini App churn unless scoped, **not** **`per_seat` / `full_bus`**. Sources: **`docs/CHAT_HANDOFF.md`**, **`docs/GROUP_ASSISTANT_RULES.md`**, **`docs/TELEGRAM_SETUP.md`**, **`docs/TECH_SPEC_TOURS_BOT.md`**, `docs/IMPLEMENTATION_PLAN.md`, `docs/OPEN_QUESTIONS_AND_TECH_DEBT.md`, `docs/TESTING_STRATEGY.md`.
...
- **Approved checkpoint:** **Phase 7 / Step 16** complete — **Phase 7** surface through **Step 16** (Steps **1–15** as before, plus **private** **`start_grp_followup_resolved_intro`** when latest **`group_followup_start`** is **`closed`** with **no** **open** row — **read** helpers + message copy only; **no** operator chat, **no** migration, **no** handoff push notifications); **`grp_private`** unchanged; **`tour_*`** / legacy **`/start`** unchanged; **no** automatic assignment from **Telegram `grp_*`** entry; **`TELEGRAM_BOT_USERNAME`** required for full group gating. **Forward:** **Phase 7 / Step 17** — **narrow private followup history / readiness signal for repeat `grp_followup` entries** (see **Next Safe Step**). Phase 6 closure: **`docs/PHASE_6_REVIEW.md`**. **Not** old **Phase 5 / Step 4–5** checkpoints; **not** admin payment mutation or **Phase 6 / Step 31** unless scoped.
...
Default forward path is **Phase 7 / Step 17** — **narrow private followup history / readiness signal for repeat `grp_followup` entries** — do not use legacy Phase 5 “next step” prompts for new implementation slices.
