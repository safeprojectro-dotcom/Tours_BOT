...
**Next work:** **Phase 7 / Step 13** — **narrow operator close/resolve** mutation for **`in_review`** **`group_followup_start`** handoffs (see **Next Safe Step**): **one** narrow resolution slice — **not** a broad workflow engine — **not** admin payment work. **i18n** / **rate limits** remain **separate** optional slices unless bundled by product. **Admin payment mutations** are **not** the default.
...
**Current phase (forward work):** **Phase 7 — Group Assistant And Operator Handoff**. **Phase 7 / Steps 1–12** **completed**. **Next:** **Phase 7 / Step 13** (see **Next Safe Step**). Phase 6 narrow admin API track **closed** through **Step 30** — see **`docs/PHASE_6_REVIEW.md`**.

**Latest approved checkpoint:** **Phase 7 / Step 12** — **Phase 7** through **Step 12** (Steps **1–11** as before, plus Step **12** **`POST /admin/handoffs/{id}/mark-in-work`** for **assigned** **`group_followup_start`** only — **`open` → `in_review`**, **`in_review`** idempotent, **`closed`** rejected; **no** migration, **no** notifications); **`grp_private`** unchanged; **`tour_*`** / legacy **`/start`** unchanged; **no** booking/payment handling **in group**; **no** automatic assignment from **Telegram `grp_*`** entry; **public** flows **unchanged** by Step **12**. **Step 1** docs: **`docs/GROUP_ASSISTANT_RULES.md`**. **Prior code baseline** unchanged: **Phase 6 / Step 30** **`move_placement_snapshot`**, **Step 29** move **POST**, admin handoffs/orders as implemented. **Admin payment mutations** remain **intentionally postponed** (see **`docs/OPEN_QUESTIONS_AND_TECH_DEBT.md`** **§1f**). **Still not:** **full** group assistant; **broad** **claim/takeover** engine; **broad** **operator workflow engine**; **booking/payment resolution** **via** the Phase **7** **`grp_*`** chain beyond existing private/Mini App flows; **persisted** move audit/timeline rows; **unassign**; broader **reassignment** redesign; **notifications** from admin handoff actions; **refund / capture / cancel-payment**; **public** customer/Mini App flow redesign; **`per_seat` / `full_bus`** track (separate — **not** in these slices). Internal **ops** JSON stays **separate** from **`/admin/*`**.

**Earlier checkpoints:** Phase 7 / Step 12 (**`mark-in-work`** for **assigned** **`group_followup_start`**); Phase 7 / Step 11 (**read-side** **`is_assigned_group_followup`**, **`group_followup_work_label`**); Phase 7 / Step 10 (**`assign-operator`** for **`group_followup_start`** only); Phase 7 / Step 9 (admin **`group_followup_start`** visibility — **`is_group_followup`**, **`source_label`**); Phase 7 / Step 8 (focused **`grp_followup`** chain tests); Phase 7 / Step 7 (narrow **`grp_followup`** handoff persistence); Phase 7 / Step 6 (narrow **`grp_*`** private `/start`); Phase 7 / Step 5 (private CTA deep-link foundation on group replies); Phase 7 / Step 4 (handoff-aware short replies in narrow group path); Phase 7 / Step 3 (narrow group runtime gating + default ack); Phase 7 / Step 2 (trigger helper services + unit tests); Phase 7 / Step 1 ( **`docs/GROUP_ASSISTANT_RULES.md`** — rules only); Phase 6 / Steps 1–7 (foundation through core tour patch); Phase 6 / Steps 8–10 (boarding create / patch / delete); Phase 6 / Steps 11–12 (tour translation upsert/delete); Phase 6 / Steps 13–14 (boarding point translation upsert/delete); Phase 6 / Step 15 (tour archive/unarchive); Phase 6 / Step 16 (order detail payment correction visibility); Phase 6 / Step 17 (order detail action preview); Phase 6 / Step 18 (admin handoff queue read API); Phase 6 / Step 19 (admin handoff mark-in-review); Phase 6 / Step 20 (admin handoff close-only); Phase 6 / Step 21 (admin handoff assign); Phase 6 / Step 22 (admin handoff reopen); Phase 6 / Step 23 (admin mark-cancelled-by-operator); Phase 6 / Step 24 (admin mark-duplicate); Phase 6 / Step 25 (admin mark-no-show); Phase 6 / Step 26 (admin mark-ready-for-departure); Phase 6 / Step 27 (admin lifecycle **`ready_for_departure_paid`** read refinement); Phase 6 / Step 28 (admin order move-readiness read-only fields); Phase 6 / Step 29 (admin order narrow **move** mutation); Phase 6 / Step 30 (admin **`move_placement_snapshot`** read-only); Phase 5 accepted + Phase 5 Step 20 docs (`docs/PHASE_5_ACCEPTANCE_SUMMARY.md`).
...
### Not Implemented Yet
- **Next (Phase 7):** **Phase 7 / Step 13** — **narrow operator close/resolve** mutation for **`in_review`** **`group_followup_start`** handoffs — see **Next Safe Step**. Optional **later** slices: i18n polish, group **rate limits**.
- **Group / handoff:** Steps **1–12** = **rules + helpers + group gating + escalation wording + deep links + private `/start`** + **narrow** **`handoffs`** write on **`/start grp_followup`** (**Step 7**) + **focused tests** (**Step 8**) + **admin read visibility** (**Step 9**) + **narrow `assign-operator`** (**Step 10**) + **read-side work-state labels** (**Step 11**) + **narrow `mark-in-work`** (**Step 12**); **group** chat path (Steps **3–4**) still **no** DB handoff rows — **`docs/OPEN_QUESTIONS_AND_TECH_DEBT.md`** §**19**.
...
- **Longer-term (beyond Phase 7 / Step 13 checkpoint):** optional group assistant at scale, content assistant, analytics — per `docs/IMPLEMENTATION_PLAN.md`

## Next Safe Step

**Primary (single outcome):** **Phase 7 / Step 13** — **narrow operator close/resolve** mutation for **`in_review`** **`group_followup_start`** handoffs (*one* minimal **POST** — e.g. dedicated **`resolve`** / **`close-followup`** route or tightly scoped service — so an operator can **finish** the narrow group-follow-up slice **without** replacing Step **20** **`close`** for all handoffs; exact URL + error codes are a **Step 13** design choice — likely **`in_review` → `closed`** with **reason** + **assignment** gates only; **no** booking/payment side effects).

- **Why:** Step **7–11** — chain + visibility + assign + read labels; Step **12** — **take-in-work** (**`open` → `in_review`**). The **next smallest increment** is **one** **resolution** action from **`in_review`** for this **reason** only — still **no** broad claim engine, **no** notifications.
- **In scope (typical):** mutation **only** for **`reason=group_followup_start`**, **`in_review`**, **`assigned_operator_id`** set (gates TBD in Step **13** prompt); response **`AdminHandoffRead`**; **no** change to general **`close`** semantics unless explicitly unified by product.
- **Out of scope for Step 13 unless rescoped:** **`per_seat` / `full_bus`**, **broad** takeover, **customer** notifications, **admin payment** mutations, **Mini App** / **public** flow changes.
- **Baseline (Steps 1–12):** **`assign-operator`**, **`mark-in-work`**, **`compute_group_followup_visibility`**, **`compute_group_followup_assignment_visibility`**, admin handoff read/write as in **`docs/CHAT_HANDOFF.md`** **Completed Steps**.
...
**Staging (recommended):** smoke Step **13** mutation after **`mark-in-work`** on a **`group_followup_start`** row, then **`GET /admin/handoffs`** / detail — **Railway** / deploy health as usual.
...
**Completed step references:** `docs/CURSOR_PROMPT_PHASE_6_STEP_1.md` … `docs/CURSOR_PROMPT_PHASE_6_STEP_30.md` (historical); **`docs/CURSOR_PROMPT_PHASE_7_STEP_1.md`** … **`docs/CURSOR_PROMPT_PHASE_7_STEP_12.md`** (historical).  
**Plan:** `docs/IMPLEMENTATION_PLAN.md` — Phase 7.

## Recommended Next Prompt
Implement **Phase 7 / Step 13** — **narrow operator close/resolve** mutation for **`in_review`** **`group_followup_start`** handoffs (see **Next Safe Step**): **one** narrow admin write slice — **not** a broad claim engine, **not** notifications, **not** public/Mini App churn unless scoped, **not** **`per_seat` / `full_bus`**. Sources: **`docs/CHAT_HANDOFF.md`**, **`docs/GROUP_ASSISTANT_RULES.md`**, **`docs/TELEGRAM_SETUP.md`**, **`docs/TECH_SPEC_TOURS_BOT.md`**, `docs/IMPLEMENTATION_PLAN.md`, `docs/OPEN_QUESTIONS_AND_TECH_DEBT.md`, `docs/TESTING_STRATEGY.md`.
...
- **Approved checkpoint:** **Phase 7 / Step 12** complete — **Phase 7** surface through **Step 12** (Steps **1–11** as before, plus Step **12** **`POST /admin/handoffs/{id}/mark-in-work`** for **assigned** **`group_followup_start`** — **`open` → `in_review`**, **`in_review`** idempotent; **no** migration, **no** notifications); **`grp_private`** unchanged; **`tour_*`** / legacy **`/start`** unchanged; **no** automatic assignment from **Telegram `grp_*`** entry; **`TELEGRAM_BOT_USERNAME`** required for full group gating. **Forward:** **Phase 7 / Step 13** — **narrow operator close/resolve** for **`in_review`** **`group_followup_start`** (see **Next Safe Step**). Phase 6 closure: **`docs/PHASE_6_REVIEW.md`**. **Not** old **Phase 5 / Step 4–5** checkpoints; **not** admin payment mutation or **Phase 6 / Step 31** unless scoped.
...
Default forward path is **Phase 7 / Step 13** — **narrow operator close/resolve** for **`in_review`** **`group_followup_start`** — do not use legacy Phase 5 “next step” prompts for new implementation slices.
