...
## Current Status
The project is ready to continue from the **latest approved checkpoint**: **Phase 6 / Step 26 completed** — admin order **write** surface in **`app/services/admin_order_write.py`** includes **`POST /admin/orders/{order_id}/mark-cancelled-by-operator`** (Step **23**), **`POST /admin/orders/{order_id}/mark-duplicate`** (Step **24**), **`POST /admin/orders/{order_id}/mark-no-show`** (Step **25**), and **`POST /admin/orders/{order_id}/mark-ready-for-departure`** (Step **26**), in addition to the handoff **write** surface from Steps **19–22** (**`app/services/admin_handoff_write.py`**).
...
**Next work:** **Phase 6 / Step 27** — see **Next Safe Step**.
...
**Current phase (forward work):** **Phase 6 — Admin Panel MVP**. **Completed through Phase 6 / Step 26**; **next implementation checkpoint: Phase 6 / Step 27** (see **Next Safe Step**).

**Latest approved checkpoint:** **Phase 6 / Step 26** — **`POST /admin/orders/{order_id}/mark-ready-for-departure`** (narrow pre-departure booking mark; see **Current Status**); **Steps 23–25** order mutations remain as documented. Order **read** surfaces (**Steps 16–17**: lifecycle, correction visibility, action preview) are unchanged in **API shape**; **`ready_for_departure`** lifecycle projection refinement is **Step 27**, not Step **26**. **Step 18** handoff list/detail unchanged. **Still not via `/admin/*`:** **unassign**, broader **reassignment** redesign, **notifications** from admin handoff actions, **refund / capture / cancel-payment**, **broad** order editor, **merge** tooling, **move-to-another-tour/date** (until a narrow slice adds one), **full** admin SPA, **publication**, **bulk** ops, **hard-delete** tour. Internal **ops** JSON stays **separate** from **`/admin/*`**. Still **no** route/itinerary editor beyond current boarding slices.

**Earlier checkpoints:** Phase 6 / Steps 1–7 (foundation through core tour patch); Phase 6 / Steps 8–10 (boarding create / patch / delete); Phase 6 / Steps 11–12 (tour translation upsert/delete); Phase 6 / Steps 13–14 (boarding point translation upsert/delete); Phase 6 / Step 15 (tour archive/unarchive); Phase 6 / Step 16 (order detail payment correction visibility); Phase 6 / Step 17 (order detail action preview); Phase 6 / Step 18 (admin handoff queue read API); Phase 6 / Step 19 (admin handoff mark-in-review); Phase 6 / Step 20 (admin handoff close-only); Phase 6 / Step 21 (admin handoff assign); Phase 6 / Step 22 (admin handoff reopen); Phase 6 / Step 23 (admin mark-cancelled-by-operator); Phase 6 / Step 24 (admin mark-duplicate); Phase 6 / Step 25 (admin mark-no-show); Phase 6 / Step 26 (admin mark-ready-for-departure); Phase 5 accepted + Phase 5 Step 20 docs (`docs/PHASE_5_ACCEPTANCE_SUMMARY.md`).
...
**Forward:** implement **Phase 6 / Step 27** only as defined in **Next Safe Step** — do **not** use legacy Phase 5 “next step” prompts for new code.
...
- Phase 6 / Step 26 completed
  ...
  - **Intentionally not in this step:** **`admin_order_lifecycle`** projection update for **`ready_for_departure`** (may still classify as **`other`** until **Step 27**)
...
- **api layer** — FastAPI; public routes + Mini App routes + payments webhooks + **internal ops** JSON endpoints + **admin API** (`/admin/*`, `ADMIN_API_TOKEN`: overview, tours/orders **lists** with **optional read-only filters**, **`GET /admin/handoffs`** and **`GET /admin/handoffs/{handoff_id}`** handoff queue visibility, **`POST /admin/handoffs/{handoff_id}/mark-in-review`** (Step **19**), **`POST /admin/handoffs/{handoff_id}/close`** (Step **20**), **`POST /admin/handoffs/{handoff_id}/assign`** (Step **21**), **`POST /admin/handoffs/{handoff_id}/reopen`** (Step **22**), **`GET /admin/orders/{order_id}`** order detail with **Step 16** correction + **Step 17** action-preview fields, **`POST /admin/orders/{order_id}/mark-cancelled-by-operator`** (Step **23**), **`POST /admin/orders/{order_id}/mark-duplicate`** (Step **24**), **`POST /admin/orders/{order_id}/mark-no-show`** (Step **25**), **`POST /admin/orders/{order_id}/mark-ready-for-departure`** (Step **26**), **tour + order detail** incl. **`cover_media_reference`**, **`POST /admin/tours`** create **core** tours, **`POST /admin/tours/{tour_id}/archive`** and **`POST /admin/tours/{tour_id}/unarchive`**, **`PUT /admin/tours/{tour_id}/cover`** for **one** media reference string, **`PATCH /admin/tours/{tour_id}`** for **core** field updates only, **`POST` / `PATCH` / `DELETE`** boarding points, **`PUT` / `DELETE`** **`/admin/tours/{tour_id}/translations/{language_code}`** for **tour** translations, **`PUT` / `DELETE`** **`/admin/boarding-points/{boarding_point_id}/translations/{language_code}`** for **boarding** translations)
...
### Not Implemented Yet
- **Next (Step 27):** first narrow **admin order lifecycle-read refinement** — align **`admin_order_lifecycle`** (and list **lifecycle** filter consistency) so **`ready_for_departure`** + paid + active is **not** left as **`other`** when that is the steady state after Step **26**; **read/projection only**, **no** new mutations — see **Next Safe Step**.
- **Deferred alternative (not Step 27):** **narrow move-to-another-date/tour** order correction — **one** explicit path when product schedules it (separate checkpoint).
- **Still postponed:** admin **refund / capture / cancel-payment**, **broad** order status editor, **merge** tooling, **payment reconciliation** rewrite, handoff **unassign**, broader **reassignment** policy, full **operator queue/workflow engine**, **notifications** from admin actions, **public** handoff UX changes, **full** admin SPA, **publication**, **bulk** ops — per plan / product.
- **Phase 7–9:** group assistant, handoff at scale, content assistant, analytics — per `docs/IMPLEMENTATION_PLAN.md`

## Next Safe Step

**Phase 6 / Step 27 — first narrow admin order lifecycle-read refinement slice** — update **`app/services/admin_order_lifecycle.py`** (`describe_order_admin_lifecycle`, **`sql_predicate_for_lifecycle_kind`**, and any **admin list** filter wiring that must stay consistent) so orders in the **post–Step 26** steady state (**`booking_status=ready_for_departure`**, **`payment_status=paid`**, **`cancellation_status=active`**) get a **clear** **`lifecycle_kind` / `lifecycle_summary`** instead of falling through to **`other`**. **Read-side / projection only** — **no** new **`/admin/orders/*` mutations**, **no** payment-row writes, **no** reconciliation/webhook changes, **no** public/catalog/Mini App changes.

### Goal
Restore **operator-facing clarity** on admin **reads** after Step **26** introduced a new persisted **`booking_status`** — smallest safe adjustment before adding more order mutations.

### Safe scope boundaries
- **Service-layer** lifecycle mapping (+ tests + docs touch as needed); keep **repositories** persistence-oriented; **no** broad status matrix editor.

### Explicitly not in Step 27
- Refund/capture/cancel-payment, **move** booking, **merge** tooling, **broad** order editor, **reconciliation** rewrite, **full** admin SPA, new **notification** workflows.

**Completed step references:** `docs/CURSOR_PROMPT_PHASE_6_STEP_1.md` … `docs/CURSOR_PROMPT_PHASE_6_STEP_26.md` (historical).  
**Plan:** `docs/IMPLEMENTATION_PLAN.md` (Phase 6 — order oversight / restricted updates, taken as **narrow slices**).

## Recommended Next Prompt
Implement **Phase 6 / Step 27** (admin **order lifecycle-read refinement** for **`ready_for_departure`**, per **Next Safe Step**) using **`docs/CHAT_HANDOFF.md`**, `docs/IMPLEMENTATION_PLAN.md`, `docs/OPEN_QUESTIONS_AND_TECH_DEBT.md`, `docs/TECH_SPEC_TOURS_BOT.md`, and `docs/TESTING_STRATEGY.md`. Add **`docs/CURSOR_PROMPT_PHASE_6_STEP_27.md`** when starting — **do not** use legacy Phase 5 “next step” prompts for new work.
...
- continue from the last approved checkpoint in `docs/CHAT_HANDOFF.md` (**Phase 6 / Step 26** complete; forward work: **Phase 6 / Step 27** per **Next Safe Step**)
