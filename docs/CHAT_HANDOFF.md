...
**Still not implemented (explicit):** **Phase 7.1** beyond **Step 2** — **Step 3** (read-side adaptation for Mini App/private bot), **Step 4** (operator-assisted full-bus), optional direct whole-bus self-service — **only when explicitly scoped** (see **Next Safe Step**); staging smoke on deploy when convenient; **operator** two-way **chat** / free-form **messaging**, **handoff push notification** subsystem, **broad** **claim/takeover** engine, **full** **operator workflow engine**, **full** group assistant, **full** private “assistant” flow redesign; **booking/payment resolution** via Phase **7** **`grp_*`** beyond existing private/Mini App flows; **long-form** group replies; **public** booking/payment/waitlist/Mini App **behavior changes** (unless a future **Phase 7.1** slice explicitly scopes them); **handoff** rows from **group** chat runtime (Steps **3–4** remain **reply-only**). **Do not** mix **`per_seat` / `full_bus`** policy into Phase **7** handoff code **ad hoc**. **Live** Telegram validation **may** still depend on **Railway** / bot deploy health.
...
**Next work:** **Phase 7.1 — tour sales mode** — **Step 3** (read-side adaptation; **`docs/TOUR_SALES_MODE_DESIGN.md` §I**) when explicitly scoped (see **Next Safe Step**); design reference **`docs/TOUR_SALES_MODE_DESIGN.md`**; keep **`docs/CHAT_HANDOFF.md`**, **`docs/IMPLEMENTATION_PLAN.md`**, **`docs/TECH_SPEC_TOURS_BOT.md`**, **`docs/OPEN_QUESTIONS_AND_TECH_DEBT.md`** aligned as the sub-track proceeds; staging smoke (**`/health`**, **`grp_followup`** path, admin handoff reads, **`tours.sales_mode`** after migration) when deploy changes — **not** default **Phase 7** **`grp_followup`** code unless product **rescopes** Phase **7**. **Admin payment mutations** remain **not** the default.
...
**Current phase (forward work):** **Phase 7 — Group Assistant And Operator Handoff** — **closed** (**implementation** Steps **1–17** + **final consolidation** + **review / closure** in **`docs/PHASE_7_REVIEW.md`**). **Active implementation sub-track:** **Phase 7.1 — tour sales mode** — **Step 1** **frozen**; **Step 2** (backend policy **`TourSalesModePolicyService`**) **completed**; **Step 3** (read-side adaptation) is **next** **only when explicitly scoped** (see **Next Safe Step**). Phase 6 narrow admin API track **closed** through **Step 30** — see **`docs/PHASE_6_REVIEW.md`**.

**Latest approved checkpoint:** **Phase 7 review / closure accepted** — narrative closure in **`docs/PHASE_7_REVIEW.md`**; **shipped** surface remains: core chain (**Steps 1–17**) plus **read-side polish** (private **`grp_followup`** + admin **`group_followup_*`** labels share one **queued / assigned / in progress / closed** model with **`group_followup_queue_state`**); **`HandoffEntryService.group_followup_private_intro_key`** behavior as shipped; **`should_show_group_followup_resolved_confirmation`** retained for tests; **no** new workflow capabilities added in closure pass. **`grp_private`** unchanged. **Step 1** docs: **`docs/GROUP_ASSISTANT_RULES.md`**. **Also shipped (separate sub-track):** **Phase 7.1 / Sales Mode / Steps 1–2** — admin **`tour.sales_mode`** + backend **`TourSalesModePolicyRead`** — see **Completed Steps** → **Phase 7.1**. **Prior code baseline** unchanged: **Phase 6 / Step 30** **`move_placement_snapshot`**, **Step 29** move **POST**, admin handoffs/orders as implemented. **Admin payment mutations** remain **intentionally postponed** (see **`docs/OPEN_QUESTIONS_AND_TECH_DEBT.md`** **§1f**). **Still not:** **full** group assistant; **operator↔customer chat**; **handoff push notifications**; **broad** **claim/takeover** engine; **broad** **operator workflow engine**; **booking/payment resolution** **via** the Phase **7** **`grp_*`** chain beyond existing private/Mini App flows; **persisted** move audit/timeline rows; **unassign**; broader **reassignment** redesign; **refund / capture / cancel-payment**; **public** customer/Mini App flow redesign driven only by **`sales_mode`** without an explicit **Phase 7.1** slice. **`per_seat` / `full_bus`** customer-visible adaptation and enforcement beyond **Step 2** backend policy — **Phase 7.1** **Step 3+** (**not** part of Phase **7** closure). Internal **ops** JSON stays **separate** from **`/admin/*`**.
...
- **Scope:** backend/service-layer interpretation of **`tour.sales_mode`** only — **no** Mini App, private bot, reservation, payment, or Phase **7** handoff changes.
...
- **Tests:** **`tests/unit/test_tour_sales_mode_policy.py`** ( **`per_seat` / `full_bus`**, large **`seats_total`** + **`per_seat`**, default **`per_seat`**, unwired **`reservation_creation`** / **`mini_app_booking`**).
- **Explicitly not in this step:** wiring policy into catalog/API responses, reservation blocking for **`full_bus`**, operator-assisted flow, direct whole-bus booking, **`full_bus_price`**, **`bus_capacity`**.
...
- **Phase 7.1 / Sales Mode / Step 1 (admin source-of-truth)** — **`TourSalesMode`** + **`Tour.sales_mode`**; admin **list/detail/create/patch** expose **`sales_mode`**; Alembic **`20260416_06`**; **initial values** **`per_seat`**, **`full_bus`**; **no** booking/pricing policy, **no** Mini App/private bot adaptation, **no** whole-bus booking flow in this step
- **Phase 7.1 / Sales Mode / Step 2 (backend policy)** — **`TourSalesModePolicyRead`** + **`TourSalesModePolicyService`**; enum-only interpretation; **not** wired to reservation/Mini App/bot; tests **`test_tour_sales_mode_policy`**
...
- **`tour.sales_mode` (Phase 7.1):** tour-level **source of truth** for **commercial sales mode** (**Step 1**); **Step 2** centralizes interpretation in **`TourSalesModePolicyService`** — consumers should use that service **not** ad hoc seat heuristics

### Not Implemented Yet
- **Next (forward, active sub-track):** **Phase 7.1 / Sales Mode / Step 3** — **read-side adaptation** for Mini App + private bot from backend policy (**`docs/TOUR_SALES_MODE_DESIGN.md` §I**) **only when explicitly scoped** (see **Next Safe Step**); **separate** from Phase **7** handoff/operator code. Optional **later** slices: i18n polish, group **rate limits** — product-scoped.
- **Phase 7.1 — still postponed (after Step 2):** **`full_bus_price`**; **`bus_capacity`**; **Mini App** / **private bot** **read-side** exposure of policy; **reservation enforcement** for **`full_bus`**; **operator-assisted** full-bus path; **direct** whole-bus booking flow; **payment** / **availability** / **seat-semantics** changes driven by **`sales_mode`** without an explicit slice.
...
### Next safe step
**Phase 7.1 / Sales Mode / Step 1** is **closed** — **no** additional Step **1** work without a **new** checkpoint.

**Phase 7.1 / Sales Mode / Step 2** is **closed** for the **narrow** policy slice — **`TourSalesModePolicyService`** + **`TourSalesModePolicyRead`** shipped; **no** customer-surface wiring.

**Phase 7.1 / Sales Mode / Step 3** — **only when explicitly requested/scoped**.

**Goal:**
- **read-side adaptation** for **Mini App** and **private bot** using **backend-owned** policy (**`TourSalesModePolicyService`**) per **`docs/TOUR_SALES_MODE_DESIGN.md` §H–I**
- expose mode-aware **presentation** (copy, CTA shape, optional API fields) — **not** UI-invented commercial rules
- **do not** implement **direct whole-bus booking** or **operator-assisted** fulfillment unless that slice is **explicitly scheduled**

**Safe scope:**
- catalog / tour detail / preparation **read** payloads or bot messages that **reflect** **`TourSalesModePolicyRead`** (or a thin DTO derived from it)
- **optional narrow** reservation guard for **`full_bus`** **only if** explicitly in scope for Step **3** (design prefers operator path first — confirm with product)

**Still must not expand into (unless a future slice explicitly scopes it):**
- **payment** / **reconciliation** changes
- **seat decrement** semantics changes without review
- new **Phase 7** **`group_followup_*`** workflow features

- **Why:** Phase **7** is **closed** (**`docs/PHASE_7_REVIEW.md`**); **Phase 7.1 / Steps 1–2** shipped **source of truth + policy** — **Step 3** is **first customer-visible** slice, still **separate** from **`grp_followup_*`**.
- **Baseline (shipped):** **Phase 7.1 / Step 1** — **`TourSalesMode`**, **`Tour.sales_mode`**, admin + Alembic **`20260416_06`**; **Step 2** — **`app/services/tour_sales_mode_policy.py`**, **`app/schemas/tour_sales_mode_policy.py`**, **`tests/unit/test_tour_sales_mode_policy.py`**.
- **Out of scope unless explicitly re-scoped:** new **Phase 7** **`grp_followup`** features, **admin payment** mutations.
...
**Staging (recommended):** routine smoke (**`/health`**, **`grp_followup`**, admin handoffs) when deploy changes — **Railway** / deploy health as usual; **`tours.sales_mode`** + admin tour reads after schema deploy.
...
- **Phase 7.1:** **Step 1** = data/admin; **Step 2** = backend policy; **Step 3** = read-side adaptation — **keep** policy in **`TourSalesModePolicyService`**.

**Completed step references:** `docs/CURSOR_PROMPT_PHASE_6_STEP_1.md` … `docs/CURSOR_PROMPT_PHASE_6_STEP_30.md` (historical); **`docs/CURSOR_PROMPT_PHASE_7_STEP_1.md`** … **`docs/CURSOR_PROMPT_PHASE_7_REVIEW_CLOSURE.md`** (historical). **Closure:** **`docs/PHASE_7_REVIEW.md`**. **Phase 7.1:** **Completed Steps** → **Phase 7.1 / Sales Mode / Step 1–2 completed**.  
**Plan:** `docs/IMPLEMENTATION_PLAN.md` — Phase 7 **closed**; **active sub-track:** **Phase 7.1 — tour sales mode** (**Steps 1–2** done; **Step 3** next when scoped).

## Recommended Next Prompt
Continue **Phase 7.1 / Sales Mode / Step 3** (see **Next Safe Step**): **read-side adaptation** for Mini App + private bot using **`TourSalesModePolicyService`** per **`docs/TOUR_SALES_MODE_DESIGN.md`** — **no** payment/reconciliation changes unless scoped; **no** direct whole-bus booking unless explicitly scheduled; **no** Phase **7** **`group_followup_*`** reopening. **Phase 7** remains **closed** — **`docs/PHASE_7_REVIEW.md`**. Sources: **`docs/TOUR_SALES_MODE_DESIGN.md`**, **`docs/CHAT_HANDOFF.md`**, **`docs/TECH_SPEC_TOURS_BOT.md`**, **`docs/IMPLEMENTATION_PLAN.md`**, **`docs/OPEN_QUESTIONS_AND_TECH_DEBT.md`**, **`docs/TESTING_STRATEGY.md`**, **`docs/PHASE_6_REVIEW.md`**, **`docs/PHASE_7_REVIEW.md`**.
...
- **Approved checkpoint:** **Phase 7 closed** — **`docs/PHASE_7_REVIEW.md`** (**review / closure accepted**); **implementation** through Steps **1–17** + **final consolidation** (private **`grp_followup`** + admin **`group_followup_*`** read labels; **no** operator chat, **no** handoff push notifications in this track); **`grp_private`** unchanged; **`tour_*`** / legacy **`/start`** unchanged; **no** automatic assignment from **Telegram `grp_*`** entry; **`TELEGRAM_BOT_USERNAME`** required for full group gating. **Active sub-track:** **Phase 7.1 — tour sales mode** — **Step 1** (**`tour.sales_mode`** admin/source-of-truth) **frozen**; **Step 2** (**`TourSalesModePolicyService`**) **completed** — **no** Mini App/private bot/reservation wiring yet; **Forward:** **Step 3** (read-side adaptation per **`docs/TOUR_SALES_MODE_DESIGN.md` §I**) **when scoped** (see **Next Safe Step**) — **separate** from **Phase 7** **`grp_followup_*`**. Phase 6 closure: **`docs/PHASE_6_REVIEW.md`**. **Not** old **Phase 5 / Step 4–5** checkpoints; **not** admin payment mutation or **Phase 6 / Step 31** unless scoped.
...
Default forward path is **Phase 7.1 — tour sales mode** — **Step 3** (Mini App + private bot read-side adaptation) when scoped — do not use legacy Phase 5 “next step” prompts; **do not** assume further Phase **7** **`grp_followup`** implementation without **explicit** product rescoping.
