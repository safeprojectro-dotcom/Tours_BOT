...
- **Next safe order:**
  1. **Y38 + Y39 (docs, accepted) —** **`docs/SUPPLIER_INTERACTION_GATE.md`** (Layer C / intent **≠** trigger) and **`docs/SUPPLIER_ENTRY_POINTS.md`** (supplier interaction may **start** only at **explicit** entry points). **`operator_workflow_intent`** and **`operator-decision`** are **not** triggers. Before any `app/` supplier-interaction work: **implementation ticket** must **name** one Y39 **entry family** and **surface**; no hidden hooks; **no** messaging, RFQ, booking/payment, Mini App, execution links, identity bridge, or notifications unless that slice is **explicitly** in scope and accepted. **Do not** auto-notify or auto-contact suppliers from intent set alone.
