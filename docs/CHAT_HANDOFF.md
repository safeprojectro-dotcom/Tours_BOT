...
- **B13A (completed, docs):** **Channel Adapter Design** — main doc **[`docs/B13_CHANNEL_ADAPTER_DESIGN.md`](B13_CHANNEL_ADAPTER_DESIGN.md)**. Design **separates** review-package / **`operator_workflow`** gates, **content assembly**, optional future **idempotency / audit / outbox** layer, and **channel adapters** (transport I/O). **Existing Telegram publish** path is documented as the **baseline**. **No** code, **no** publish behavior or readiness changes, **no** Mini App / booking / payment / orders / migrations. **Recommended next implementation:** **B13B** — adapter interface + Telegram wrapper, **behavior-preserving** refactor only.
