...
- **B12/B13 (Telegram showcase — first slice):** **`supplier_offer_showcase_message`** — explicit template vs CTA vs HTML assembly; **`build_showcase_publication`** returns **text-only** send (**`photo_url`** always **`None`**; **`sendPhoto`** not used); primary CTA **„Deschide în bot”** → stable **`supoffer_<id>`** bot deep link; secondary **„Vezi în aplicație”** → supplier-offer Mini App landing (**non-booking** wording vs former **„Rezervă”**); **`Preț`** line **„orientativ”** + disclaimer row for Mini App/booking truth. **`POST .../publish`** orchestration unchanged.
