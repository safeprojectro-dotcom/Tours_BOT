...
**Continuity anchor (read this first):** The **live** approved checkpoint is **Phase 7 / Step 7 completed** — **Phase 7** surface to date: **group trigger** + **handoff trigger** helpers (Step **2**); **minimal group runtime gating** (Step **3**); **category-aware** short escalation wording (Step **4**); **private CTA** deep-link foundation (Step **5**); **narrow private `/start`** for **`grp_private`** / **`grp_followup`** (Step **6** — two **distinct** short intros + catalog; **`tour_*`** and other legacy **`/start`** payloads **unchanged**); **narrow runtime handoff persistence** for **`/start grp_followup`** only (Step **7** — reason **`group_followup_start`**, dedupe-safe for **open** rows, **no** persistence for **`grp_private`**). **No** operator assignment from this flow, **no** booking/payment initiation from Phase **7** **`grp_*`** paths. **Next:** **Phase 7 / Step 8** — **focused runtime/bot-flow test coverage** for the **private `grp_followup`** chain — see **Next Safe Step**. **Phase 6** / **`docs/PHASE_6_REVIEW.md`** — historical admin closure. **Not** **Phase 6 / Step 31**, **not** **admin payment** mutation. **Phase 5** — do **not** resume old **Step 4–5** narratives. Use **`Next Safe Step`** + **Current Status**.

## Current Status
The **latest approved checkpoint** is **Phase 7 / Step 7 completed** (**documentation + code checkpoint aligned**). **Phase 7 implemented surface (Steps 1–7):** Step **1** — **`docs/GROUP_ASSISTANT_RULES.md`**; Step **2** — **group** + **handoff** trigger helpers (`evaluate_group_trigger`, `evaluate_handoff_triggers`, …); Step **3** — **minimal group runtime gating**; Step **4** — **category-aware** short escalation wording in group replies; Step **5** — **private CTA** **`t.me`** deep-link foundation (`grp_private` / `grp_followup`); Step **6** — **narrow private `/start`** branching: **`grp_private`** → short **`start_grp_private_intro`**; **`grp_followup`** → **different** short **`start_grp_followup_intro`**; then **`_send_catalog_overview`**; **`/start tour_*`** and other legacy **`/start`** behavior **unchanged**; Step **7** — **narrow** **`handoffs`** persistence on **`/start grp_followup`** only (**reason** **`group_followup_start`**; **dedupe** for repeated **`/start grp_followup`** while an **open** matching row exists; **new** row allowed after **closed**; **`grp_private`** — **no** handoff write). **No** operator assignment from this flow; **no** booking/payment initiation from Phase **7** **`grp_*`** paths. **Step 1** rules: **`docs/GROUP_ASSISTANT_RULES.md`**. **Public** booking / payment / waitlist / Mini App **flows and APIs did not change**.

**Still not implemented (explicit):** **automatic operator assignment** from the **`grp_followup`** handoff entry; **full** **operator workflow engine**; **full** group assistant; **full** private “assistant” flow redesign; **booking/payment resolution** driven by the Phase **7** group/private **`grp_*`** chain beyond existing private/Mini App flows; **long-form** group replies; **public** booking/payment/waitlist/Mini App **behavior changes**; **handoff** rows from **group** chat runtime (Steps **3–4** remain **reply-only**). **Live** Telegram validation **may** still depend on **Railway** / bot deploy health — **product scope** for Phase **7** remains as documented.
...
**Next work:** **Phase 7 / Step 8** — **focused runtime/bot-flow test coverage** for the **private `grp_followup`** chain (see **Next Safe Step**): validate end-to-end expectations before widening operator behavior — **not** admin payment work. **i18n** / **rate limits** remain **separate** optional slices unless bundled by product. **Admin payment mutations** are **not** the default.
...
### Not Implemented Yet
- **Next (Phase 7):** **Phase 7 / Step 8** — **focused runtime/bot-flow test coverage** for the **private `grp_followup`** chain (end-to-end validation **before** widening operator behavior) — see **Next Safe Step**. *Alternative (product-prioritized only):* **narrow operator visibility/readiness** for **`group_followup_start`** handoffs — **not** the default forward path here. Optional **later** slices: i18n polish, group **rate limits**.
- **Group / handoff:** Steps **1–7** = **rules + helpers + group gating + escalation wording + deep links + private `/start`** + **narrow** **`handoffs`** write on **`/start grp_followup`** (**Step 7**); **group** chat path (Steps **3–4**) still **no** DB handoff rows — **`docs/OPEN_QUESTIONS_AND_TECH_DEBT.md`** §**19**.
...
- **Phase 7–9 (beyond Step 8):** optional group assistant at scale, content assistant, analytics — per `docs/IMPLEMENTATION_PLAN.md`

## Next Safe Step

**Primary (single outcome):** **Phase 7 / Step 8** — **focused runtime/bot-flow test coverage** for the **private `grp_followup`** chain (*alias:* **narrow integration-style tests / smoke harness for `grp_followup` handoff persistence** — same intent).

- **Why:** Step **7** added **persisted** **`handoffs`** rows for **`/start grp_followup`**. The **next smallest safe increment** is to **lock in** the runtime chain with **focused** tests (and/or documented manual smoke) **before** expanding **operator** behavior — **without** new product features in the same slice.
- **In scope (typical):** additional **focused** tests or a **thin** test harness that exercises **`private_entry`** **`grp_followup`** → **`HandoffEntryService`** path expectations; optional **checklist** for **Railway** staging; **no** new business rules unless required to fix gaps.
- **Out of scope for Step 8 unless rescoped:** **automatic operator assignment**, **full** workflow engine, **admin payment** mutations, **Mini App** / **public API** surface changes, **broad** refactors.
- **Alternative (not default here):** **Phase 7 / Step 8** — **narrow operator visibility/readiness** for **`group_followup_start`** handoffs — only if product prioritizes **operator UX** before more test harness work.
- **Baseline (Steps 1–7):** **`docs/GROUP_ASSISTANT_RULES.md`**; **`group_trigger_evaluation`**, **`handoff_trigger_evaluation`**; **`group_gating`**, **`group_chat_gating`**, **`group_private_cta`**; **`private_entry`** **`grp_*`** + **`grp_followup`** persistence (**`group_followup_start`**).
...
**Staging (recommended):** **Railway** (or host) bot deploy healthy → smoke **`/start grp_followup`** → confirm **`handoffs`** row (**reason** **`group_followup_start`**) and **dedupe** behavior; re-check **`grp_private`** introduces **no** row.
...
**Completed step references:** `docs/CURSOR_PROMPT_PHASE_6_STEP_1.md` … `docs/CURSOR_PROMPT_PHASE_6_STEP_30.md` (historical); **`docs/CURSOR_PROMPT_PHASE_7_STEP_1.md`** … **`docs/CURSOR_PROMPT_PHASE_7_STEP_7.md`** (historical).  
**Plan:** `docs/IMPLEMENTATION_PLAN.md` — Phase 7.

## Recommended Next Prompt
Implement **Phase 7 / Step 8** — **focused runtime/bot-flow test coverage** for the **private `grp_followup`** chain (see **Next Safe Step**): extend or add **narrow** tests / smoke checklist so the **`grp_followup`** → **`group_followup_start`** persistence path is **validated end-to-end** before operator workflow expansion — **no** assignment engine, **no** public API/Mini App churn unless scoped. Sources: **`docs/CHAT_HANDOFF.md`**, **`docs/GROUP_ASSISTANT_RULES.md`**, **`docs/TELEGRAM_SETUP.md`**, **`docs/TECH_SPEC_TOURS_BOT.md`**, `docs/IMPLEMENTATION_PLAN.md`, `docs/OPEN_QUESTIONS_AND_TECH_DEBT.md`, `docs/TESTING_STRATEGY.md`.
...
- `/start grp_private` / `/start grp_followup` (Phase 7 / Step 6) — **distinct** short intros (`start_grp_private_intro` vs `start_grp_followup_intro`) then catalog; **`/start grp_followup`** also persists **narrow** handoff (**Phase 7 / Step 7**, reason **`group_followup_start`**, dedupe **open**)
- guided browsing by:
  - date presets
  - destination keyword
...
- **group chat:** Phase **7 / Steps 3–6** — **trigger gating** + **category-aware** wording + **`t.me`** CTA (`group_gating`); Phase **7 / Step 6** — matching **`grp_*`** private `/start` intros; Phase **7 / Step 7** — **`/start grp_followup`** private handoff row (**not** group-side persistence) — **not** full group assistant (see **`docs/GROUP_ASSISTANT_RULES.md`**)
...
- **Approved checkpoint:** **Phase 7 / Step 7** complete — **Phase 7** surface through **Step 7** (helpers, group gating, escalation wording, deep links, **`grp_*`** private `/start`, **`/start grp_followup`** → **`handoffs`** row **`group_followup_start`**); **`grp_private`** unchanged; **`tour_*`** / legacy **`/start`** unchanged; **no** operator assignment from **`grp_followup`** entry; **`TELEGRAM_BOT_USERNAME`** required for full group gating. **Forward:** **Phase 7 / Step 8** — **focused runtime/bot-flow test coverage** for the **`grp_followup`** chain (see **Next Safe Step**). Phase 6 closure: **`docs/PHASE_6_REVIEW.md`**. **Not** old **Phase 5 / Step 4–5** checkpoints; **not** admin payment mutation or **Phase 6 / Step 31** unless scoped.
...
Default forward path is **Phase 7 / Step 8** — **focused test coverage / validation** for the **`grp_followup`** persistence chain — do not use legacy Phase 5 “next step” prompts for new implementation slices.
