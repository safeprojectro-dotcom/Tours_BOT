...
**Continuity anchor (read this first):** The **live** approved checkpoint is **Phase 7 / Step 11 completed** — **Phase 7** surface to date: **group trigger** + **handoff trigger** helpers (Step **2**); **minimal group runtime gating** (Step **3**); **category-aware** short escalation wording (Step **4**); **private CTA** deep-link foundation (Step **5**); **narrow private `/start`** for **`grp_private`** / **`grp_followup`** (Step **6**); **narrow runtime handoff persistence** for **`/start grp_followup`** (Step **7**); **focused runtime/bot-flow tests** for that chain (Step **8**); **narrow admin read-side visibility** for **`group_followup_start`** (Step **9** — **`is_group_followup`**, **`source_label`**); **narrow admin assignment** for **`group_followup_start`** only (Step **10** — **`POST /admin/handoffs/{handoff_id}/assign-operator`**; same narrow rules as Step **21** **`assign`**; general **`POST .../assign`** **unchanged**); **read-side work-state visibility** for **assigned** **`group_followup_start`** (Step **11** — **`is_assigned_group_followup`**, **`group_followup_work_label`** on **`GET /admin/handoffs`**, handoff detail, order-detail **`handoffs`**; **`compute_group_followup_assignment_visibility`**; **read-only**). **No** automatic operator assignment **from the Telegram `grp_*` bot paths**, **no** booking/payment initiation from Phase **7** **`grp_*`** paths. **Next:** **Phase 7 / Step 12** — **narrow operator acknowledge / take-in-work** mutation for **assigned** **`group_followup_start`** handoffs — see **Next Safe Step**. **Phase 6** / **`docs/PHASE_6_REVIEW.md`** — historical admin closure. **Not** **Phase 6 / Step 31**, **not** **admin payment** mutation. **Phase 5** — do **not** resume old **Step 4–5** narratives. Use **`Next Safe Step`** + **Current Status**.

## Current Status
The **latest approved checkpoint** is **Phase 7 / Step 11 completed** (**documentation + code checkpoint aligned**). **Phase 7 implemented surface (Steps 1–11):** Step **1** — **`docs/GROUP_ASSISTANT_RULES.md`**; Step **2** — **group** + **handoff** trigger helpers (`evaluate_group_trigger`, `evaluate_handoff_triggers`, …); Step **3** — **minimal group runtime gating**; Step **4** — **category-aware** short escalation wording in group replies; Step **5** — **private CTA** **`t.me`** deep-link foundation (`grp_private` / `grp_followup`); Step **6** — **narrow private `/start`** branching: **`grp_private`** / **`grp_followup`** intros + catalog; legacy **`tour_*`** **unchanged**; Step **7** — **narrow** **`handoffs`** persistence on **`/start grp_followup`** (**reason** **`group_followup_start`**; dedupe **open**; **`grp_private`** — **no** write); Step **8** — **focused runtime/bot-flow tests** (**`test_private_entry_grp_followup_chain.py`**); Step **9** — **admin read visibility** (**`is_group_followup`**, **`source_label`**); Step **10** — **`POST /admin/handoffs/{handoff_id}/assign-operator`** — **only** **`reason=group_followup_start`**; same narrow assignment rules as Step **21**; **`POST .../assign`** for other reasons **unchanged**; Step **11** — **read-side work-state** for **`group_followup_start`**: **`is_assigned_group_followup`**, **`group_followup_work_label`** on handoff **list**, **detail**, order-detail **`handoffs`** — **derived only**; **no** write semantics change, **no** booking/payment/public/Mini App changes. **No** automatic assignment **from Telegram `grp_*` paths**; **no** booking/payment initiation from Phase **7** **`grp_*`** paths. **Step 1** rules: **`docs/GROUP_ASSISTANT_RULES.md`**.

**Still not implemented (explicit):** **next** documented slice — **Phase 7 / Step 12** — **narrow operator acknowledge / take-in-work** mutation for **assigned** **`group_followup_start`** handoffs (see **Next Safe Step**); **broad** **claim/takeover** engine, **notifications** from this flow, **full** **operator workflow engine**, **full** group assistant, **full** private “assistant” flow redesign; **booking/payment resolution** via Phase **7** **`grp_*`** beyond existing private/Mini App flows; **long-form** group replies; **public** booking/payment/waitlist/Mini App **behavior changes**; **handoff** rows from **group** chat runtime (Steps **3–4** remain **reply-only**). **Live** Telegram validation **may** still depend on **Railway** / bot deploy health — **product scope** for Phase **7** remains as documented.
...
**Next work:** **Phase 7 / Step 12** — **narrow operator acknowledge / take-in-work** mutation for **assigned** **`group_followup_start`** handoffs (see **Next Safe Step**): **one** narrow admin mutation slice — **not** a broad workflow engine — **not** admin payment work. **i18n** / **rate limits** remain **separate** optional slices unless bundled by product. **Admin payment mutations** are **not** the default.
...
**Current phase (forward work):** **Phase 7 — Group Assistant And Operator Handoff**. **Phase 7 / Steps 1–11** **completed**. **Next:** **Phase 7 / Step 12** (see **Next Safe Step**). Phase 6 narrow admin API track **closed** through **Step 30** — see **`docs/PHASE_6_REVIEW.md`**.

**Latest approved checkpoint:** **Phase 7 / Step 11** — **Phase 7** through **Step 11** (Steps **1–10** as before, plus Step **11** **read-side** **`is_assigned_group_followup`**, **`group_followup_work_label`** on handoff list, detail, and order-detail **`handoffs`** — **no** write semantics change); **`grp_private`** unchanged; **`tour_*`** / legacy **`/start`** unchanged; **no** booking/payment handling **in group**; **no** automatic assignment from **Telegram `grp_*`** entry; **public** flows **unchanged** by Step **11**. **Step 1** docs: **`docs/GROUP_ASSISTANT_RULES.md`**. **Prior code baseline** unchanged: **Phase 6 / Step 30** **`move_placement_snapshot`**, **Step 29** move **POST**, admin handoffs/orders as implemented. **Admin payment mutations** remain **intentionally postponed** (see **`docs/OPEN_QUESTIONS_AND_TECH_DEBT.md`** **§1f**). **Still not:** **full** group assistant; **broad** **claim/takeover** engine; **broad** **operator workflow engine**; **booking/payment resolution** **via** the Phase **7** **`grp_*`** chain beyond existing private/Mini App flows; **persisted** move audit/timeline rows; **unassign**; broader **reassignment** redesign; **notifications** from admin handoff actions; **refund / capture / cancel-payment**; **public** customer/Mini App flow redesign. Internal **ops** JSON stays **separate** from **`/admin/*`**.

**Earlier checkpoints:** Phase 7 / Step 11 (**read-side** **`is_assigned_group_followup`**, **`group_followup_work_label`** for **`group_followup_start`**); Phase 7 / Step 10 (**`assign-operator`** for **`group_followup_start`** only); Phase 7 / Step 9 (admin **`group_followup_start`** visibility — **`is_group_followup`**, **`source_label`**); Phase 7 / Step 8 (focused **`grp_followup`** chain tests); Phase 7 / Step 7 (narrow **`grp_followup`** handoff persistence); Phase 7 / Step 6 (narrow **`grp_*`** private `/start`); Phase 7 / Step 5 (private CTA deep-link foundation on group replies); Phase 7 / Step 4 (handoff-aware short replies in narrow group path); Phase 7 / Step 3 (narrow group runtime gating + default ack); Phase 7 / Step 2 (trigger helper services + unit tests); Phase 7 / Step 1 ( **`docs/GROUP_ASSISTANT_RULES.md`** — rules only); Phase 6 / Steps 1–7 (foundation through core tour patch); Phase 6 / Steps 8–10 (boarding create / patch / delete); Phase 6 / Steps 11–12 (tour translation upsert/delete); Phase 6 / Steps 13–14 (boarding point translation upsert/delete); Phase 6 / Step 15 (tour archive/unarchive); Phase 6 / Step 16 (order detail payment correction visibility); Phase 6 / Step 17 (order detail action preview); Phase 6 / Step 18 (admin handoff queue read API); Phase 6 / Step 19 (admin handoff mark-in-review); Phase 6 / Step 20 (admin handoff close-only); Phase 6 / Step 21 (admin handoff assign); Phase 6 / Step 22 (admin handoff reopen); Phase 6 / Step 23 (admin mark-cancelled-by-operator); Phase 6 / Step 24 (admin mark-duplicate); Phase 6 / Step 25 (admin mark-no-show); Phase 6 / Step 26 (admin mark-ready-for-departure); Phase 6 / Step 27 (admin lifecycle **`ready_for_departure_paid`** read refinement); Phase 6 / Step 28 (admin order move-readiness read-only fields); Phase 6 / Step 29 (admin order narrow **move** mutation); Phase 6 / Step 30 (admin **`move_placement_snapshot`** read-only); Phase 5 accepted + Phase 5 Step 20 docs (`docs/PHASE_5_ACCEPTANCE_SUMMARY.md`).
...
### Not Implemented Yet
- **Next (Phase 7):** **Phase 7 / Step 12** — **narrow operator acknowledge / take-in-work** mutation for **assigned** **`group_followup_start`** handoffs — see **Next Safe Step**. Optional **later** slices: i18n polish, group **rate limits**.
- **Group / handoff:** Steps **1–11** = **rules + helpers + group gating + escalation wording + deep links + private `/start`** + **narrow** **`handoffs`** write on **`/start grp_followup`** (**Step 7**) + **focused tests** (**Step 8**) + **admin read visibility** (**Step 9**) + **narrow `assign-operator`** (**Step 10**) + **read-side work-state labels** (**Step 11**); **group** chat path (Steps **3–4**) still **no** DB handoff rows — **`docs/OPEN_QUESTIONS_AND_TECH_DEBT.md`** §**19**.
...
- **Longer-term (beyond Phase 7 / Step 12 checkpoint):** optional group assistant at scale, content assistant, analytics — per `docs/IMPLEMENTATION_PLAN.md`

## Next Safe Step

**Primary (single outcome):** **Phase 7 / Step 12** — **narrow operator acknowledge / take-in-work** mutation for **assigned** **`group_followup_start`** handoffs (*one* minimal **POST**-style slice — e.g. dedicated route or tightly scoped service method — so an **already assigned** **`group_followup_start`** row can record **take-in-work** / **acknowledged** state **without** a broad claim engine; exact persistence shape is a **Step 12** design choice — prefer **narrow** columns or **JSON** ops detail only if unavoidable; **no** booking/payment side effects).

- **Why:** Step **7** — persistence; Step **8** — chain tests; Step **9** — origin visibility; Step **10** — **assign**; Step **11** — **read** triage labels. The **next smallest increment** is **one** operator **action** after assignment — still **no** full workflow redesign, **no** **internal ops** claim takeover, **no** notifications unless explicitly scoped later.
- **In scope (typical):** mutation **only** for **`reason=group_followup_start`** with **`assigned_operator_id`** set (and **narrow** status gates TBD in Step **12** prompt); responses reuse **`AdminHandoffRead`** where practical; **no** changes to **`assign`** / **`assign-operator`** semantics unless an unavoidable shared helper adjustment is scoped.
- **Out of scope for Step 12 unless rescoped:** **broad** takeover for all handoff reasons, **customer** notifications, **admin payment** mutations, **Mini App** / **public** flow changes, **full** operator inbox productization.
- **Baseline (Steps 1–11):** **`assign-operator`**, **`compute_group_followup_visibility`**, **`compute_group_followup_assignment_visibility`**, admin handoff read/write as in **`docs/CHAT_HANDOFF.md`** **Completed Steps**.
...
**Staging (recommended):** smoke new Step **12** mutation on a **`group_followup_start`** row **after** **`assign-operator`**, then **`GET /admin/handoffs`** / detail — **Railway** / deploy health as usual.
...
**Completed step references:** `docs/CURSOR_PROMPT_PHASE_6_STEP_1.md` … `docs/CURSOR_PROMPT_PHASE_6_STEP_30.md` (historical); **`docs/CURSOR_PROMPT_PHASE_7_STEP_1.md`** … **`docs/CURSOR_PROMPT_PHASE_7_STEP_11.md`** (historical).  
**Plan:** `docs/IMPLEMENTATION_PLAN.md` — Phase 7.

## Recommended Next Prompt
Implement **Phase 7 / Step 12** — **narrow operator acknowledge / take-in-work** mutation for **assigned** **`group_followup_start`** handoffs (see **Next Safe Step**): **one** narrow admin write slice — **not** a broad claim engine, **not** notifications, **not** public/Mini App churn unless scoped. Sources: **`docs/CHAT_HANDOFF.md`**, **`docs/GROUP_ASSISTANT_RULES.md`**, **`docs/TELEGRAM_SETUP.md`**, **`docs/TECH_SPEC_TOURS_BOT.md`**, `docs/IMPLEMENTATION_PLAN.md`, `docs/OPEN_QUESTIONS_AND_TECH_DEBT.md`, `docs/TESTING_STRATEGY.md`.
...
- **Approved checkpoint:** **Phase 7 / Step 11** complete — **Phase 7** surface through **Step 11** (Steps **1–10** as before, plus Step **11** read-side **`is_assigned_group_followup`**, **`group_followup_work_label`** on admin handoff list, detail, order-detail **`handoffs`** — **no** write semantics change; **no** notifications); **`grp_private`** unchanged; **`tour_*`** / legacy **`/start`** unchanged; **no** automatic assignment from **Telegram `grp_*`** entry; **`TELEGRAM_BOT_USERNAME`** required for full group gating. **Forward:** **Phase 7 / Step 12** — **narrow operator acknowledge / take-in-work** mutation for **assigned** **`group_followup_start`** handoffs (see **Next Safe Step**). Phase 6 closure: **`docs/PHASE_6_REVIEW.md`**. **Not** old **Phase 5 / Step 4–5** checkpoints; **not** admin payment mutation or **Phase 6 / Step 31** unless scoped.
...
Default forward path is **Phase 7 / Step 12** — **narrow operator acknowledge / take-in-work** for **assigned** **`group_followup_start`** — do not use legacy Phase 5 “next step” prompts for new implementation slices.
