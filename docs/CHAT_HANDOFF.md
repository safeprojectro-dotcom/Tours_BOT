...
**Next work:** **Phase 7 / Step 18** — **narrow private escalation CTA refinement for active/in-work `grp_followup` states** (see **Next Safe Step**): **one** small **private copy/CTA** tweak (e.g. clearer safe **`/contact`** / **`/human`** lines when **`start_grp_followup_readiness_*`** applies) — **not** operator chat, **not** notification subsystem — **not** admin payment work. **i18n** / **rate limits** remain **separate** optional slices unless bundled by product. **Admin payment mutations** are **not** the default.
...
### Not Implemented Yet
- **Next (Phase 7):** **Phase 7 / Step 18** — **narrow private escalation CTA refinement for active/in-work `grp_followup` states** — see **Next Safe Step** (tighter safe **/contact** / **/human** wording when **readiness** intros apply — **still** **no** operator chat, **no** notification subsystem). Optional **later** slices: i18n polish, group **rate limits**.
- **Group / handoff:** Steps **1–17** = **rules + helpers + group gating + escalation wording + deep links + private `/start`** + **narrow** **`handoffs`** write on **`/start grp_followup`** (**Step 7**) + **focused tests** (**Step 8**) + **admin read visibility** (**Step 9**) + **narrow `assign-operator`** (**Step 10**) + **read-side work-state labels** (**Step 11**) + **narrow `mark-in-work`** (**Step 12**) + **narrow `resolve-group-followup`** + **`group_followup_resolution_label`** (**Steps 13–14**) + **`group_followup_queue_state`** + **`group_followup_queue`** list filter (**Step 15**) + **private resolved confirmation** on **`grp_followup`** (**Step 16**) + **private readiness/history signal** (**Step 17**); **group** chat path (Steps **3–4**) still **no** DB handoff rows — **`docs/OPEN_QUESTIONS_AND_TECH_DEBT.md`** §**19**.
...
- **Longer-term (beyond Phase 7 / Step 18 checkpoint):** optional group assistant at scale, content assistant, analytics — per `docs/IMPLEMENTATION_PLAN.md`

## Next Safe Step

**Primary (single outcome):** **Phase 7 / Step 18** — **narrow private escalation CTA refinement for active/in-work `grp_followup` states** (*one* small **private** copy pass so **`start_grp_followup_readiness_pending`**, **`_assigned`**, and **`_in_progress`** lines include a **clearer, still safe** pointer to **`/contact`** / **`/human`** where product wants it — **without** operator↔customer chat, **without** handoff push notifications, **without** new admin API or persistence).

- **Why:** Steps **7–17** — repeat **`grp_followup`** entries already surface **readiness**; the **next smallest increment** is **escalation CTA clarity** for **active** / **in-work** states only — still **no** notification subsystem, **no** two-way operator messaging.
- **In scope (typical):** **`app/bot/messages.py`** (and **only** if needed **`private_entry`**) — wording only; **prefer** **no** schema migration, **no** new **`handoffs`** semantics.
- **Out of scope for Step 18 unless rescoped:** **`per_seat` / `full_bus`**, **broad** takeover/reassign, **operator chat**, **customer** notification pipeline, **admin payment** mutations, **Mini App** / **public** booking flow changes.
- **Baseline (Steps 1–17):** **`group_followup_private_intro_key`**, **`start_grp_followup_readiness_*`**, **`start_grp_followup_resolved_intro`**, admin **assign** / **in-work** / **resolve**, **`group_followup_queue_state`** — see **`docs/CHAT_HANDOFF.md`** **Completed Steps**.
...
**Staging (recommended):** smoke **`/start grp_followup`** with **open**, **assigned-open**, and **`in_review`** fixtures — confirm copy stays short and **no** new side effects — **Railway** / deploy health as usual.
...
**Completed step references:** `docs/CURSOR_PROMPT_PHASE_6_STEP_1.md` … `docs/CURSOR_PROMPT_PHASE_6_STEP_30.md` (historical); **`docs/CURSOR_PROMPT_PHASE_7_STEP_1.md`** … **`docs/CURSOR_PROMPT_PHASE_7_STEP_12.md`** + **`docs/CURSOR_PROMPT_PHASE_7_STEP_13_14_COMBINED.md`** + **`docs/CURSOR_PROMPT_PHASE_7_STEP_15.md`** + **`docs/CURSOR_PROMPT_PHASE_7_STEP_16.md`** + **`docs/CURSOR_PROMPT_PHASE_7_STEP_17.md`** (historical).  
**Plan:** `docs/IMPLEMENTATION_PLAN.md` — Phase 7.

## Recommended Next Prompt
Implement **Phase 7 / Step 18** — **narrow private escalation CTA refinement for active/in-work `grp_followup` states** (see **Next Safe Step**): **one** small **private** wording pass for **readiness** intros — **not** operator chat, **not** handoff notification subsystem, **not** public/Mini App churn unless scoped, **not** **`per_seat` / `full_bus`**. Sources: **`docs/CHAT_HANDOFF.md`**, **`docs/GROUP_ASSISTANT_RULES.md`**, **`docs/TELEGRAM_SETUP.md`**, **`docs/TECH_SPEC_TOURS_BOT.md`**, `docs/IMPLEMENTATION_PLAN.md`, `docs/OPEN_QUESTIONS_AND_TECH_DEBT.md`, `docs/TESTING_STRATEGY.md`.
...
- **Approved checkpoint:** **Phase 7 / Step 17** complete — **Phase 7** surface through **Step 17** (Steps **1–16** as before, plus **`group_followup_private_intro_key`** + **`start_grp_followup_readiness_*`** for repeat **`grp_followup`** — **read-only** message mapping; **no** operator chat, **no** migration, **no** handoff push notifications); **`grp_private`** unchanged; **`tour_*`** / legacy **`/start`** unchanged; **no** automatic assignment from **Telegram `grp_*`** entry; **`TELEGRAM_BOT_USERNAME`** required for full group gating. **Forward:** **Phase 7 / Step 18** — **narrow private escalation CTA refinement for active/in-work `grp_followup` states** (see **Next Safe Step**). Phase 6 closure: **`docs/PHASE_6_REVIEW.md`**. **Not** old **Phase 5 / Step 4–5** checkpoints; **not** admin payment mutation or **Phase 6 / Step 31** unless scoped.
...
Default forward path is **Phase 7 / Step 18** — **narrow private escalation CTA refinement for active/in-work `grp_followup` states** — do not use legacy Phase 5 “next step” prompts for new implementation slices.
