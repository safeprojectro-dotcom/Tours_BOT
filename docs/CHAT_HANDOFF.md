...
**Recommended next implementation block:** ~~`C2B11A_ADMIN_OPS_CONVERSION_STATUS_PANEL`~~ **done** — see **Slice C2B11A** below. ~~**B10.6A** Bot-as-router copy for **`/start supoffer_<id>`**~~ **done** — see **Slice B10.6A** below; handoff [`docs/HANDOFF_B10_6A_BOT_AS_ROUTER_SUPPLIER_OFFER_START_TO_NEXT_STEP.md`](HANDOFF_B10_6A_BOT_AS_ROUTER_SUPPLIER_OFFER_START_TO_NEXT_STEP.md). **Next likely step:** **Production/OPS Mode A/B smoke** — [`docs/PRODUCTION_E2E_SUPPLIER_OFFER_WALKTHROUGH.md`](PRODUCTION_E2E_SUPPLIER_OFFER_WALKTHROUGH.md) (operator-owned; dry-run Mode **A** / live Mode **B**). Earlier closeout pointer: [`docs/NEXT_BLOCK_RECOMMENDATION_AFTER_CONVERSION_CLOSEOUT.md`](NEXT_BLOCK_RECOMMENDATION_AFTER_CONVERSION_CLOSEOUT.md). C2B11A handoff: [`docs/HANDOFF_ADMIN_OPS_CONVERSION_STATUS_PANEL_C2B11A_TO_NEXT_STEP.md`](HANDOFF_ADMIN_OPS_CONVERSION_STATUS_PANEL_C2B11A_TO_NEXT_STEP.md).
...
**B15B–B15F (closed):** Safe **Admin Publishing Console** foundation — **[`docs/B15_PUBLISHING_CONSOLE_FOUNDATION_CLOSURE_CHECKPOINT.md`](B15_PUBLISHING_CONSOLE_FOUNDATION_CLOSURE_CHECKPOINT.md)**. **`GET /admin/publishing-console`** is **read-only** and exposes readiness, blockers, **B15C** exact CTA safety, **`actions[]`** affordances (metadata only), source/template/channel/media metadata, future-disabled hints, **`publish_readiness`** (B15H/B15I), **`console_preview`** (**B15F2/B15F3**), **`template_library`** (**B15K**), **`preview_payload`** (**B15L** — structured showcase-oriented fields + warnings/blockers; **no** Telegram/publish/scheduler/prepare-chain execution/Layer A mutation/Mini App routing). **B15M:** read-only **`GET /admin/publishing-console/supplier-offers/{offer_id}`** bundles the same nested objects + summaries for one offer (**404** if missing; **not** list-queue-filtered). **B15P:** additive read-only presentation hints — per-list-row **`ui_card`**, supplier-offer detail **`ui_sections`** — **[`docs/HANDOFF_B15P_ADMIN_UI_READ_ONLY_ALIGNMENT.md`](HANDOFF_B15P_ADMIN_UI_READ_ONLY_ALIGNMENT.md)** (**no** frontend app, **no** new side effects). **No** action execution on the console read **GET** routes (except **B15E2** prepare-chain is a separate **POST**), Telegram send/retry, scheduler, template editor, or channel selector on these routes. **Next options** require an **explicit design gate** — **[`docs/HANDOFF_B15_CLOSE_PUBLISHING_CONSOLE_FOUNDATION_CHECKPOINT_TO_NEXT_STEP.md`](HANDOFF_B15_CLOSE_PUBLISHING_CONSOLE_FOUNDATION_CHECKPOINT_TO_NEXT_STEP.md)**.

**B17 (design gate):** **Channel / Template Editor** product/technical boundaries — **[`docs/B17_CHANNEL_TEMPLATE_EDITOR_DESIGN_GATE.md`](B17_CHANNEL_TEMPLATE_EDITOR_DESIGN_GATE.md)** (**B17** markdown is **design-first**; **B17A/B17B** editor **GET** is shipped in code). **B17A (done):** read-only **`GET /admin/publishing-console/supplier-offers/{offer_id}/editor`** — section DTOs + **`future_actions`** + **`source_snapshot`** (**no** Telegram I/O, **no** publish, **no** **`prepare_conversion_chain`** from this **GET**). **B17B (done):** same **`GET`** — additive **`channel_selection`** and **`template_selection`** **response** metadata for a future admin UI; **metadata is not persisted** (no DB writes, no POST/PATCH). **Handoffs:** **[`docs/HANDOFF_B17A_READ_ONLY_EDITOR_DETAIL_VIEW.md`](HANDOFF_B17A_READ_ONLY_EDITOR_DETAIL_VIEW.md)**, **[`docs/HANDOFF_B17B_CHANNEL_TEMPLATE_SELECTION_METADATA.md`](HANDOFF_B17B_CHANNEL_TEMPLATE_SELECTION_METADATA.md)**. **B17Z (done / docs closure):** **B17** read-only editor foundation formally closed — **[`docs/B17Z_EDITOR_READ_ONLY_FOUNDATION_CLOSURE.md`](B17Z_EDITOR_READ_ONLY_FOUNDATION_CLOSURE.md)** (**no** runtime change in B17Z). **Next priority clusters:** **M1** — Marketing Identity & Deep Link Capture design gate; **O1** — Order / Ticket QR & Boarding Validation design gate when secure operational QR is priority. **Editor continuation** otherwise: **B17C** draft-copy **design-only** — **not** publish automation or Telegram send without **explicit** go/no-go (**B17E/F**).

**P0 (done / docs roadmap):** Operational Automation Roadmap checkpoint — **[`docs/OPERATIONAL_AUTOMATION_ROADMAP.md`](OPERATIONAL_AUTOMATION_ROADMAP.md)**. It does **not** replace **`docs/IMPLEMENTATION_PLAN.md`**; it sits above the MVP plan, supplier marketplace plan, and B15/B17 publishing/editor checkpoints. It records next priority lines: **A1** Admin Automation Cockpit, **A2/A3** supplier validation + clarification, **A4/A5** AI packaging + fact-lock marketing review, **A6** controlled catalog/conversion prep, **S1** supplier departure/passenger counts, **M1** marketing identity / deep-link capture, **O1** secure order/ticket/boarding QR, and **AI1/AI2/AI3** role separation. Execution rule: work in meaningful functional blocks, but split into narrow steps for dangerous areas (Layer A, payments, inventory, migrations, public sends, scheduler, QR security, manifests, consent, auth, B11, external providers, mutating AI tools). **P0 is docs-only**: no app/tests/migrations/runtime changes.

**S1A (done):** Read-only **departure passenger counts** — aggregates from Layer A **order** + **tour** state only (**`AdminDeparturePassengerCountsService`**, **`app/services/admin_departure_passenger_counts_service.py`**); read schemas **`AdminDeparturePassengerCountsRead`**, **`AdminDeparturePassengerCountsListRead`** (**`app/schemas/admin_departure_passenger_counts.py`**). Admin **`GET`** routes: **`/admin/tours/{tour_id}/departure-passenger-counts`**, **`/admin/supplier-offers/{offer_id}/departure-passenger-counts`**, **`/admin/suppliers/{supplier_id}/departure-passenger-counts`**. **No** supplier notifications **;** **no** Telegram channel publish **;** **no** scheduler/worker behavior **;** **no** passenger manifest **;** **no** personal customer data **;** **no** Layer A mutation **;** **no** payment/reconciliation changes **;** **no** seat inventory mutation **.** Verified: **`python -m compileall app tests`** OK **`;`** **`python -m pytest tests/unit/test_admin_departure_passenger_counts.py -q`** — **9** passed **.** Handoff: **[`docs/HANDOFF_S1A_READ_ONLY_SUPPLIER_DEPARTURE_PASSENGER_COUNTS.md`](HANDOFF_S1A_READ_ONLY_SUPPLIER_DEPARTURE_PASSENGER_COUNTS.md)**.

**S1B (done):** Supplier **Telegram contact mapping** — read-only resolver for future supplier DMs **`;`** **no** notification send **`;`** uses **`Supplier.primary_telegram_user_id`** **`;`** **`AdminSupplierTelegramContactResolutionRead`** (**`app/schemas/admin_supplier_telegram_contact_resolution.py`**)**;** **`AdminSupplierTelegramContactResolutionService`** (**`app/services/admin_supplier_telegram_contact_resolution_service.py`**)**.** Admin **`GET`** **`/admin/suppliers/{supplier_id}/supplier-telegram-contact-resolution`**, **`/admin/supplier-offers/{offer_id}/supplier-telegram-contact-resolution`**, **`/admin/tours/{tour_id}/supplier-telegram-contact-resolution`**, **`/admin/orders/{order_id}/supplier-telegram-contact-resolution`**. Resolution **`:`** supplier reads **`primary_telegram_user_id`** **`;`** offer resolves owning supplier **`;`** tour **`/`** order **`→`** distinct **`supplier_id`** from active **`supplier_offer_execution_links`** and **`supplier_offer_tour_bridges`** **`;`** missing link **`→`** **`missing_relationship`** **`;`** multiple suppliers **`→`** **`ambiguous_suppliers`** with no chat id **`;`** inactive supplier **`→`** **`s1b_supplier_not_active`**. Test fixture-only **`:`** **`tests/unit/test_supplier_offer_workspace_y23.py`** **`test_published_linked_offer_exposes_booking_aggregates_only`** aligned with **`approved_for_publish`** packaging **`+`** active bridge matching tour **(** execution-link gate unchanged**)** **.** **No** supplier notification send **`;`** **no** channel publish **`;`** **no** scheduler **`/`** worker **`;`** **no** manifest **`;`** **no** customer PII **`;`** **no** Layer A **`/`** payment **`/`** seat mutation **`;`** **no** B11 routing change **.** Verified **`:`** **`pytest`** **`tests/unit/test_supplier_offer_workspace_y23.py::SupplierOfferWorkspaceY23Tests::test_published_linked_offer_exposes_booking_aggregates_only`** — **1** passed **`;`** **`pytest tests/unit/test_admin_supplier_telegram_contact_resolution.py`** — **10** passed **`;`** **`pytest tests -k "supplier_telegram_contact or telegram_contact or supplier"`** — **445** passed **`;`** **`python -m compileall app tests`** OK **.** Handoff **`:`** **[`docs/HANDOFF_S1B_SUPPLIER_TELEGRAM_CONTACT_MAPPING.md`](HANDOFF_S1B_SUPPLIER_TELEGRAM_CONTACT_MAPPING.md)** **.** **Recommended next:** **S1C — Supplier Notification After Channel Publish / Order** only with explicit **outbox **`/`** audit **`/`** idempotency** or a separately approved safe-send design **.**

**S1C-1 (done):** Supplier notification **outbox **`/`** payload foundation** — table **`supplier_notification_outbox`** **`;`** Alembic **`20260617_34_supplier_notification_outbox_s1c1.py`** **`;`** ORM **`SupplierNotificationOutbox`** (**`app/models/supplier_notification_outbox.py`**) **`;`** schema **`SupplierNotificationOutboxRead`** (**`app/schemas/supplier_notification_outbox.py`**) **`;`** **`SupplierNotificationOutboxRepository`** (**`app/repositories/supplier_notification_outbox.py`**) **`;`** **`SupplierNotificationOutboxService`** (**`app/services/supplier_notification_outbox_service.py`**) **`.`** Event types **`supplier_offer_published`** **`/`** **`supplier_order_created`** **`;`** uses **S1B** **`AdminSupplierTelegramContactResolutionService`** **`;`** deterministic **idempotency** keys **`;`** **`pending_dispatch`** **`/`** **`skipped_no_target`** **`;`** safe payloads **without** customer **PII** **`.`** **No** Telegram send **`;`** **no** Telegram channel publish **`;`** **no** scheduler **`/`** worker **`;`** **no** publish **`/`** order automatic hooks **`;`** **no** passenger manifest **`;`** **no** customer personal data **`;`** **no** Layer A mutation **`;`** **no** payment **`/`** reconciliation changes **`;`** **no** seat inventory mutation **`;`** **no** B11 routing change **`.`** Verified **`:`** **`python -m alembic upgrade head`** OK **`;`** **`python -m compileall app tests`** OK **`;`** **`python -m pytest tests/unit/test_supplier_notification_outbox_s1c1.py -q`** — **7** passed **`;`** **`python -m pytest tests -k "supplier_notification or supplier_telegram_contact or supplier" -q`** — **452** passed **`.`** Handoff **`:`** **[`docs/HANDOFF_S1C1_SUPPLIER_NOTIFICATION_OUTBOX_FOUNDATION.md`](HANDOFF_S1C1_SUPPLIER_NOTIFICATION_OUTBOX_FOUNDATION.md)** **.**

**S1C-2 (done):** Controlled **supplier Telegram DM** **`—`** migration **`20260617_35_supplier_notification_outbox_s1c2_delivery.py`** **`/`** dispatch **`delivery_in_progress`**, **`delivered`**, **`send_failed`** **`+`** audit **`telegram_message_id`**, **`last_delivery_error`**, **`delivered_at`** **`;`** **`SupplierNotificationOutboxDeliveryService`** (**`app/services/supplier_notification_outbox_delivery_service.py`**) **`;`** repository claim **`/`** finalize helpers **`;`** **`SupplierNotificationOutboxDeliveryResultRead`** **`;`** **`POST /admin/supplier-notification-outbox/{outbox_id}/deliver`** **`;`** same **Aiogram private** pattern as **`NotificationDeliveryService`** **`/`** **`TELEGRAM_BOT_TOKEN`** for real sends **`;`** Telegram text **`=`** **`title`** **`\n\n`** **`message`** **`;`** outbox-only **`;`** **`pending_dispatch`** **`→`** atomic claim **`;`** **`delivered`** · idempotent (**no** re-send**) **`;`** **`Supplier.primary_telegram_user_id`** drift gate **`;`** **no** publish **`/`** order hooks **`;`** **no** scheduler **`;`** **no** channel publish **`/`** manifest **`;`** **no** Layer A **`/`** payment mutation **`/`** customer PII **`.`** Verified **`:`** **`python -m alembic heads`** **`/`** **`current`** **`=`** **`20260617_35`** **`;`** **`python -m compileall app tests`** OK **`;`** **`pytest`** **`tests/unit/test_supplier_notification_outbox_s1c1.py tests/unit/test_supplier_notification_outbox_s1c2.py -q`** — **13** passed **`;`** **`pytest`** **`-k "supplier_notification_telegram_delivery or supplier_notification_outbox or supplier"`** — **458** passed **`;`** **`2`** pytest warnings (**pre-existing**) **`.`** Handoff **`:`** **[`docs/HANDOFF_S1C2_SUPPLIER_NOTIFICATION_TELEGRAM_DELIVERY_FROM_OUTBOX.md`](HANDOFF_S1C2_SUPPLIER_NOTIFICATION_TELEGRAM_DELIVERY_FROM_OUTBOX.md)** **.**

**S1C-3 (done):** **Supplier notification enqueue after successful showcase/channel publish** **`—`** integration **`SupplierOfferModerationService`** channel showcase publish path **`;`** after **`mark_persisted`** / successful public publish **`session.flush()`** then **`SupplierNotificationOutboxService().enqueue_supplier_offer_published(...)`** **`;`** **`actor_surface="s1c3_after_showcase_channel_publish"`** **`;`** idempotent via **S1C-1** keys **`;`** enqueue failure logged **`s1c3_supplier_notification_outbox_enqueue_failed_after_showcase_publish`** and **not** re-raised (**channel publish success not rolled back**) **`;`** **no** direct supplier Telegram send **`;`** **no** **`POST …/deliver`** from publish flow **`;`** **no** scheduler **`/`** worker **`;`** **no** order hook **`;`** **no** extra channel publish **`;`** **no** passenger manifest **`;`** **no** customer PII **`;`** **no** Layer A mutation **`;`** **no** payment **`/`** reconciliation **`/`** seat inventory mutation **`;`** **no** B11 routing change **`.`** Regression **`:`** **`tests/unit/test_telegram_admin_moderation_y281.py`** expects **`"booking link"`** button label (**production UI unchanged**) **`.`** Verified **`:`** **`pytest`** **`tests/unit/test_supplier_offer_track3_moderation.py`** (S1C-3 cases) — **2** passed **`;`** **`pytest`** **`tests/unit/test_telegram_admin_moderation_y281.py::TelegramAdminModerationY281Tests::test_publish_action_available_only_in_approved_unpublished_view`** — **1** passed **`;`** **`pytest`** **`-k "supplier_notification_outbox or supplier_offer_publish or publish or supplier"`** — **485** passed **`;`** **`python -m compileall app tests`** OK **`.`** Handoff **`:`** **[`docs/HANDOFF_S1C3_WIRE_SUPPLIER_NOTIFICATION_AFTER_CHANNEL_PUBLISH.md`](HANDOFF_S1C3_WIRE_SUPPLIER_NOTIFICATION_AFTER_CHANNEL_PUBLISH.md)** **.**

**S1C-4 (done):** **Supplier notification enqueue after successful Layer A temporary reservation / order row** **`—`** **`TemporaryReservationService.create_temporary_reservation`** (**`app/services/reservation_creation.py`**) **`;`** after order persist **`SupplierNotificationOutboxService().enqueue_supplier_order_created(...)`** **`;`** **`actor_surface="s1c4_after_layer_a_temporary_reservation"`** **`;`** enqueue failure logged **`s1c4_supplier_notification_outbox_enqueue_failed_after_order_created`** and **not** re-raised (**reservation/order row not rolled back**) **`;`** **S1C-1** idempotency **`s1c1:supplier_order_created:order:{order_id}`** **`/`** **`skipped_no_target`** when no supplier link **`/`** contact **`;`** covers Mini App **`/`** RFQ bridge via **`MiniAppBookingService`** **`/`** private bot holds using the same reservation service **`;`** **no** direct supplier Telegram send **`;`** **no** **`POST …/deliver`** from reservation path **`;`** **no** scheduler **`/`** worker **`;`** **no** channel publish **`;`** **no** passenger manifest **`;`** **no** customer PII in outbox payload **`/`** message **`;`** **no** payment-confirmation hook **`;`** **no** payment **`/`** reconciliation changes **`;`** **no** seat-inventory semantics change **`;`** **no** B11 routing change **`.`** Verified **`:`** **`pytest`** **`tests/unit/test_services_reservation.py`** **`tests/unit/test_services_mini_app_booking.py`** **`tests/unit/test_supplier_notification_outbox_s1c1.py`** — **26** passed **`;`** **`pytest`** **`-k "supplier_notification_outbox or temporary_reservation or reservation or order or supplier"`** — **612** passed **`;`** **`python -m compileall app tests`** OK **`.`** Handoff **`:`** **[`docs/HANDOFF_S1C4_WIRE_SUPPLIER_NOTIFICATION_AFTER_CUSTOMER_ORDER.md`](HANDOFF_S1C4_WIRE_SUPPLIER_NOTIFICATION_AFTER_CUSTOMER_ORDER.md)** **.** **Recommended next **`:`** physical demo playbook (** S1C-3 **`+`** S1C-4 enqueue **`+`** S1C-2 manual deliver**)** **`;`** or **S1D-1** **`/`** **S1D-2** operational sales push **(** preview **`+`** channel publish**)** **`.**

**S1D-1 (done):** **Operational sales push eligibility & preview** **`—`** **`AdminOperationalSalesPushPreviewService`** (**`app/services/admin_operational_sales_push_preview_service.py`**) **`/`** **`AdminOperationalSalesPushPreviewRead`** (**`app/schemas/admin_operational_sales_push_preview.py`**) **`;`** **`GET /admin/tours/{tour_id}/operational-sales-push-preview`** **`;`** push shapes **`:`** **`predeparture`** (** departure within **`PREDEPARTURE_SALES_PUSH_DAYS_BEFORE`**, default **2** days **`;`** seats remain **) **`/`** **`low_availability`** (** **`1`** **`≤`** **`seats_available`** **`≤`** **`LOW_AVAILABILITY_SEATS_THRESHOLD`**, default **2**) **`/`** **`combined`** (** both **) **`/`** **`not_eligible`** (** no trigger **or** gate failure **) **`;`** gates **`:`** **`OPEN_FOR_SALE`**, future departure, sales deadline not passed, **`seats_available`** **`>`** **0**, **S1A** inventory trusted (**no** **`s1a_inventory_vs_active_order_seats_mismatch`**) **`;`** **no** channel publish **`/`** Telegram send **`/`** scheduler **`/`** worker **`/`** supplier notification **`/`** customer PII **`/`** passenger manifest **`/`** Layer A mutation **`/`** payment **`/`** reconciliation **`/`** seat inventory mutation **`/`** B11 routing change **`.`** Verified **`:`** **`pytest`** **`tests/unit/test_admin_operational_sales_push_preview.py`** — **5** passed **`;`** **`pytest`** **`-k "operational_sales_push or predeparture_sales_push or last_seats or departure_passenger_counts or supplier"`** — **474** passed **`;`** **`python -m compileall app tests`** OK **`.`** Handoff **`:`** **[`docs/HANDOFF_S1D1_OPERATIONAL_SALES_PUSH_ELIGIBILITY_AND_PREVIEW.md`](HANDOFF_S1D1_OPERATIONAL_SALES_PUSH_ELIGIBILITY_AND_PREVIEW.md)** **.**

**S1D-2 (done):** **Admin-gated operational sales push channel publish** **`—`** **`AdminOperationalSalesPushPublishService`** (**`app/services/admin_operational_sales_push_publish_service.py`**) **`;`** **`AdminOperationalSalesPushPublishBody`** **`/`** **`AdminOperationalSalesPushChannelPublishResultRead`** (**`app/schemas/admin_operational_sales_push_publish.py`**) **`;`** **`POST /admin/tours/{tour_id}/operational-sales-push/publish`** **`;`** **`send_channel_plain_message`** (**`app/services/telegram_showcase_client.py`**) **`;`** **`channel_plain_text_for_operational_sales_push_read`** (**preview service**) **`;`** body **`confirm: true`** required **`;`** re-runs **S1D-1** at publish **`;`** sends only when **S1D-1** eligible **`;`** system channel text only **`;`** **`TELEGRAM_OFFER_SHOWCASE_CHANNEL_ID`** **`/`** **`TELEGRAM_BOT_TOKEN`** **`;`** structured logs **`s1d2_operational_sales_push_published`** **`/`** **`s1d2_operational_sales_push_channel_send_failed`** **`;`** push types inherit **S1D-1** (**`predeparture`** **`/`** **`low_availability`** **`/`** **`combined`**) **`;`** **no** scheduler **`/`** worker **`/`** automatic publish **`/`** supplier notification **`/`** passenger manifest **`/`** customer PII **`/`** Layer A mutation **`/`** payment **`/`** reconciliation **`/`** seat inventory mutation **`/`** B11 routing change **`;`** **no** persisted audit table **(** this slice**)** **`.`** Verified **`:`** **`pytest`** **`tests/unit/test_admin_operational_sales_push_channel_publish.py`** **`tests/unit/test_admin_operational_sales_push_preview.py`** — **10** passed **`;`** **`pytest`** **`-k "operational_sales_push or predeparture_sales_push or supplier"`** — **469** passed **`;`** **`python -m compileall app tests`** OK **`.`** Handoff **`:`** **[`docs/HANDOFF_S1D2_ADMIN_GATED_OPERATIONAL_SALES_PUSH_CHANNEL_PUBLISH.md`](HANDOFF_S1D2_ADMIN_GATED_OPERATIONAL_SALES_PUSH_CHANNEL_PUBLISH.md)** **.** **Recommended next **`:`** physical Telegram demo **`—`** **S1D-1** preview **`→`** **S1D-2** publish **`+`** **S1C-3** **`/`** **S1C-4** outbox **`+`** **S1C-2** deliver **`;`** scheduler **`/`** automation only after separate design gate **`.**

**DEMO-1 (docs / manual smoke):** Physical Telegram demo playbook **`—`** **[`docs/DEMO1_PHYSICAL_TELEGRAM_DEMO_SMOKE_PLAYBOOK.md`](DEMO1_PHYSICAL_TELEGRAM_DEMO_SMOKE_PLAYBOOK.md)** **`—`** chain **`:`** showcase publish **`→`** Mini App reservation **`/`** order **`→`** **`supplier_notification_outbox`** enqueue **`→`** **S1C-2** deliver **`→`** **S1D-1** preview **`→`** **S1D-2** channel publish **`;`** **no** runtime code in doc **`.**

**O1-DG (done / docs design gate):** Production role, identity, access, supplier **`/`** driver ops **`&`** boarding **`—`** **[`docs/O1_PRODUCTION_ROLE_IDENTITY_ACCESS_BOARDING_DESIGN_GATE.md`](O1_PRODUCTION_ROLE_IDENTITY_ACCESS_BOARDING_DESIGN_GATE.md)** **`;`** under **`P0`** **`→`** **O1** — Order **`/`** Ticket QR **`&`** Boarding Validation **`;`** **not** a new roadmap **`;`** does **not** replace **`P0`** or **`IMPLEMENTATION_PLAN.md`** **`;`** customer **`/`** supplier admin **`/`** dispatcher **`/`** driver **`/`** admin **`/`** super admin **`/`** service **`&`** vehicle-device procedures **`;`** org **`/`** membership **`/`** group-access model **`;`** supplier surface **`/`** menu model **`;`** driver boarding mode **`;`** vehicle **`/`** device model **`;`** departure assignment **`;`** ticket **`/`** QR future model **`;`** **M1** marketing QR vs **O1** secure ticket **`/`** boarding QR boundary **`;`** boarding validation **`;`** visibility matrix **`;`** trip closeout **`/`** report **`;`** privacy **`/`** audit boundaries **`;`** **docs-only`** **`—`** **no** code **`/`** migrations **`/`** endpoints **`/`** QR generation **`/`** QR scanning **`/`** manifest exposure **`/`** driver UI **`/`** supplier portal change **`/`** Layer A mutation **`.`** Handoff **`:`** **[`docs/HANDOFF_O1_DG_PRODUCTION_ROLE_IDENTITY_ACCESS_BOARDING_DESIGN_GATE.md`](HANDOFF_O1_DG_PRODUCTION_ROLE_IDENTITY_ACCESS_BOARDING_DESIGN_GATE.md)** **.**

**O1A (done / docs schema review):** Production IAM baseline inventory **`—`** **[`docs/O1A_PRODUCTION_IAM_BASELINE_SCHEMA_REVIEW.md`](O1A_PRODUCTION_IAM_BASELINE_SCHEMA_REVIEW.md)** **`;`** **`P0`** **`→`** **O1** (**after** **O1-DG**)** **`;`** **docs-only`** **`—`** **no** IAM **`/`** migrations **`/`** endpoints **`;`** **`users`** **`/`** **`telegram_user_id`** **`;`** bot **`User`** resolution **`;`** Mini App **`telegram_user_id`** path **`&`** **init_data** gap under **`app/`** **`;`** **`suppliers`** **`/`** **`supplier_api_credentials`** **`/`** **`Supplier.primary_telegram_user_id`** **`;`** supplier-admin bearer **`+`** SHA-256 **`token_hash`** **`;`** **`ADMIN_API_TOKEN`** **`;`** **`TELEGRAM_ADMIN_ALLOWLIST_USER_IDS`** **`;`** **`OPS_QUEUE_TOKEN`** **`;`** order **`/`** user ownership refs **`;`** gaps **`:`** **no** production RBAC **`/`** admin-user rows **`/`** driver **`/`** vehicle **`/`** device **`/`** departure assignment **`/`** access audit **`/`** O1 ticket·QR·boarding schema **`;`** **do** **not** duplicate **`:`** second customer **`/`** supplier table **`;`** **no** Telegram **group** membership as auth **`;`** **no** **`ADMIN_API_TOKEN`** as final RBAC **`;`** **no** **M1** **`/`** **O1** QR key mix **`.`** Handoff **`:`** **[`docs/HANDOFF_O1A_PRODUCTION_IAM_BASELINE_SCHEMA_REVIEW.md`](HANDOFF_O1A_PRODUCTION_IAM_BASELINE_SCHEMA_REVIEW.md)** **.** **Recommended next **`:`** **`O1A-1`** **`—`** IAM final entity list **`/`** additive schema design **`;`** before migration **`/`** implementation **`.**

**A1 (done / docs design gate):** Admin Automation Cockpit & Controlled Operations Design Gate — **[`docs/A1_ADMIN_AUTOMATION_COCKPIT_DESIGN_GATE.md`](A1_ADMIN_AUTOMATION_COCKPIT_DESIGN_GATE.md)**. Defines cockpit queues, generic card model, action taxonomy, next-best-action rules, fact-lock, AI role boundaries, supplier clarification, marketing review, catalog/conversion, departure ops, customer handoff, RFQ handling, safety boundaries, and manual UAT vision. **A1-Block 1 (done):** **`GET /admin/automation-cockpit`** — **[`docs/HANDOFF_A1_BLOCK1_COCKPIT_READ_ONLY_FOUNDATION.md`](HANDOFF_A1_BLOCK1_COCKPIT_READ_ONLY_FOUNDATION.md)** — core operational queues. **A1-Block 2 (done):** same **`GET`** — additive **marketing_review**, **publishing_queue**, **catalog_conversion** lanes + **`commercial_context`** / fact-lock copy — **[`docs/HANDOFF_A1_BLOCK2_COMMERCIAL_MARKETING_CONVERSION_QUEUES.md`](HANDOFF_A1_BLOCK2_COMMERCIAL_MARKETING_CONVERSION_QUEUES.md)**; still **no** writes, **no** Telegram/send, **no** scheduler, **no** prepare-chain execution from this GET. **A1V (done):** allowlisted-admin **Telegram** cockpit buttons / callbacks over the same read model — **[`docs/HANDOFF_A1V_VISIBLE_ADMIN_BUTTON_SURFACE_OVER_COCKPIT.md`](HANDOFF_A1V_VISIBLE_ADMIN_BUTTON_SURFACE_OVER_COCKPIT.md)** (**read-only**; **no** channel publish, scheduler, or mutations). **A1V2 (done):** Telegram cockpit **UX polish** (demo-ready labels, compact summary safety + **`ac:s`** detail screen, human queue rows, offer/tour open buttons) — **[`docs/HANDOFF_A1V2_COCKPIT_TELEGRAM_UX_POLISH.md`](HANDOFF_A1V2_COCKPIT_TELEGRAM_UX_POLISH.md)**. **A1V3 (done):** **queue list** cards standardized (one format for all lanes; localized status + next-step codes; RO fallback for unknown actions) — **[`docs/HANDOFF_A1V3_COCKPIT_QUEUE_LIST_STANDARDIZATION.md`](HANDOFF_A1V3_COCKPIT_QUEUE_LIST_STANDARDIZATION.md)**. **A1V4 (done):** **card detail** screens standardized (human sources + safety; no raw path keys / `read_only=` dumps; RO risk phrase substitutions where matched) — **[`docs/HANDOFF_A1V4_COCKPIT_CARD_DETAIL_STANDARDIZATION.md`](HANDOFF_A1V4_COCKPIT_CARD_DETAIL_STANDARDIZATION.md)**. **Next recommended block:** **A1-Block 3 — Operations / Handoff / RFQ Queues** (see A1 §17). A1A-A1I remain optional planning labels only, not the required execution order.

**B16 Step 1 (done):** Read-only **`GET /admin/ops-dashboard`** — **[`docs/B16_ADMIN_OPS_VISIBILITY_READ_DASHBOARD.md`](B16_ADMIN_OPS_VISIBILITY_READ_DASHBOARD.md)** — **`summary`**, **`attention_items`**, **`recent_orders`**, **`upcoming_tours`**, **`recent_publications`**, **`conversion_links`**, **`generated_at`**, **`audit_hint`**; existing admin token auth. **No** mutation, **no** Telegram, **no** Layer A changes, **no** Mini App routing changes. **Handoff:** **[`docs/HANDOFF_B16_ADMIN_OPS_VISIBILITY_READ_DASHBOARD_TO_NEXT_STEP.md`](HANDOFF_B16_ADMIN_OPS_VISIBILITY_READ_DASHBOARD_TO_NEXT_STEP.md)**.

**B16B (done):** **`GET /admin/ops-dashboard`** read-only **filters** / **limits** / **time-window**: **`days_ahead`**, **`recent_days`**, **`orders_limit`**, **`tours_limit`**, **`publications_limit`**, **`conversion_links_limit`**, **`attention_limit`**, **`include_sections`**; response **`filters`** metadata; **excluded** sections return **empty** lists; **invalid** **`include_sections`** → **422**; **no** order/payment/reservation mutation, **no** Telegram send/publish/retry, **no** Layer A / Mini App routing changes.

**B16C (done):** **`/admin/ops-dashboard`** exposes **consistent** **admin** **navigation** **paths** for **`recent_orders`**, **`upcoming_tours`**, **`recent_publications`**, **`conversion_links`**, and **`attention_items`** — **`/admin/orders/{id}`**, **`/admin/tours/{id}`**, **`/admin/supplier-offers/{id}/review-package`** where applicable **(** **`conversion_links`** **also** **`supplier_offer_admin_path`** **/** **`tour_admin_path`** **)** **;** endpoint **read-only** **;** **no** order/payment/reservation mutation, **no** Telegram send/publish/retry, **no** Layer A / Mini App routing changes **(** **`app/api/routes/admin.py`** **unchanged** **)** **.** **Tests:** **`tests/unit/test_admin_ops_dashboard.py`** **11** **passed** **;** with **`tests/unit/test_admin_publishing_console.py`** **19** **passed**.

**B16D1 done:** added read-only **`GET /admin/supplier-offers/{offer_id}/prepare-conversion-chain/plan`**. It previews future **`prepare_conversion_chain`** steps (create tour bridge, activate catalog, create execution link), blockers, warnings, recommended next action, and explicit **`will_not_do`** boundaries. Endpoint is read-only; no bridge/catalog/link mutation, no Telegram publish/send/retry, no order/payment/reservation/seat mutation, no Layer A or Mini App routing changes. Regression fixed by restoring **`AdminOpsDashboardService`** import. Tests: plan + review package 12 passed; publishing console + ops dashboard 19 passed.

**B16D1.1 done:** admin read models now expose **`prepare_conversion_chain_plan_path`** for supplier-offer-related rows. Review package, prepare-conversion-chain plan response, publishing console supplier-offer rows, and ops dashboard supplier-offer-related publication/conversion/attention rows can navigate to **`/admin/supplier-offers/{offer_id}/prepare-conversion-chain/plan`**. Metadata only; no bridge/catalog/link mutation, no Telegram publish/send/retry, no order/payment/reservation/seat mutation, no Layer A or Mini App routing changes. Tests: combined focused suite 31 passed.

**B16D1.2 done:** admin read models now expose lightweight prepare_conversion_chain readiness summary/status alongside the plan path. Fields: **`prepare_conversion_chain_plan_status`**, **`prepare_conversion_chain_recommended_action`**, **`prepare_conversion_chain_blockers_count`**. Status values: **`ineligible`**, **`blocked`**, **`partial`**, **`already_prepared`**. Surfaces: plan endpoint, supplier offer review package, publishing console supplier-offer rows, and ops dashboard supplier-offer-related publication/conversion/attention rows. Metadata only; no bridge/catalog/link mutation, no Telegram publish/send/retry, no order/payment/reservation/seat mutation, no Layer A or Mini App routing changes. Tests: combined focused suite 31 passed.

**B16E done:** **`/admin/ops-dashboard`** now exposes read-only **`audit_events`** with **`audit_events_limit`** and **`include_sections`** support. Current audit sources: showcase publish attempts, failed notification outbox rows, and supplier execution failed/blocked requests plus failed attempts. Metadata/read-only only; no Telegram publish/send/retry, no supplier execution mutation, no notification mutation, no order/payment/reservation/seat mutation, no Layer A or Mini App routing changes. Tests: **`test_admin_ops_dashboard`** 12 passed; focused regression 32 passed.

**B16D (design doc; execution deferred):** Guarded OPS automation and future **`prepare_conversion_chain`** mutation — **[`docs/B16D_OPS_ACTIONS_GUARDED_AUTOMATION_DESIGN.md`](B16D_OPS_ACTIONS_GUARDED_AUTOMATION_DESIGN.md)** **;** **`prepare_conversion_chain`** **execution** **=** **B16D2** **(** **explicit** **gate** **)** **.** Public **`publish_showcase_channel`** remains **separate** **/** **confirmation-gated** **.** **Handoff** **[`docs/HANDOFF_B16D_OPS_ACTIONS_GUARDED_AUTOMATION_DESIGN_TO_NEXT_STEP.md`](HANDOFF_B16D_OPS_ACTIONS_GUARDED_AUTOMATION_DESIGN_TO_NEXT_STEP.md)**.

**B16D2 (design gate only; no code):** Scoped spec for **`POST /admin/supplier-offers/{offer_id}/prepare-conversion-chain`** — internal chain (tour bridge → catalog activation when eligible → active execution link); **`idempotency_key`** **/** **`confirm`** **/** **`dry_run`** body; below Telegram publish boundary — **[`docs/B16D2_PREPARE_CONVERSION_CHAIN_EXECUTION_DESIGN_GATE.md`](B16D2_PREPARE_CONVERSION_CHAIN_EXECUTION_DESIGN_GATE.md)** **·** handoff **[`docs/HANDOFF_B16D2_PREPARE_CONVERSION_CHAIN_EXECUTION_DESIGN_GATE_TO_NEXT_STEP.md`](HANDOFF_B16D2_PREPARE_CONVERSION_CHAIN_EXECUTION_DESIGN_GATE_TO_NEXT_STEP.md)** **·** prompt **[`docs/CURSOR_PROMPT_B16D2_PREPARE_CONVERSION_CHAIN_EXECUTION_DESIGN_GATE.md`](CURSOR_PROMPT_B16D2_PREPARE_CONVERSION_CHAIN_EXECUTION_DESIGN_GATE.md)**.

**B16D2A done:** Added guarded admin action audit/idempotency persistence foundation for future **`prepare_conversion_chain`** execution. New tables **`admin_guarded_action_attempts`** and **`admin_guarded_action_steps`**, unique idempotency on **`action_code`** + **`source_entity_type`** + **`source_entity_id`** + **`idempotency_key`**, step-level audit, ORM models, repository/service helpers, Alembic **`20260609_30_admin_guarded_action_audit.py`**. **B16D2C** adds the guarded **`POST`** (below); B16D2A slice itself introduced tables/helpers only. No Telegram publish/send/retry from audit layer. Tests: **`test_admin_guarded_action_audit`** 5 passed; **Railway/prod after deploy:** **`python -m alembic upgrade head`**.

**B16D2B done:** Added service-level **`prepare_conversion_chain`** execution using guarded action audit/idempotency foundation. The service runs the internal preparation chain only through existing safe admin services: ensure tour bridge, activate catalog, ensure active execution link; writes attempt/step audit; supports **`dry_run`**, **`partial_success`**, and idempotent replay for succeeded/partial_success/failed stored attempts. Failed/blocked steps write skipped audit rows for remaining steps. No Telegram publish/send/retry, no order/payment/reservation mutation, no Layer A booking/payment changes, no Mini App routing changes. Tests: **`test_prepare_conversion_chain_execution_service`** 8 passed.

**B16D2C done:** **`POST /admin/supplier-offers/{offer_id}/prepare-conversion-chain`** — thin central-admin route over **`PrepareConversionChainExecutionService.execute`**. **`idempotency_key`** required (non-blank); live run requires **`confirm=true`**; **`dry_run`** supported; existing admin token auth; response **`AdminPrepareConversionChainExecutionResultRead`**. No publishing-console/Telegram/dashboard execution, no Layer A booking/payment mutation, no Mini App routing changes, no migration. Tests: **`test_admin_prepare_conversion_chain_api`**, **`test_prepare_conversion_chain_execution_service`**, **`test_admin_prepare_conversion_chain_plan`**, and related B16-focused suites.

**B16D2D done:** Read-only **`prepare_conversion_chain_action`** affordance (**`PrepareConversionChainActionAffordanceRead`**) on **`GET …/review-package`**, publishing-console supplier-offer rows, and ops-dashboard publication / conversion-link / supplier-offer-scoped attention rows. **`derive_prepare_conversion_chain_action_affordance`** + **`append_prepare_conversion_chain_operator_action`** (service layer); **`enabled`** when plan status is **`partial`** or **`already_prepared`**, disabled for **`ineligible`** / **`blocked`**. No execution, audit rows, or mutations from read paths; B16D2C POST remains the only mutation entry. Tests: **`test_prepare_conversion_chain_d2d_affordance`**, updated review-package / publishing-console / ops-dashboard tests. Handoff: **`docs/HANDOFF_B16D2D_PREPARE_CONVERSION_CHAIN_ACTION_AFFORDANCES.md`**.

**B16D2E (operator smoke):** Runbook + run log template — **[`docs/B16D2E_PREPARE_CONVERSION_CHAIN_PRODUCTION_SMOKE.md`](B16D2E_PREPARE_CONVERSION_CHAIN_PRODUCTION_SMOKE.md)**. Validates **`POST …/prepare-conversion-chain`** (dry_run, live, replay) and read-model checks on a deployed env **without** Telegram publish, Layer A booking mutations, or Mini App/B11 changes. **Status:** run log **pending** until an operator executes smoke and fills the doc (no in-repo commit of secrets).

**B15E2 done:** Publishing-console execution entry **`POST /admin/publishing-console/supplier-offers/{offer_id}/prepare-conversion-chain`** — same body/response as B16D2C, delegates via shared **`_handle_prepare_conversion_chain_post`** to **`PrepareConversionChainExecutionService`** with **`actor_surface="publishing_console"`**. **`idempotency_key`**, **`confirm`/`dry_run`** rules unchanged; no Telegram, no Layer A mutation, no migration. Tests **`test_admin_publishing_console_prepare_chain_action`**. Handoff **`docs/HANDOFF_B15E2_PUBLISHING_CONSOLE_PREPARE_CHAIN_ACTION_EXECUTION.md`**.

**B15G (design-only, no code):** Future **guarded auto-publish** gates, modes (disabled / suggest / queue / scheduled / not MVP full-auto), audit/idempotency, rollback, and future data/API sketches — **`docs/B15G_GUARDED_AUTO_PUBLISH_DESIGN.md`**. **`prepare_conversion_chain`** remains **internal** only; **public** Telegram showcase publish stays **explicit** until a separate go/no-go. **No** scheduler, worker, or Telegram auto-send introduced by this gate.

**B15H (done):** Read-only **publish readiness** / **suggest-only** metadata on **`GET /admin/supplier-offers/{id}/review-package`** and **supplier-offer rows** of **`GET /admin/publishing-console`** — derived in **`supplier_offer_publish_readiness`** (`derive_supplier_offer_publish_readiness`); **`can_auto_publish`** always **`false`**; **`auto_publish_mode`** **`disabled`**; **no** Telegram I/O, **no** publish attempts, **no** scheduler, **no** mutation, **no** migration. Tour-promotion console rows expose **`not_applicable`** readiness. **Handoff:** **[`docs/HANDOFF_B15H_READ_ONLY_PUBLISH_READINESS.md`](HANDOFF_B15H_READ_ONLY_PUBLISH_READINESS.md)**. **Tests:** **`tests/unit/test_supplier_offer_publish_readiness.py`**, updates to **`test_supplier_offer_review_package`**, **`test_admin_publishing_console`**.

**B15I (done):** **UX layer** on the same **`publish_readiness`** object — **`summary`**, **`badge`** (mirrors status), **`next_action_code`** / **`next_action_label`** (admin intent hints, not route execution), **`primary_blocker`**, **`warning_summary`**, **`gate_summary`** — computed in **`_compute_publish_readiness_ux`** / **`_finalize_publish_readiness`** (**read-only**). **Handoff:** **[`docs/HANDOFF_B15I_PUBLISH_READINESS_SUGGEST_ONLY_UX.md`](HANDOFF_B15I_PUBLISH_READINESS_SUGGEST_ONLY_UX.md)**.

**B15F2/B15F3 (done):** Read-only publishing console **template / preview display** metadata — **`AdminPublishingConsolePreviewRead`** is a **separate** DTO from **`AdminPublishingConsoleItemRead`**; each console row includes **`console_preview`**. **`supplier_offer_initial`** rows surface supplier-offer **showcase**-oriented preview/display fields; **`tour_promotion`** rows stay **placeholder / read-only** (no real compose). **`tests/unit/test_admin_publishing_console.py`** includes **`console_preview`** assertions. **No** Telegram I/O, publish attempts, scheduler, auto-publish, prepare-chain execution, Layer A mutation, migration, commits, or pushes were touched. **Handoff:** **[`docs/HANDOFF_B15F2F3_PUBLISHING_CONSOLE_TEMPLATE_PREVIEW_REFINEMENT.md`](HANDOFF_B15F2F3_PUBLISHING_CONSOLE_TEMPLATE_PREVIEW_REFINEMENT.md)**.

**B15K (done):** Read-only **`template_library`** on each publishing-console item — **`AdminPublishingConsoleTemplateLibraryRead`** + entry DTOs (**`PublishingConsoleTemplateLibraryFamily`**, **`PublishingConsoleTemplateLibraryEntryStatus`**, **`AdminPublishingConsoleTemplateLibraryEntryRead`**). **`supplier_offer_initial`** rows expose **`supplier_offer_showcase`** metadata; **`custom_request_cta`** appears as a variant when applicable; **`tour_promotion`** rows stay placeholder/read-only with **`future`** variants. **`console_preview`** unchanged and not replaced. **No** Telegram I/O, publish attempts, scheduler, auto-publish, **`prepare_conversion_chain`** execution, Layer A mutation, Mini App/B11 routing changes, or migration. **Handoff:** **[`docs/HANDOFF_B15K_PUBLISHING_CONSOLE_TEMPLATE_LIBRARY_PREVIEW_LAYER.md`](HANDOFF_B15K_PUBLISHING_CONSOLE_TEMPLATE_LIBRARY_PREVIEW_LAYER.md)**.

**B15L (done):** Read-only **`preview_payload`** on every publishing-console item — **`AdminPublishingConsolePreviewPayloadRead`** (**`PublishingConsolePreviewPayloadStatus`**, **`PublishingConsolePreviewPayloadSource`**). **`supplier_offer_initial`**: title/body/caption/CTA/media/channel/warnings/blockers from existing **`showcase_preview`** + review-package/offer data where available; **`publish_readiness_note`** / **`template_library_note`** link back to the same row’s **`publish_readiness`** and **`template_library`**. **`tour_promotion`**: **`source=tour_placeholder`**, placeholder/read-only. **No** Telegram I/O, publish attempts, scheduler, auto-publish, **`prepare_conversion_chain`** execution, Layer A mutation, Mini App/B11 routing changes, or migration. **Handoff:** **[`docs/HANDOFF_B15L_SUPPLIER_OFFER_SHOWCASE_PREVIEW_PAYLOAD.md`](HANDOFF_B15L_SUPPLIER_OFFER_SHOWCASE_PREVIEW_PAYLOAD.md)**.

**B15M (done):** Read-only **`GET /admin/publishing-console/supplier-offers/{offer_id}`** — **`AdminPublishingConsoleSupplierOfferDetailRead`** via **`AdminPublishingConsoleService.read_supplier_offer_detail`**. One response bundles **`publish_readiness`**, **`console_preview`**, **`template_library`**, **`preview_payload`**, **`actions`** and path metadata (**`review_package_path`**, **`prepare_conversion_chain_plan_path`**, **`publish_action_path`**, **`prepare_conversion_chain_action`**), plus **`conversion_summary`**, **`linked_tour_summary`**, **`publication_summary`**, and **`safety_summary`**. Detail can return **any** existing supplier offer (404 if missing), including ids **not** currently in the list queue filter. **No** Telegram I/O, publish attempts, scheduler, auto-publish, **`prepare_conversion_chain`** execution, Layer A mutation, Mini App/B11 routing changes, or migration. **Handoff:** **[`docs/HANDOFF_B15M_PUBLISHING_CONSOLE_SUPPLIER_OFFER_DETAIL_READ_VIEW.md`](HANDOFF_B15M_PUBLISHING_CONSOLE_SUPPLIER_OFFER_DETAIL_READ_VIEW.md)**.

**B15P (done):** **Read-only admin UI presentation metadata** on the publishing console read models — each **`GET /admin/publishing-console`** list item includes **`ui_card`** (`card_title`, `card_subtitle`, `status_badge`, `status_label`, `status_tone`, `primary_line`, `secondary_line`, primary/secondary action `label` / `code` / `enabled` / `kind` / `path`, `warning_line`, `blocker_line`, `safety_line`). Supplier-offer **`GET /admin/publishing-console/supplier-offers/{offer_id}`** includes ordered **`ui_sections`** for a future admin UI. Ready supplier-offer rows may surface **`primary_action_kind=guarded_post`** for **`prepare_conversion_chain`** — **internal guarded preparation only**, not Telegram showcase publish. **`tests/unit/test_admin_publishing_console.py`** asserts shape/backward-compat for **`ui_card`** (and detail sections). **No** admin frontend implementation, **no** Telegram I/O, **no** publish attempts, **no** scheduler, **no** auto-publish, **no** **`prepare_conversion_chain`** execution from **GET**, **no** Layer A mutation, **no** Mini App/B11 routing changes, **no** migration. **Handoff:** **[`docs/HANDOFF_B15P_ADMIN_UI_READ_ONLY_ALIGNMENT.md`](HANDOFF_B15P_ADMIN_UI_READ_ONLY_ALIGNMENT.md)**.

**B15O (done / pending commit):** **Publishing Console Foundation Closure** — formal docs checkpoint **[`docs/B15O_PUBLISHING_CONSOLE_FOUNDATION_CLOSURE.md`](docs/B15O_PUBLISHING_CONSOLE_FOUNDATION_CLOSURE.md)**. **B15 foundation is closed** as **read-model + guarded internal prepare-chain** (no public automation). **Read surfaces:** **`GET /admin/publishing-console`**, **`GET /admin/publishing-console?kind=supplier_offer_initial`**, **`GET /admin/publishing-console/supplier-offers/{offer_id}`**, **`GET /admin/publishing-console/supplier-offers/{offer_id}/editor`** (**B17A** / **B17B** read-only metadata), **`GET /admin/supplier-offers/{offer_id}/review-package`**, **`GET /admin/supplier-offers/{offer_id}/prepare-conversion-chain/plan`**. **Guarded prepare-chain POST:** **`POST /admin/supplier-offers/{offer_id}/prepare-conversion-chain`**, **`POST /admin/publishing-console/supplier-offers/{offer_id}/prepare-conversion-chain`**. **Safety:** **No** Telegram I/O, **no** scheduler, **no** auto-publish, **no** publish attempts from read surfaces, **no** Layer A mutation, **no** Mini App/B11 routing changes, **no** migration on this foundation line.

**B15C exact CTA chain — earlier checkpoint (B15C6, docs):** Exact-tour **Rezervă**, execution-link-before-publish, short-name **`startapp`**, cover guard, admin nav, copy alignment — [`docs/B15C_EXACT_CTA_CHAIN_CLOSURE_CHECKPOINT.md`](B15C_EXACT_CTA_CHAIN_CLOSURE_CHECKPOINT.md). Slice docs: **B15D** [`docs/B15D_ADMIN_PUBLISHING_CONSOLE_RICH_READ_VIEW.md`](B15D_ADMIN_PUBLISHING_CONSOLE_RICH_READ_VIEW.md), **B15E** [`docs/B15E_ADMIN_PUBLISHING_CONSOLE_ACTION_AFFORDANCES_DESIGN.md`](B15E_ADMIN_PUBLISHING_CONSOLE_ACTION_AFFORDANCES_DESIGN.md), **B15F** [`docs/B15F_PUBLISHING_CONSOLE_TEMPLATE_SOURCE_CHANNEL_READ_MODEL.md`](B15F_PUBLISHING_CONSOLE_TEMPLATE_SOURCE_CHANNEL_READ_MODEL.md). **Tests:** **`tests/unit/test_admin_publishing_console.py`** — **14 passed** (includes **B15M** detail GET, **B15P** **`ui_card`** / **`ui_sections`**, **B17A** **`/editor`**).

## Continuity Sync — Admin Offer Review & Approval Gate — Slice 1 (2026-04-27)

**Read-only endpoint:** **`GET /admin/supplier-offers/{offer_id}/review-package`** aggregates offer snapshot, packaging axis, moderation/showcase axis, showcase preview, bridge readiness, active bridge / linked Tour, catalog activation readiness, execution-link readiness, Mini App conversion preview, **`conversion_status_panel`** (C2B11A — five-layer ops visibility), **`conversion_closure`**, warnings, and `recommended_next_actions`.
...
**Admin operator workflow — Slice C1 / C1.1 / C2A (implemented):** Telegram **private admin offer detail** (moderation workspace) appends **read-only** **`operator_workflow`** text from **`SupplierOfferReviewPackageService.review_package()`** — display only (**C1.1:** compact block for mobile **;** full detail stays on **`GET …/review-package`**). **C2A:** optional inline buttons only for **`review_package_refresh`** and **`get_showcase_preview`** (read-only callbacks — **no** POST)**.** **`get_showcase_preview`** uses **`SupplierOfferModerationService.showcase_preview`**. **C2B4:** when **`cover_media_reference`** yields a Telegram-sendable photo (**`telegram_photo:{file_id}`** or HTTPS URL), the bot sends **`sendPhoto`** with the same **`caption_html`** as **`GET …/showcase-preview`** and **`POST …/publish`** (private admin chat only; local-preview notice; no channel publish); otherwise the prior text-only preview path applies **.**

**Slice C2B5 / C2B8A (implemented):** **`GET …/review-package`** exposes **`cover_media_quality_review`** plus matching strings in aggregated **`warnings`** and **`operator_workflow.warnings`** (deterministic cover/B7.1 **`media_review`** hints)**.** **C2B8A:** **`publish_showcase_channel`** **`disabled`** until **`approved_for_publish`** packaging and **`approved`** lifecycle **`and`** **`showcase_preview.can_publish_now`**, **and** while **`cover_media_quality_review`** contains configured hard codes (**photo-publication-only** warnings skipped when **`publication_mode`** is **`text_only`** **`;`** **`cover_media_missing_showcase_photo`** never blocks alone**)** **`;`** **`POST …/publish`** unchanged **.**

**Slice C2B8B (implemented):** Telegram offer detail adds **Publică / Publish** when **`operator_workflow.publish_showcase_channel.enabled`** **`;`** mandatory confirm **`+`** re-read **`review-package`** **`;`** **`SupplierOfferModerationService.publish`** **`+`** **`notify_published`** parity with HTTP **`;`** legacy one-tap **`ADMIN_OFFERS_ACTION_PUBLISH`** retired on the card (callback answers retired message)**.**

**Slice C2B10T-A (implemented):** Telegram offer detail adds **Link tour / Leagă tur** when **`operator_workflow.actions.create_tour_bridge.enabled`** **`;`** propose **`+`** confirm each **re-read** **`review-package`** **`;`** confirm calls **`SupplierOfferTourBridgeService.create_or_replay_bridge`** with **`created_by="telegram:{admin_id}"`** (same semantics as HTTP **`POST …/tour-bridge`**)**;** **no** catalog activation **,** **no** execution link **,** **no** Mini App **/** booking **/** orders **,** **no** migrations **.** Handoff: **`docs/HANDOFF_TELEGRAM_CREATE_TOUR_BRIDGE_BUTTON_C2B10T_A_TO_NEXT_STEP.md`**.

**Slice C2B10T-B (implemented):** Telegram offer detail adds **List for sale / În catalog** when **`operator_workflow.actions.activate_tour_for_catalog.enabled`** **`;`** propose **`+`** confirm each **re-read** **`review-package`** **`;`** confirm calls **`AdminTourWriteService.activate_tour_for_catalog`** with **`tour_id`** from **`linked_tour_catalog.tour_id`**, **`activated_by="telegram:{admin_id}"`** (same B10.2 semantics as HTTP **`POST …/tours/{tour_id}/activate-for-catalog`**)**;** **no** execution link **,** **no** publish **,** **no** Mini App **/** booking **/** orders **,** **no** migrations **.** Handoff: **`docs/HANDOFF_TELEGRAM_ACTIVATE_TOUR_FOR_CATALOG_BUTTON_C2B10T_B_TO_NEXT_STEP.md`**.

**Slice C2B10T-C (implemented):** Telegram offer detail adds **Booking link / Link rezervări** when **`operator_workflow.actions.create_execution_link.enabled`** **`;`** propose **`+`** confirm each **re-read** **`review-package`** **`;`** confirm calls **`SupplierOfferExecutionLinkService.link_offer_to_tour`** with **`tour_id`** from **`linked_tour_catalog.tour_id`**, **`link_note=None`** (same semantics as HTTP **`POST …/execution-link`** — create/replace active link only)**;** missing **`linked_tour_catalog`** **→** **`admin_offer_ow_action_unavailable`** **`;`** **no** B11 routing code changes **,** **no** catalog activation **,** **no** publish semantics changes **,** **no** Mini App **/** booking **/** orders **,** **no** migrations **.** **Telegram strip order** when conversion actions enabled: **Link tour → List for sale → Publish → Booking link** — **B15C:** **`can_publish_now`** / **`publish_showcase_channel`** require an active booking link first **`;`** use **`review-package`**, not left-to-right, as the source of what is blocking **.** Handoff: **`docs/HANDOFF_TELEGRAM_EXECUTION_LINK_BUTTON_C2B10T_C_TO_NEXT_STEP.md`**.
...
**Slice C2B11A (implemented):** **Admin/OPS conversion status panel** — **`GET …/review-package`** exposes read-only **`conversion_status_panel`** with five layers: **Showcase**, **Tour bridge**, **Catalog**, **Booking link**, **Customer action** (each `status`, `summary`, optional `detail`). Logic: **`app/services/supplier_offer_conversion_status_panel.py`**; Telegram compact block: **`app/bot/supplier_offer_conversion_status_panel_telegram.py`**, appended on private admin offer detail under **`operator_workflow`** when the review-package model is real (Pydantic). **Read-only only** — **no** mutations, **no** migrations, **no** Mini App, **no** booking/payment/orders, **no** B11 routing changes. Design: **[`docs/ADMIN_OPS_CONVERSION_VISIBILITY_POLISH_DESIGN.md`](ADMIN_OPS_CONVERSION_VISIBILITY_POLISH_DESIGN.md)**. **Original next pointer:** Production/OPS smoke, then B10.6 Bot-as-router — **superseded:** **B10.6A implemented** (see slice below); design gate **[`docs/B10_6_BOT_AS_ROUTER_DESIGN_GATE.md`](B10_6_BOT_AS_ROUTER_DESIGN_GATE.md)**. Handoff: **[`docs/HANDOFF_ADMIN_OPS_CONVERSION_STATUS_PANEL_C2B11A_TO_NEXT_STEP.md`](HANDOFF_ADMIN_OPS_CONVERSION_STATUS_PANEL_C2B11A_TO_NEXT_STEP.md)**. Read-only smoke log template: **[`docs/C2B11A_READ_ONLY_SMOKE_LOG.md`](C2B11A_READ_ONLY_SMOKE_LOG.md)**. Tests: **`test_supplier_offer_review_package`**, **`test_supplier_offer_conversion_status_panel`**, **`test_telegram_admin_moderation_y281`**.

**Slice B10.6A (implemented):** **Customer Bot-as-router copy** for private **`/start supoffer_<id>`** — messages follow **`nav.copy_bucket`** from **`resolve_sup_offer_start_mini_app_routing`** (**B11** URL/gate **semantics preserved**; read-only **`copy_bucket`** / **`context_tour_code`** for wording only). **`app/bot/handlers/private_entry.py`** **`_supplier_offer_start_intro()`** maps buckets to **`start_sup_offer_router_*`** keys **without** adding routing rules. Exact Mini App tour path: booking-safe copy + CTA; published/no execution link, not-in-catalog, not-visible, broken link, missing Mini App base, and generic unavailable states use safe fallbacks; **`linked_is_full_bus`** appends the full-bus note. **No** tour-bridge / catalog / execution-link / publish mutations; **no** Mini App changes; **no** booking / payment / orders; **no** migrations. Locale coverage for new keys includes **sr** / **hu** / **it** / **de** (and **ru**) to avoid missing translation keys. Tests: **`test_supplier_offer_bot_start_routing_b11.py`**, **`test_private_entry_supoffer_start_hotfix.py`**. Handoff: **[`docs/HANDOFF_B10_6A_BOT_AS_ROUTER_SUPPLIER_OFFER_START_TO_NEXT_STEP.md`](HANDOFF_B10_6A_BOT_AS_ROUTER_SUPPLIER_OFFER_START_TO_NEXT_STEP.md)**. **Next likely step:** Production/OPS **Mode A/B** smoke (same walkthrough).
...
**C2B9B (docs, done):** **Conversion chain** synced across **[`docs/SUPPLIER_OFFER_TO_TOUR_BUSINESS_PLAN.md`](SUPPLIER_OFFER_TO_TOUR_BUSINESS_PLAN.md)**, **[`docs/ADMIN_OPERATOR_WORKFLOW.md`](ADMIN_OPERATOR_WORKFLOW.md)**, **[`docs/ADMIN_SHOWCASE_PUBLISH_RUNBOOK.md`](ADMIN_SHOWCASE_PUBLISH_RUNBOOK.md)** — packaging **→** readiness **→** **`tour-bridge`** **→** **`activate-for-catalog`** **→** **execution link** **→** showcase publish (**HTTP** or **Telegram C2B8B**) **→** **B11** **`supoffer_<id>`** **→** **Layer A**. Prompt: **[`docs/CURSOR_PROMPT_SUPPLIER_OFFER_CONVERSION_DOCS_SYNC_C2B9B.md`](CURSOR_PROMPT_SUPPLIER_OFFER_CONVERSION_DOCS_SYNC_C2B9B.md)**.

**Slice C2B1 (implemented):** Telegram **private admin offer detail** adds workflow mutation **`approve_packaging_for_publish`** only when **`enabled`** in **`operator_workflow.actions`** — **mandatory confirmation**: confirm step **re-reads `GET …/review-package`**, executes **only** **`SupplierOfferPackagingReviewService.approve`** if action **still enabled** ( **`accept_warnings`** when **`needs_admin_review`** ) **`;`** **`reviewed_by = telegram:{admin_id}`** **`;`** cancel **does not** mutate **`;`** refreshed offer card after success.

**Slice C2B2 (implemented, docs sync):** Telegram adds **`generate_packaging_draft`** only when **`enabled`** **`;`** first tap — confirmation only **`;`** confirm **re-reads `GET …/review-package`** and executes **only** **`SupplierOfferPackagingService.generate_and_persist`** if action **still enabled** **`;`** **does not** auto-approve packaging (**C2B1** separate) **`;`** cancel **no** mutation **`;`** refreshed card after success/error handling **`;`** legacy **Aprobă / Respinge** unchanged **`;`** **`create_tour_bridge`** — **C2B10T-A** **`;`** **`activate_tour_for_catalog`** — **C2B10T-B** (**List for sale**) when **`enabled`** **`;`** **`create_execution_link`** — **C2B10T-C** (**Booking link**) when **`enabled`** **`;`** HTTP **/** published-offer link wizard still available **`;`** showcase **publish** — **C2B8B** **`;`** **no** batch workflow. **Possible next:** legacy moderation consolidation **.**

**Telegram Button UX Policy (accepted, docs-only):** Private admin card is **not** a developer console **`;`** labels **1–2 words**, action/outcome oriented **`;`** **no** snake_case **,** endpoints **,** enums **,** HTTP in button text **`;`** disabled **`operator_workflow`** actions **hidden** (not greyed) **`;`** safe reads — **no** confirmation **`;`** safe mutations — **confirmation + re-read `GET …/review-package`** before execute **`;`** **`publish_showcase_channel`** — confirm **`+`** **two** **`review-package`** re-reads (**C2B8B**) **`;`** **`create_tour_bridge`** — **C2B10T-A** (**Link tour / Leagă tur**) **`;`** **`activate_tour_for_catalog`** — **C2B10T-B** (**List for sale / În catalog**) **`;`** **`create_execution_link`** — **C2B10T-C** (**Booking link / Link rezervări**) **`;`** **B15C accepted chain** **:** observe/read → packaging → **Link tour** → **List for sale** → legacy moderation when needed → **Booking link** → **Publish** ( **`can_publish_now`** blocks publish until booking link exists ) → Orders **/** Requests → navigation **`;`** **Telegram strip** may show **Publish** before **Booking link** — use **`review-package`** **`;`** HTTP **/** link wizard remain for manual tour flows **.** Label direction **:** **`review_package_refresh`** → **Actualizează / Refresh** **`;`** **`get_showcase_preview`** → **Preview** **`;`** **`generate_packaging_draft`** → **Generează text** or **Pregătește** **`;`** **`approve_packaging_for_publish`** → **Aprobă text / Aprobă pachet** (not bare **Aprobă**) **`;`** legacy moderation → **Aprobă oferta**, **Respinge oferta** **`;`** **`create_tour_bridge`** → **Link tour / Leagă tur** **`;`** **`activate_tour_for_catalog`** → **List for sale / În catalog** **`;`** showcase → **Publică / Publish** **`;`** **`create_execution_link`** → **Booking link / Link rezervări** **.**

**Slice C2B3 (implemented):** Offer-detail keyboard uses that policy **`;`** **Telegram strip order** (**workflow read-only → packaging → Link tour when enabled → List for sale when enabled → legacy moderation → Publish when enabled → Booking link when enabled → Orders **`/`** Requests → navigation**) **`;`** **B15C:** **`can_publish_now`** requires **Booking link** before **Publish** succeeds — use **`review-package`**, not button order alone **`;`** RO **`/`** EN short labels **`;`** workflow mutation slices include **C2B1–C2B8B**, **C2B10T-A**, **C2B10T-B**, **C2B10T-C**, **C2B6** **`;`** alternative execution-link entry — published-offer UI **`/`** HTTP **/** wizard as before **.**
...
**Slice C2B7.2 (implemented):** After **`PUT …/cover`**, re-approve hero for card via **`POST …/media/approve-for-card`** **or** Telegram **`OK poză` / `OK photo`** when **`operator_workflow`** exposes **`approve_cover_for_card`** **`enabled`** **`;`** propose → confirm → re-read **`review-package`** **`;`** **`SupplierOfferMediaReviewService.approve_for_card`** only **`;`** **`reviewed_by = telegram:{admin_id}`** **`;`** no publish **`/`** lifecycle **`/`** cover mutation from this action **.** **`approve_cover_for_card`** stays **`disabled`** when **`media_review.status`** is **`replacement_requested`** **`/`** **`rejected_*`** **`/`** **`fallback_card_required`** **and** snapshot **`cover_media_reference`** **still equals** row **`cover_media_reference`** (C2B7.2a)**;** **enabled** after **`PUT …/cover`** when snapshot **≠** current cover until **`approve-for-card`**. Detail **`:`** **[`docs/HANDOFF_ADMIN_COVER_REAPPROVAL_AFTER_REPLACEMENT_C2B7_2_DESIGN.md`](HANDOFF_ADMIN_COVER_REAPPROVAL_AFTER_REPLACEMENT_C2B7_2_DESIGN.md)** **.**
...
**Architecture (unchanged invariants):** **B8** **only** **creates** **drafts**; **catalog** **activation** **explicit** **via** **B10.2**; **no** **auto**-activation; **no** **Telegram** **publish** **on** **generation**; **no** **order/reservation/payment** **in** **generation**; **B8** **does** **not** **create** **`SupplierOfferTourBridge`** **rows**. **Duplicate** **draft** `Tour`s **allowed** (tech **debt).** **Duplicate** **`open_for_sale`** **for** **same** **source** **offer** + **`departure_datetime`** **blocked** **for** **B8-audited** **activations** (sibling **B8** **or** **B10** **primary** **bridged** **tour** **—** see **`OPEN_QUESTIONS`**, B8.3). **Second** **vehicle** **same** **date/offer** = **future** **ops** **policy** (not code); **B7.3A** **policy** **accepted** **(B7.3** **bytes** **/** **storage** **not** **done** **);** **B10.6A** **customer** **copy** **for** **private** **`/start`** **`supoffer_*`** **implemented** **(** **B11** **resolver** **unchanged** **;** **see** **«Slice** **B10.6A»** **above** **)** **;** **B11** **routing** **(** **private** **`/start`** **`supoffer_*` →** **Mini** **App** **`/tours/{code}`** **via** **`supplier_offer_execution_links`,** **skips** **generic** **catalog** **overview** **after** **exact** **Tour** **CTA)** **—** **`OPEN_QUESTIONS`** **§24** **.**

**Next** **product** **step** **(not** **auto-approved):** **B7.3A** **policy** **(below)** **is** **accepted** **—** next **B7** **implementation** may **be** **B7.3B** **metadata** **stub** **or** **other** **B-line** **work** (**B12/B13,** **etc.** **)** per **stakeholder** **decision** **(** **B11** **+** **B10.6A** **customer** **router** **copy** **are** **in** **place** **for** **current** **scope** **).**

---

## Continuity Sync — B7.3A media policy acceptance (2026)

### Current checkpoint (B7.3 policy accepted; no implementation)

- **B7.3** **publish-safe** **media** **pipeline** **policy** **accepted** **(docs-only** **B7.3A** **).** **Raw** **supplier** **media** **is** **not** **publish-safe.** **Telegram** **`file_id`** / **`telegram_photo:{file_id}`** **is** **not** a **stable** **public** **URL.**
- **MVP** **remains** **metadata-first** **:** **`cover_media_reference`**, **`media_references`**, **`packaging_draft_json.media_review`**, **B7.2** **card** **render** **preview** **JSON** **(no** **new** **runtime** **code** **in** **B7.3A** **).**
- **Not** **implemented** **yet** **:** **`getFile`**, **byte** **download**, **durable** **object** **storage,** **real** **pixel** **rendering,** **sendPhoto**/**sendMediaGroup**, **automatic** **Telegram** **publish.**
- **`approved_for_card`** **(B7.1) ≠** **`publish_safe`** **(durable** **off-system** **asset** **when** **built** **)** **;** **`publish_safe` ≠`published`** **(channel** **/** **showcase** **is** **separate** **explicit** **admin** **action** **).** **B7.3** **does** **not** **send** **Telegram** **messages.**
- **No** **Railway** **local** **filesystem** **as** **canonical** **durable** **media** **store;** **future** **durable** **assets** **—** **object** **storage** **/** **S3-compatible** **when** **explicitly** **scoped.**
- **Mini** **App** **execution** **truth** **independent** **of** **marketing** **/** **card** **media** **pipeline;** **no** **Mini** **App** **UI** **change** **in** **B7.3A.**
- **Next** **optional** **B7** **step:** **B7.3B** **(metadata-only** **`publish_safe`** **stub** **in** **`packaging_draft_json` **)** **or** **continue** **BUSINESS** **line** **(e.g.** **B11** **/** **B12** **/** **B13** **)** if **media** **bytes** **are** **not** **priority.** **See** [`docs/OPEN_QUESTIONS_AND_TECH_DEBT.md`](OPEN_QUESTIONS_AND_TECH_DEBT.md) **(B7.3)** **and** [`docs/SUPPLIER_OFFER_PHOTO_MODERATION_CARD_GENERATION_DESIGN.md`](SUPPLIER_OFFER_PHOTO_MODERATION_CARD_GENERATION_DESIGN.md) **(B7.3A** **decision** **log** **).**
- **B7.4A** **(media** **storage** **readiness** **audit,** **docs** **2026):** **Audit** **completed** **—** **[`docs/B7_4A_MEDIA_STORAGE_PIPELINE_READINESS_AUDIT.md`](B7_4A_MEDIA_STORAGE_PIPELINE_READINESS_AUDIT.md)**. **Current** **media** **pipeline** **is** **metadata-only** **;** **`publish_safe`** **is** **deferred** **with** **no** **durable** **`object_key`** **/** **`public_url`** **.** **Handoff:** [`docs/HANDOFF_MEDIA_STORAGE_PIPELINE_READINESS_B7_4A_TO_NEXT_STEP.md`](HANDOFF_MEDIA_STORAGE_PIPELINE_READINESS_B7_4A_TO_NEXT_STEP.md)**. **No** **code,** **migrations,** **or** **production** **storage** **access** **in** **B7.4A.**
- **B7.4B** **(media** **storage** **ingestion** **contract,** **docs** **2026):** **[`docs/B7_4B_MEDIA_STORAGE_INGESTION_CONTRACT.md`](B7_4B_MEDIA_STORAGE_INGESTION_CONTRACT.md)** **created** **—** **contract** **covers** **allowed** **source** **types** **;** **ingestion** **eligibility** **(** **aligned** **with** **C2B8A** **/** **C2B7.2a** **)** **;** **storage** **adapter** **contract** **;** **Telegram** **`getFile`** **/** **download** **design** **(** **implementation** **later** **)** **;** **deterministic** **object** **key** **/** **idempotency** **/** **retry** **;** **`publish_safe`** **vNext** **lifecycle** **;** **security** **/** **config** **/** **testing** **expectations** **.** **Scope** **:** **no** **code,** **migrations,** **storage** **dependencies,** **Telegram** **download,** **production** **storage** **access,** **Mini** **App,** **booking** **/** **payment** **/** **orders,** **or** **publish** **readiness** **behavior** **changes** **.** **Handoff:** [`docs/HANDOFF_MEDIA_STORAGE_INGESTION_CONTRACT_B7_4B_TO_NEXT_STEP.md`](HANDOFF_MEDIA_STORAGE_INGESTION_CONTRACT_B7_4B_TO_NEXT_STEP.md)**.**
- **B7.4C** **(media** **storage** **implementation** **foundation,** **code** **2026):** **`MEDIA_STORAGE_*`** **config** **(** **`disabled`** **default** **;** **unknown** **backend** **→** **`disabled`** **;** **`MEDIA_STORAGE_ALLOW_HTTPS_FETCH`** **default** **false** **;** **`MEDIA_STORAGE_MAX_BYTES`** **clamped** **;** **no** **credentials** **required** **at** **startup** **)** **;** **`app/core/media_storage_types.py`** **(** **`StoredObject`** **,** **enums** **)** **;** **`MediaStorageAdapter`** **/** **`DisabledMediaStorageAdapter`** **/** **`InMemoryMediaStorageAdapter`** **/** **`get_media_storage_adapter()`** **(** **`s3_compatible`** **→** **`MediaStorageNotImplementedError`** **)** **;** **`classify_cover_media_reference`** **/** **`evaluate_media_ingestion_eligibility`** **(** **pure** **,** **no** **I/O** **)** **(** **deterministic** **object-key** **builder** **not** **yet** **—** **B7.4D** **)** **;** **`.env.example`** **block** **;** **tests** **`tests/unit/test_media_storage_adapter.py`** **,** **`tests/unit/test_supplier_offer_media_ingestion_eligibility.py`** **.** **Not** **in** **slice:** **real** **`getFile`** **/** **download** **,** **S3** **/** **object** **storage** **,** **remote** **URL** **fetch** **,** **publish** **integration** **,** **`publish_safe`** **vNext** **metadata** **(** **B7.4D** **)** **,** **publish** **readiness** **behavior** **changes** **,** **migrations** **,** **Mini** **App** **/** **booking** **/** **payment** **/** **orders** **.** **Handoff:** [`docs/HANDOFF_MEDIA_STORAGE_FOUNDATION_B7_4C_TO_NEXT_STEP.md`](HANDOFF_MEDIA_STORAGE_FOUNDATION_B7_4C_TO_NEXT_STEP.md)**.**
- **B7.4D** **(`publish_safe`** **vNext** **metadata,** **code** **2026):** **`app/services/supplier_offer_publish_safe_vnext.py`** **—** **`PublishSafeVNextStatus`** **(** **deferred** **/** **pending** **/** **ready** **/** **failed** **/** **blocked** **)** **,** **`B7_4D_VERSION`** **,** **fingerprint** **/** **review** **`snapshot`** **helpers** **,** **merge** **only** **`publish_safe`** **(** **preserves** **other** **`packaging_draft_json`** **keys** **)** **,** **state** **builders** **,** **`publish_safe_apply_hero_drift_block`** **(** **stale** **ready** **→** **blocked** **)** **,** **`mark_publish_safe_ready`** **,** **`merge_deferred_publish_safe_compat`** **;** **tests** **`tests/unit/test_supplier_offer_publish_safe_vnext.py`** **.** **Helpers** **do** **not** **mutate** **`media_review`** **/** **`cover_media_reference`** **/** **lifecycle** **(** **callers** **assign** **draft** **)** **.** **Not** **in** **slice:** **`getFile`** **/** **download** **,** **storage** **upload** **,** **URL** **fetch** **,** **publish** **integration** **,** **readiness** **changes** **,** **migrations** **,** **Mini** **App** **/** **booking** **/** **payment** **/** **orders** **.** **Further** **B7.4** **micro-steps** **paused** **—** **see** **«Checkpoint:** **Telegram** **conversion** **workflow** **+** **media** **foundation** **pause»** **below** **.** **Handoff:** [`docs/HANDOFF_PUBLISH_SAFE_VNEXT_METADATA_B7_4D_TO_NEXT_STEP.md`](HANDOFF_PUBLISH_SAFE_VNEXT_METADATA_B7_4D_TO_NEXT_STEP.md)**.**

## Checkpoint: Telegram conversion workflow + media foundation pause

**Status:** **closed** **/** **paused** **intentionally** **—** **Telegram** **conversion** **chain** **and** **media** **foundation** **through** **B7.4D** **are** **recorded** **complete** **;** **further** **media** **/** **storage** **micro-steps** **stop** **here** **until** **a** **new** **explicit** **product** **/** **technical** **decision** **.** **Next** **work** **should** **be** **chosen** **as** **a** **larger** **block** **(** **not** **an** **open-ended** **B7.4** **continuation** **)** **.**

**Expanded** **checkpoint** **(** **terminology** **,** **OPS** **smoke** **,** **next** **blocks** **,** **guardrails** **)** **:** **[`docs/TELEGRAM_CONVERSION_AND_MEDIA_FOUNDATION_CHECKPOINT.md`](TELEGRAM_CONVERSION_AND_MEDIA_FOUNDATION_CHECKPOINT.md)** **.**

### Completed — Telegram operator conversion chain

- **C2B8B** **—** **Publică** **/** **Publish** **(** **`operator_workflow.actions.publish_showcase_channel.enabled` **)** **;** **propose** **/** **confirm** **+** **`review-package`** **re-read** **;** **legacy** **one-step** **publish** **retired** **.**
...
- **C2B10T-C** **—** **Booking** **link** **(** **`create_execution_link.enabled` **)** **;** **execution** **link** **only** **.**
- **C2B10T-D** **—** **OPS** **smoke** **/** **runbook** **(** **docs** **)** **;** **keyboard** **order** **:** **Link** **tour** **→** **List** **for** **sale** **→** **Publish** **→** **Booking** **link** **.**
- **B10.6A** **—** **customer** **`/start`** **`supoffer_*`** **copy** **/** **CTA** **from** **`copy_bucket`** **(** **read-only** **;** **B11** **gate** **semantics** **unchanged** **)** **.**
- **Layer** **A** **booking** **/** **payment** **/** **orders** **were** **not** **touched** **;** **Mini** **App** **was** **not** **touched** **;** **bridge** **/** **catalog** **/** **execution** **/** **publish** **mutations** **were** **not** **added** **in** **this** **slice** **.**

### Completed — Media storage foundation (safe point only)

- **B7.4A** **—** **readiness** **audit** **;** **B7.4B** **—** **ingestion** **contract** **;** **B7.4C** **—** **config** **+** **adapter** **+** **eligibility** **;** **B7.4D** **—** **`publish_safe`** **vNext** **metadata** **helpers** **(** **states** **:** **deferred** **/** **pending** **/** **ready** **/** **failed** **/** **blocked** **)** **.**
- **Not** **implemented** **at** **this** **pause** **:** **durable** **ingestion** **/** **`getFile`** **/** **real** **object** **storage** **/** **render** **/** **channel** **`sendPhoto`** **integration** **/** **readiness** **policy** **change** **for** **durable** **assets** **.**

### Paused intentionally — awaiting explicit next decision

- **B7.4D2** **/** **ingestion** **orchestrator** **(** **placeholder** **label** **—** **not** **a** **committed** **slice** **)** **.**
- **B7.4C2** **—** **Telegram** **downloader** **mock** **/** **contract** **.**
- **B7.4E** **—** **durable-media** **readiness** **policy** **/** **gates** **.**
- **B7.5** **—** **rendered** **branded** **card** **asset** **.**
- **B7.6** **—** **`sendPhoto`** **/** **`sendMediaGroup`** **publish** **integration** **.**

---

## Continuity Sync — B10.x supplier offer → Tour → Mini App (2026)

### Current checkpoint (B10.5 done; B10.6A customer router copy done; B8 line done)

- **Status:** **B10.5** completed and smoke-accepted; **B10.6A** implements **customer-facing** Bot-as-router **copy/CTA** for **`/start supoffer_<id>`** (**B11** resolver decisions unchanged; see **Slice B10.6A** in the Admin Offer section above). **B8** (slice 1 + stabilization + B8.2 + B8.3) **documented** **complete** for this handoff **—** see **section** **above** and **[`docs/HANDOFF_B10X_TO_B8_RECURRING_SUPPLIER_OFFERS.md`](HANDOFF_B10X_TO_B8_RECURRING_SUPPLIER_OFFERS.md)**. **Tech** **debt** **(B8** **re-run,** `start_offset_days=0`): **`OPEN_QUESTIONS`**. **Next** **(product** **choice** **not** **implied** **here):** **B7.3B** **(metadata** **stub** **)** **/** **storage** **slice** **when** **prioritized,** **or** **B12/B13** per **plan** **;** **B7.3A** **policy** **—** **section** **above** **;** **Production/OPS** **Mode** **A/B** **smoke** **—** **walkthrough** **.**
- **Roles (2026):** **Telegram channel** = marketing showcase. **Telegram private bot** = router / consultant / entry (**B10.6A** copy layer on **`supoffer_*`** deep links; not duplicate Mini App catalog). **Mini App** = execution truth and conversion. **Layer A** = booking / payment authority. **Supplier offer** = source facts. **Tour** = customer-facing catalog object. **Admin** = final bridge and activation decisions. **AI** = draft packaging only, not final publisher.
...
**B10.6 → B10.6A (delivered for current scope):** **B10.6** was the **router/consultant** positioning for the **private** bot (**short** **summary** **+** **deep** **link** **into** **Mini** **App**, **not** **a** **second** **catalog**). **B10.6A** implements **that** **layer** **for** **`/start supoffer_<id>`** **via** **customer** **copy** **/** **CTA** **only** **(** **`Slice B10.6A`** **above** **;** **B11** **unchanged** **)** **.** **Further** **B10.6** **ideas** **(** **e.g.** **other** **entry** **points** **)** **remain** **`OPEN_QUESTIONS`** **if** **needed** **.**
...
- **Y37.2 implementation (accepted):** **`POST /admin/custom-requests/{id}/mark-under-review`** with **`X-Admin-Actor-Telegram-Id`**; Telegram **Mark under review** on detail when **Owner = you** and **status = open**; idempotent when already **`under_review`** and same assignee; blocks unassigned / other operator / non-actionable statuses; **does not** change assignment or touch booking, payment, bridge, supplier routes, execution links, or Mini App scoping. Tests: **`tests/unit/test_api_admin.py`**, **`tests/unit/test_telegram_admin_moderation_y281.py`**.
- **Y37.3 design gate (docs-only):** operator **decision** layer after **`under_review`** — **`docs/OPERATOR_DECISION_GATE.md`**. First-class **intent** (e.g. need supplier follow-up vs manual follow-up) without auto-booking/bridge/supplier/execution-link/customer side effects; **one** narrow first runtime slice in a **future** ticket; **additive** data model only if needed. **No** app code, migrations, or handlers in Y37.3.
- **Y37.4 implementation (accepted):** first operator workflow intent **`need_manual_followup`** — **`POST /admin/custom-requests/{id}/operator-decision`** with **`X-Admin-Actor-Telegram-Id`**; only when **status = under_review** and **assigned to current operator**; idempotent when intent already matches; **no** booking, bridge, supplier routes, execution-link, payment, Mini App, or customer-notification side effects; design source **`docs/OPERATOR_DECISION_GATE.md`**.
- **Y37.5 implementation + docs acceptance (post–Y37.5 checkpoint):** second intent **`need_supplier_offer`** added to enum/API/Telegram alongside Y37.4 (same rules, idempotency, permissions). **Migration `20260501_20`** adds **`need_supplier_offer`** to **`operator_workflow_intent`**. Telegram detail when **Owner = you**, **under_review**, **intent = null** shows **both** buttons with readable labels **Need manual follow-up** and **Need supplier offer**; after selection both buttons hide; **Next step** shows the selected intent with the same readable labels. **Bug fix accepted:** **Next step** for **`NEED_SUPPLIER_OFFER`** must not fall through to raw enum text. Operator workflow intent remains **internal admin/ops only**. **No** supplier action, RFQ, bridge, booking, payment, Mini App, customer notification, execution-link, or identity bridge side effects. **Tests:** **`AdminRouteTests::test_admin_custom_request_operator_decision`**; full **`tests/unit/test_telegram_admin_moderation_y281.py`** — **54** passed. **Y38 (docs-only):** **Supplier interaction** boundary gate is **`docs/SUPPLIER_INTERACTION_GATE.md`** (operator workflow decision-only; **`operator_workflow_intent`** does not execute supplier logic; future supplier work is a **separate layer** and consumes intent as **input only**). **Next safe:** after **Y38** gate **acceptance**, schedule minimal **separate** implementation slices; **do not** auto-contact or notify suppliers from intent set alone. Older link: **`docs/Y38_SUPPLIER_INTENT_INTERACTION_DESIGN_GATE.md`** → canonical **`docs/SUPPLIER_INTERACTION_GATE.md`**.

- **Post–Y38 (docs) —** Canonical **next step** is only in **`docs/SUPPLIER_INTERACTION_GATE.md`** — section *Post–Y38: explicit next step*. **Layer C** is **complete** in current scope; **no** in-repo supplier/RFQ automation until a **separate** future design gate + ticket. **`operator_workflow_intent`** is **not** a direct trigger for supplier execution.

- **Y39 (docs-only, accepted design checkpoint) —** **`docs/SUPPLIER_ENTRY_POINTS.md`**: catalog of **explicit only** future **entry points** for supplier-impacting work (admin explicit action, scheduled/background job, external trigger, operator “do” action **separate** from intent — **types** only; no new routes). **`operator_workflow_intent`** and **`POST /admin/custom-requests/{id}/operator-decision`** are **not** triggers; future **implementation** tickets must **name one** Y39 **entry family** and **surface** (explicit API, worker, or admin action). **Preserves Y38.** **No** implementation in this checkpoint: **no** supplier messaging, RFQ, booking/payment, Mini App, execution links, identity bridge, or notifications.

- **Y40 (docs-only, accepted design checkpoint) —** **`docs/SUPPLIER_EXECUTION_FLOW.md`**: **logical** supplier **execution** pipeline after a Y39 start — stages: explicit entry → validation → **execution / audit** record → supplier action **attempt** → result recording → optional operator/admin **review**; **distinction** — `operator_workflow_intent` = **decision data**; **entry** = start signal; **flow** = **controlled** pipeline. **Invariants:** idempotency, auditability, **retry** safety, no **hidden** triggers, **fail-closed**, explicit **permissions**. **May eventually** (only when implemented in a **separate** ticket) prepare contact / send / record response; **Y40 adds** **no** `app/` code, **no** messaging, **no** RFQ, booking, payment, Mini App, execution links, identity bridge, or notifications. **Preserves Y38 + Y39.**

- **Y41 (docs-only, accepted design checkpoint) —** **`docs/SUPPLIER_EXECUTION_DATA_CONTRACT.md`**: **logical** (no migrations) **data contract** for future execution **request** + **attempt** + **result/audit** fields; **status** set: `pending`, `validated`, `blocked`, `attempted`, `succeeded`, `failed`, `cancelled`; `operator_workflow_intent` **snapshotted** for context, **not** live trigger, **not** primary execution state. **Separation:** no **orders** / **payments** / **bookings**, no **Mini App** dependency, no **execution link** or **identity bridge** mutation, no **customer** notifications from this contract. **No** `app/` in Y41. **Cites** Y38–Y40.

- **Y42 (docs-only, accepted design gate) —** **`docs/SUPPLIER_EXECUTION_PERMISSION_AUDIT_GATE.md`**: **who** may start future supplier execution (central **admin** explicit, **authorized** operator **explicit**, **authorized** system **job**, **authenticated** **external** integration); **permission** — assigned operator may **record intent**, **not** the same as **execution** permission; `operator-decision` **not** an execution route; **audit** — initiator, **entry** point, **source** entity, **intent** snapshot (if any), **idempotency** key, **validation** outcome, **attempt/result** state, **error** on block/fail; **fail-closed** — no permission, no **source** entity, no **idempotency** (when required), **ambiguous** source, **stale** snapshot must **not** auto-refresh into execution; **no** hidden **DB** triggers. **No** `app/`. **No** messaging, RFQ, booking/payment, Mini App, execution links, identity bridge, customer notifications. **Cites** Y38–Y41.

- **Y43 (implementation, persistence-only) —** **Migration `20260502_21`**: tables **`supplier_execution_requests`**, **`supplier_execution_attempts`** (Y41 **logical** contract); new enums for **source entry**, **source entity type**, **request** and **attempt** **statuses**, **attempt channel**; **`operator_workflow_intent_snapshot`** reuses `operator_workflow_intent` PG enum — **context only**; **check** constraints: positive `source_entity_id`, **non-blank** `idempotency_key`; **unique** `idempotency_key`. **ORM:** `SupplierExecutionRequest`, `SupplierExecutionAttempt`; **repository** `app/repositories/supplier_execution.py` with **fail-closed** validation (empty idempotency, bad ids). **Tests:** `tests/unit/test_supplier_execution_persistence.py`. **No** **API** routes, **no** workers, **no** **messaging**, **no** `operator-decision` changes, **no** Mini App, orders, execution links, identity bridge, customer notifications, RFQ behavior. **Cites** Y38–Y42.

- **Y45 (docs-only, accepted design checkpoint) —** **`docs/SUPPLIER_EXECUTION_TRIGGER_DESIGN.md`**: the **first** allowed **trigger** to **open** a supplier **execution** **audit** line is **admin explicit** **action** **only** (Y39 **family** → `source_entry_point` **`admin_explicit`**); **not** **`POST /admin/custom-requests/{id}/operator-decision`**, **not** **`operator_workflow_intent`** as a **start** signal. The **trigger** **creates** and **validates** a **`supplier_execution_request`** row **only** (Y41); it **must** **not** **contact** suppliers and **must** **not** **create** **`supplier_execution_attempt`** rows in this **design** **slice**; no supplier **messaging**, no RFQ/bridge/booking/payment work, no Mini App, no execution **links**, no identity **bridge**, no **notifications**. **Y38**–**Y42** **hold**: intent **≠** **execution**; **explicit** **entry** **control** (Y39); Y40 **flow** **stages** **1–3** for this **start**; Y41 **data** **contract**; Y42 **permission** + **audit** + **fail-closed**. **Y45** **as** a **document** is **design-only**; **next** **safe** **in-app** **step** is **implementation** of a **safe** **admin** **trigger** **endpoint** (or same **semantics** on an **approved** **surface**) with **no** **supplier** **messaging** and **no** **attempt**-level **execution** **runtime** until a **separate** ticket.

- **Y47 (docs-only, accepted design checkpoint) —** **`docs/SUPPLIER_EXECUTION_ATTEMPT_DESIGN.md`**: **request** **≠** **attempt** — **`supplier_execution_request`** = **intention** / **audit** **anchor** (opened by **Y45**/**Y46** **admin** **trigger**); **`supplier_execution_attempt`** = **one** **try** at **future** **supplier-impacting** **action** (Y40 **stage** **4**), **child** of **request**. **Attempts** are **not** **created** **by** the **Y46** **trigger**; **not** **automatic**; **not** from **hidden** **DB**/**ORM** **triggers**; they **require** a **separate** **explicit** **entry** in a **future** ticket. This **design** **adds** **no** supplier **messaging**, **no** **supplier** **API** **calls**, **no** **workers**, **no** new **RFQ** **flow**, **no** **booking**/**order**/**payment**, **no** **Mini** **App**, **no** **execution** **links**, **no** **identity** **bridge**, **no** **notifications**. **Cites** Y38–Y45. **Y48** **implements** **safe** **attempt** **row** **creation** — see **bullet** list **+** `OPEN_QUESTIONS` **(Y48)**.

- **Y49 (docs-only, accepted design gate) —** **`docs/SUPPLIER_OUTBOUND_MESSAGING_DESIGN.md`**: **outbound** **supplier** **messaging** is the **first** **external** **irreversible** **side** **effect** in the **Y38**–**Y48** **execution** **line** ( **real** **contact** to **suppliers** / **partner** **channels**). **Messaging** is **only** **architecturally** **allowed** **inside** **execution-attempt**-**tied** **outbound** **logic** (Y40 **stage** **4**); **not** in **Y45**/**Y46** **trigger**; **not** in **`supplier_execution_request`** **create**; **not** in **Y48** **`POST` …/attempts** **row** **create** **alone** **as** a **send**. **Every** **conforming** **send** must **have** **explicit** **permission** (Y42), **idempotency** ( **send** **scope**), **and** **audit** ( **who** / **when** / **target** / **correlation** / **outcome**). **Messaging** **must** **not** be **triggered** by **`operator_workflow_intent`**, **`operator-decision`**, **request** **creation**, or **attempt**-**row** **creation** **by** **themselves** (Y38, Y45, Y47, Y48). This **doc** does **not** **implement** **sends** by **itself**; it **defined** the **Y50** **slice**. **Y38**–**Y48**+**Y49** **guarantees** **remain** after **Y50** **(below)**.
- **Y50 (implementation, completed) —** **Telegram**-**only** **outbound** **messaging** is **implemented** and **controlled**: **`POST` `/admin/supplier-execution-attempts/{attempt_id}/send-telegram`** (**`ADMIN_API_TOKEN`**, **explicit** **admin** **action** via **`X-Admin-Actor-Telegram-Id`**, **pending** **attempt** **only**, **mandatory** **idempotency** **header** **or** **body**). **One** **attempt** = **at** **most** **one** **deduplicated** **successful** **send** per **idempotency** **key**; **succeeded**/**failed** **outcome** **on** **`supplier_execution_attempt`**, **`provider_reference`** = **Telegram** **message** **id** **when** **ok**. **Migration `20260425_22`**: **`supplier_execution_attempt_telegram_idempotency`** with **`UNIQUE` (`supplier_execution_attempt_id`, `idempotency_key`)**. **No** **automatic** **sends**; **no** **sends** from **Y46** **request** **create** or **Y48** **attempt** **create**; **no** **intent**-**triggered** **sends**; **Y38**–**Y49** **architecture** **preserved** (RFQ, booking/order/payment, Mini App, execution links, identity bridge, **customer** **notifications**, **fan**-**out**, **workers**/**retries** still **out** **of** **scope**).
- **Y51 (docs-only, accepted design gate) —** **`docs/SUPPLIER_MESSAGING_AUDIT_RETRY_DESIGN.md`**: **admin/operator** **visibility** **requirements**; **audit** **hardening**; **retry** **principles**. **No** **automatic** **retries**; **no** **hidden** **retry** **on** **read**; **retry** **implementation** **follows** **a** **separate** **gate** **(Y53** **design** **+** **Y54** **`POST` …/retry**)**; **Y38**–**Y50** **guarantees** **unchanged** **in** **meaning**.
- **Y52 (implementation, completed) —** **Read** /**audit** **visibility** for **supplier** **messaging** **attempts** (status, **target_supplier_ref**, **provider_reference**, **errors**, **idempotency** **evidence**, **timestamps** **where** **available**); **no** **retry** **execution**; **no** **auto** **retry**; **cites** **Y51**. **Handoff:** **`docs/HANDOFF_Y52_SUPPLIER_MESSAGING_READ_AUDIT_VISIBILITY.md`**.
- **Y53 (design-only, accepted checkpoint) —** **[`docs/SUPPLIER_MANUAL_RETRY_DESIGN.md`](SUPPLIER_MANUAL_RETRY_DESIGN.md)**: **design** **+** **rules** **only** (**no** **runtime** **or** **migrations** **in** **this** **gate**). **1)** **Retry** is **manual** **only** **(explicit** **admin** **/ **Y42**-**class** **operator** **as** **policy** **allows**)**. **2)** **No** **automatic** **retry**; **no** **hidden** **retry**; **no** **retry** **on** **read** **(list** / **GET** / **detail** **must** **not** **invoke** **send**). **3)** **Preferred** **model**:** create **a** **new** **`supplier_execution_attempt`** **for** a **retry** — **not** **in**-**place** **resend** **of** a **failed** **row** **by** **default** **(Y47**)**. **4)** **Each** **new** **retry** **send** **requires** **a** **new** **idempotency** **key** **(Y50** **per**-**attempt** **scope**)**. **5)** **Same** **`attempt_id` + `idempotency_key`** = **Y50** **replay** / **no** **duplicate** **Telegram** **message**. **Audit** **minimums** **(Y53):** **`original_attempt_id`**, **`retry_attempt_id`**, **`retry_requested_by`**, **`retry_reason`**, **timestamp**, **idempotency** **key** **(Y54** **persistence** **for** **link** **/** **reason** **/** **requester**; **idempotency** **key** **still** **from** **Y50** **send**)**. **Y38**–**Y52** **unchanged** **in** **meaning**. **Handoff:** **`docs/HANDOFF_Y53_SUPPLIER_MANUAL_RETRY_DESIGN.md`**.
- **Y54 (implementation, completed) —** **Manual** **retry** **is** **live:** **`POST` `/admin/supplier-execution-attempts/{attempt_id}/retry`**. **Creates** **a** **new** **`supplier_execution_attempt`** **(next** **`attempt_number`**, **pending**)**; **does** **not** **send** **Telegram** **automatically**. **Only** **failed** **original** **attempts** **are** **retry**-**eligible**; **explicit** **admin** **action** **only** (**`ADMIN_API_TOKEN`**, **`X-Admin-Actor-Telegram-Id`**, **`retry_reason`** **in** **body**). **Does** **not** **reuse** **Y50** **send** **idempotency** **—** **outbound** **send** **requires** **`POST` …/send-telegram` (Y50)** on **the** **new** **attempt** **with** **a** **new** **idempotency** **key**. **Audit** **(migration** **`20260526_23`):** **`retry_from_supplier_execution_attempt_id`**, **`retry_reason`**, **`retry_requested_by_user_id`**. **All** **Y38**–**Y53** **rules** **remain** **in** **force** **(unchanged** **in** **meaning**)**; **Y54** **adds** **no** **booking**/**order**/**payment**, **Mini** **App**, **execution** **links**, **identity** **bridge**, **customer** **notifications**, **or** **fan**-**out**. **Handoff:** **`docs/HANDOFF_Y54_SUPPLIER_MANUAL_RETRY_IMPLEMENTATION.md`**.

### BUSINESS line — supplier offer → tour (B1–B13 design baseline)

- **Status (2026-04-25 / B2 2026-04-26):** **BUSINESS** **baseline** **+** **B1** **design** **accepted** **(docs-only).** **B2** **(implementation)** **—** **supplier** **offer** **content**/**data** **upgrade** **completed** **(storage** **+** **contracts;** **no** **AI,** **Tour,** **publish** **behavior,** **Mini** **App,** **booking,** **messaging**)**.** **Handoff** **records:** [`docs/HANDOFF_B0_B1_SUPPLIER_OFFER_BUSINESS_BASE_AND_INTAKE_DESIGN.md`](HANDOFF_B0_B1_SUPPLIER_OFFER_BUSINESS_BASE_AND_INTAKE_DESIGN.md) (B0/B1); [`docs/HANDOFF_B1_SUPPLIER_OFFER_INTAKE_AI_PACKAGING_MODERATION_DESIGN.md`](HANDOFF_B1_SUPPLIER_OFFER_INTAKE_AI_PACKAGING_MODERATION_DESIGN.md) (B1); [`docs/HANDOFF_B2_SUPPLIER_OFFER_CONTENT_DATA_UPGRADE.md`](HANDOFF_B2_SUPPLIER_OFFER_CONTENT_DATA_UPGRADE.md) (B2).
- **Authoritative BUSINESS plan:** **[`docs/SUPPLIER_OFFER_TO_TOUR_BUSINESS_PLAN.md`](SUPPLIER_OFFER_TO_TOUR_BUSINESS_PLAN.md)** — sequences **B1**–**B13** (intake, AI packaging, moderation, templates, bridge, routing, ops visibility, smoke). **Core rule (accepted):** a **published** supplier offer **becomes** or **attaches** to a **Layer A** **`Tour`** visible in the Mini App catalog **only** through an **explicit bridge** (not silent DB/AI side effects). **V2** track numbering in `IMPLEMENTATION_PLAN_V2_SUPPLIER_MARKETPLACE.md` remains **historical** delivery context; **B**-line is the **forward** product sequence for this domain.
- **B1 (design, accepted):** **[`docs/SUPPLIER_OFFER_INTAKE_AI_PACKAGING_MODERATION_DESIGN.md`](SUPPLIER_OFFER_INTAKE_AI_PACKAGING_MODERATION_DESIGN.md)** — **§§1–8:** **principle**, **intake** **data**, **supplier** **dialog**, **AI** **output** **contract**, **admin** **moderation** **flow**, **status** **model**, **guardrails**, **next** **steps** (**B2**–**B5**, **B9**). Supplier **raw** **facts**; **AI** **draft** **packaging** **only**; **admin** **review**/**edit**/**approve**; **only** an **approved** **package** **may** **be** **published**; **tour** **link** is **B9**/**B10**; **AI** **must** **not** invent **dates**/**prices**/**seats**, **must** **not** **publish**, **book**, **or** **silently** **create** **Tour**; **Layer** **A** **booking**/**payment** **unchanged**.
- **B2 (implementation, completed):** **Migration** **`20260526_24`** **—** **extended** **`supplier_offers`** **columns** **+** **supplier-admin** **/** **admin** **read** **models** **(`AdminSupplierOfferRead`** **for** **admin-only** **metadata)**. **Data** **only** **—** **see** [`HANDOFF_B2_...`](HANDOFF_B2_SUPPLIER_OFFER_CONTENT_DATA_UPGRADE.md). **Next** **safe** **BUSINESS** **implementation** **step:** **B3** **—** **Supplier** **dialog** **upgrade** *(historical baseline line; in-repo B-line progress has advanced — see B6/B7 handoffs in this file).*
- **B6 (implementation, completed):** **Branded** **Telegram** **preview** **JSON** **in** **`packaging_draft_json`** **(deterministic,** **no** **image** **download,** **no** **publish).** **Handoff:** [`docs/HANDOFF_B6_BRANDED_TELEGRAM_POST_CARD_TEMPLATE.md`](docs/HANDOFF_B6_BRANDED_TELEGRAM_POST_CARD_TEMPLATE.md).
...
- Y36.1 **Operator assignment** gate is in **`docs/OPERATOR_ASSIGNMENT_GATE.md`**. The implemented and **accepted** slice is **Y36.2** + **Y36.2A** + **Y36.3** + **Y36.4** with **Y36.5** docs acceptance and **Y36.6** travel-date copy polish. **Y37.1** operator **workflow** gate: **`docs/OPERATOR_WORKFLOW_GATE.md`**. **Y37.2** **mark under review** is **implemented/accepted** (see bullet list above). **Y37.3** (docs-only): operator **decision** layer after **`under_review`** — **`docs/OPERATOR_DECISION_GATE.md`**. **Y37.4** / **Y37.5** first/second operator intents are **implemented and accepted** (enum/API/Telegram; migration **`20260501_20`**; internal intent only). **Y36.4** Telegram polish is **shipped/accepted**; **reassign** / **unassign** / **resolve** / **close** / supplier RFQ / bridge / booking **driven from** intent remain **out of scope** until their **design gates** (not ad hoc); **`need_supplier_offer`** does **not** auto-contact suppliers.
- **Next safe order:**
  1. **Supplier** **execution** **line** **Y38**–**Y54** **is** **MVP** **complete** **( **`docs/HANDOFF_Y54_SUPPLIER_MANUAL_RETRY_IMPLEMENTATION.md`**) **—** **trigger** **(Y46)**, **attempts** **(Y48)**, **Telegram** **send** **(Y50)**, **read**/**audit** **(Y51**–**Y52)**, **manual** **retry** **design** **(Y53)** + **implementation** **(Y54)**. **Next:** **treat** **this** **execution** **layer** **as** **closed** **for** **MVP** **(document** **/ **architect** **if** **needed**) **or** **move** **to** **supplier** **onboarding** **/** **identity** **/** **other** **product** **flows** **—** **not** **ad** **hoc** **RFQ** **/** **booking** **/** **Mini** **App** **/** **execution** **links** **/** **bridge** **without** **gates**)**.
  2. Operator/admin workflow for creating/replacing/closing **execution links** (ongoing where applicable — separate from RFQ assignment / supplier-intent automation).
...
- **V2 supplier marketplace — Track 3 (Supplier offer publication / moderation) — completed (implementation, stabilization reviewed):** extends **`supplier_offer_lifecycle`** with **`approved`**, **`rejected`**, **`published`** and moderation/showcase columns (**Alembic `20260418_08`**); moderation/publish routes are **central admin only** under the same **`ADMIN_API_TOKEN`** gate as **`/admin/*`** — **`GET /admin/supplier-offers`**, **`POST .../moderation/approve`**, **`POST .../moderation/reject`**, **`POST .../publish`** (Telegram channel via **`TELEGRAM_OFFER_SHOWCASE_CHANNEL_ID`** + bot token); **suppliers cannot** set moderation/publish states or edit **`approved`**/**`published`** offers (**`SupplierOfferService`** guards); **`publish`** requires prior **`approved`** (rejected/draft/ready cannot publish); private **`/start supoffer_<id>`** intro only when offer is **`published`** — **B11 slice:** active **`supplier_offer_execution_links`** + **`open_for_sale`** visible **`Tour`** → primary Mini App **`/tours/{code}`** WebApp; **no generic `_send_catalog_overview`** after that path; **fallback** retains intro + catalog/overview (**Layer A** unchanged).
- **B10.6 (first implementation slice — bot router):** private **`/start tour_<code>`** and **callback** tour-detail from catalog use **primary WebApp** to exact Mini App **`/tours/{tour_code}`** (`build_tour_detail_keyboard` + **`mini_app_tour_detail_url`**); **`prepare_reservation`** / **`request_booking_assistance`** / back / language unchanged; **secondary** WebApp rows: catalog home + bookings.
- **B10.6B (generic `/start` / `/tours`):** **`router_home_body`** + **`send_or_edit_router_home`** (`_send_router_home`) — Mini App catalog + My bookings + optional in-chat filters + **Browse tours** + language; **no automatic 1–3 in-chat tour cards** on generic entry. **`_send_catalog_overview`** unchanged for **Browse tours** callback, cancel-destination return, and other explicit in-chat catalog paths; **`grp_*`** **/** invalid **`supoffer`** fallback still use **`_send_catalog_overview`** after their intros. **Transient store:** router singleton vs home+catalog pair (`transient_messages`).
- **B10.6C (copy / full_bus wording):** **`router_home_body`** lines aligned with visible **`open_catalog`** labels per language (RO **„Vezi tururi”** etc.); EN/RO **`assisted_booking_detail_note`** reframed from “per-seat automation” to **self-serve / charter or whole-vehicle package** so full_bus is not read as individual-seat selection. **No handler or booking logic change.**
- **B12/B13 (Telegram showcase — first slice + B13.1 branded text):** **`supplier_offer_showcase_message`** — explicit template vs CTA vs HTML assembly; **`build_showcase_publication`** returns **text-only** send (**`photo_url`** always **`None`**; **`sendPhoto`** not used); primary CTA **„Deschide în bot”** → stable **`supoffer_<id>`** bot deep link; secondary **„Vezi în aplicație”** → supplier-offer Mini App landing (**non-booking** wording vs former **„Rezervă”**); **`Preț`** line **„orientativ”** + disclaimer row for Mini App/booking truth. **`POST .../publish`** orchestration unchanged. **B13.1** **(docs** **sync** **2026):** **Romanian** **emoji-section** **marketing** **layout** **(deterministic** **supplier** **facts** **;** **`included_text`**/**`excluded_text`** **when** **set** **;** **no** **channel** **`/tours/{code}`** **)** **—** **branded** **channel** **post** **vs** **dry** **ORM** **listing** **;** **Mini** **App** **execution** **truth** **unchanged** **;** **admin** **final** **publisher** **unchanged** **;** **B7.4**/**B7.5**/**B7.6** **storage**/**render**/**sendPhoto** **still** **future** **.**
- **B12A (implemented):** **Showcase marketing template library foundation** — **[`docs/B12_SHOWCASE_MARKETING_TEMPLATE_LIBRARY.md`](B12_SHOWCASE_MARKETING_TEMPLATE_LIBRARY.md)**; **`ShowcaseMarketingTemplateId`** (**`PER_SEAT_STANDARD`**, **`FULL_BUS_PRIVATE_GROUP`**, **`EARLY_BIRD_DISCOUNT`**, **`CUSTOM_REQUEST_CTA`**, **`SUPPLIER_SERVICE_PROMO`**, **`LAST_SEATS_URGENT`**, **`SHORT_ANNOUNCEMENT`**, **`IMAGE_ONLY_TEASER`**, **`BRAND_AWARENESS_POST`**); **`app/services/showcase_marketing_template_library.py`** infers a safe template from **`SupplierOffer`** facts (early bird only when discount + deadline are both set; **`LAST_SEATS_URGENT`** never auto-inferred); **`packaging/generate`** merges **`showcase_marketing_template_library_v1`** into **`packaging_draft_json`** (read-only for review-package); **`build_showcase_publication`** **unchanged** (no channel semantic change). Preview line builders for alternate templates are **ops-only**. Tests: **`tests/unit/test_showcase_marketing_template_library.py`**.
- **B12B (implemented):** **Admin template preview + optional PATCH selection** — **`GET …/review-package`** adds **`showcase_template_preview`** (inferred / effective / optional admin selection, RO HTML fact lines for ops, **`template_choices`**, invalid-selection **`notes`**, **`channel_publish_unchanged: true`**). **`PATCH /admin/supplier-offers/{id}/packaging/showcase-template`** (**`AdminPackagingShowcaseTemplatePatch`**) writes admin selection into **`packaging_draft_json.showcase_marketing_template_library_v1`**; **`template_id: null`** clears; **`last_seats_urgent`** requires **`live_seats_remaining >= 1`**; blocked when packaging **`approved_for_publish`**; packaging regenerate preserves admin keys; **`flag_modified`** for nested JSONB; **`operator_workflow`** includes **`patch_showcase_marketing_template`** (**PATCH**); Telegram offer detail appends a **read-only** B12B summary when preview exists. **Channel publish output unchanged**; **no publish readiness changes**; **no** Mini App / booking / payment / orders / migrations. Handoff: **[`docs/HANDOFF_B12B_ADMIN_TEMPLATE_PREVIEW_SELECT_TO_NEXT_STEP.md`](HANDOFF_B12B_ADMIN_TEMPLATE_PREVIEW_SELECT_TO_NEXT_STEP.md)**. Tests: **`test_showcase_marketing_template_library`**, **`test_supplier_offer_review_package`**, **`test_supplier_offer_b5_packaging_review`**, **`test_supplier_offer_b4_packaging`**.
- **B12C (implemented):** **Telegram Template / Șablon UI** — **gated** by **`operator_workflow.patch_showcase_marketing_template.enabled`**; placement after **Approve text** / **Aprobă text**, before bridge/publish actions. Picker reflects review-package (**inferred** / **effective** / **selected**, **`notes`**, blocked last-seats hint when verified seats are required). **Safe** one-tap apply for non-seat-gated **`template_choices`**; handler **re-reads** review-package and enforces **`_template_id_allowed_for_telegram_direct_apply`** before **`SupplierOfferPackagingReviewService.patch_showcase_marketing_template`** (same **B12B** service path). **`LAST_SEATS_URGENT`:** FSM **`AdminModerationState.awaiting_showcase_template_last_seats`** → **positive integer** → PATCH **`live_seats_remaining`**. **Clear** selection supported (**`template_id: null`**). **No** publish output or **publish readiness** change; **no** Mini App / booking / payment / orders / migrations. Spec: **[`docs/B12_SHOWCASE_MARKETING_TEMPLATE_LIBRARY.md`](B12_SHOWCASE_MARKETING_TEMPLATE_LIBRARY.md)** §B12C; handoff: **[`docs/HANDOFF_B12C_TELEGRAM_TEMPLATE_SELECTION_UI_TO_NEXT_STEP.md`](HANDOFF_B12C_TELEGRAM_TEMPLATE_SELECTION_UI_TO_NEXT_STEP.md)**. Tests: **`test_operator_workflow_b12c_specs`**, **`test_operator_workflow_c2b3_keyboard`**, **`test_telegram_admin_moderation_y281`**.
- **B13A (completed, docs):** **Channel Adapter Design** — main doc **[`docs/B13_CHANNEL_ADAPTER_DESIGN.md`](B13_CHANNEL_ADAPTER_DESIGN.md)**. Design **separates** review-package / **`operator_workflow`** gates, **content assembly**, optional future **idempotency / audit / outbox** layer, and **channel adapters** (transport I/O). **Existing Telegram publish** path is documented as the **baseline**. **No** code in B13A slice, **no** publish behavior or readiness changes in that slice, **no** Mini App / booking / payment / orders / migrations.
- **B13B (implemented):** **Showcase channel adapter interface + Telegram wrapper** — **`app/services/showcase_channel_adapter.py`** (**`ShowcaseChannelPublishRequest`**, **`ShowcaseChannelPublishResult`**, **`ShowcaseChannelAdapter`**, **`TelegramShowcaseChannelAdapter`**, **`default_telegram_showcase_adapter`**); **`SupplierOfferModerationService.publish`** calls the adapter, which delegates to **`send_showcase_publication`**; **behavior-preserving** refactor — same Telegram output/persistence, **no** publish readiness change, **no** outbox / publish-attempt table / migrations / new channels, **no** Mini App / booking / payment / orders. Handoff: **[`docs/HANDOFF_B13B_CHANNEL_ADAPTER_INTERFACE_TELEGRAM_WRAPPER_TO_NEXT_STEP.md`](HANDOFF_B13B_CHANNEL_ADAPTER_INTERFACE_TELEGRAM_WRAPPER_TO_NEXT_STEP.md)**. Tests: adapter + track3 moderation + review_package + catalog_conversion_closure + full **`test_telegram_admin_moderation_y281`** (**89** passed).
- **B13C (completed, docs):** **Publish attempt / audit design** — **[`docs/B13C_PUBLISH_ATTEMPT_AUDIT_DESIGN.md`](B13C_PUBLISH_ATTEMPT_AUDIT_DESIGN.md)**. Covers current publish **baseline**, purpose of **attempt audit**, **duplicate-send** risk, **idempotency** concept, **retry** policy (future), **transaction / side-effect** boundary, implementation **options** (table vs JSONB vs logs). **No** code, **no** tests, **no** migrations, **no** publish behavior or **readiness** changes, **no** Mini App / booking / payment / orders. Handoff: **[`docs/HANDOFF_B13C_PUBLISH_ATTEMPT_AUDIT_DESIGN_TO_NEXT_STEP.md`](HANDOFF_B13C_PUBLISH_ATTEMPT_AUDIT_DESIGN_TO_NEXT_STEP.md)**. **Recommended:** **explicit product decision** before any attempt-table / migration work; **do not** auto-start implementation.
- **B13D-alt (implemented):** **Read-only channel payload preview** — **`GET /admin/supplier-offers/{offer_id}/showcase-channel-payload`** (same **`ADMIN_API_TOKEN`** admin auth as other **`/admin/supplier-offers`** reads); response **`AdminSupplierOfferShowcaseChannelPayloadRead`** (nested **`AdminShowcasePublicationPayloadRead`**) derived from **`build_showcase_publication`** and **`telegram_showcase_channel_publish_request_preview`** — same logical **`ShowcaseChannelPublishRequest`** as **`publish`**, **`idempotency_key`** **`null`**, **no** Telegram send, **404** if offer missing. **No** **`POST …/publish`** or publish-readiness change, **no** publish-attempt **rows** from this endpoint, **no** retries, **no** new channels, **no** Mini App / booking / payment / orders. Handoff: **[`docs/HANDOFF_B13D_ALT_CHANNEL_PREVIEW_PAYLOAD_READ_MODEL_TO_NEXT_STEP.md`](HANDOFF_B13D_ALT_CHANNEL_PREVIEW_PAYLOAD_READ_MODEL_TO_NEXT_STEP.md)**. **Tests:** **`tests/unit/test_showcase_channel_adapter.py`**, **`tests/unit/test_supplier_offer_track3_moderation.py`** — **23** passed. **Next:** operator ops smoke per **[`docs/B13G_PRODUCTION_PUBLISH_AUDIT_SMOKE_RUNBOOK.md`](B13G_PRODUCTION_PUBLISH_AUDIT_SMOKE_RUNBOOK.md)** when ready; **B13H+** retry/resend **docs-first** (separate ticket).
- **B13D (implemented):** **Showcase publish attempt table + repository/service** — table **`supplier_offer_showcase_publish_attempts`**, Alembic **`20260531_29`**, ORM **`SupplierOfferShowcasePublishAttempt`**, **`SupplierOfferShowcasePublishAttemptRepository`**, **`SupplierOfferShowcasePublishAttemptService`**; statuses **`requested`**, **`provider_sent`**, **`persisted`**, **`failed`**; actor surfaces **`http_admin`**, **`telegram_bot`**; FK **`ON DELETE RESTRICT`**; ORM **`passive_deletes`** on **`SupplierOffer.showcase_publish_attempts`**. **B13E** wires rows from **`publish`** (**next bullet**). Handoff: **[`docs/HANDOFF_B13D_PUBLISH_ATTEMPT_TABLE_SKELETON_TO_NEXT_STEP.md`](HANDOFF_B13D_PUBLISH_ATTEMPT_TABLE_SKELETON_TO_NEXT_STEP.md)**. **Tests:** **`tests/unit/test_supplier_offer_showcase_publish_attempt.py`**.
- **B13E (implemented):** **Publish-path audit** — **`SupplierOfferModerationService.publish`** creates/updates **`supplier_offer_showcase_publish_attempts`**: attempt **`requested`** after guards + **`build_showcase_publication`**; **`TelegramShowcaseChannelAdapter`** call **unchanged**; success **`provider_sent`** → offer **`PUBLISHED`** (unchanged) → attempt **`persisted`** with chat/message ids; **`TelegramShowcaseSendError`** / missing **`message_id`** → **`failed`**, **same** HTTP/Telegram errors as before. HTTP: **`requested_by="http_admin"`**, **`http_admin`** surface; Telegram **C2B8B**: **`telegram_bot`**, **`requested_by="telegram:{admin_id}"`**; SHA-256 **payload fingerprint** on row. **No** publish readiness / operator-workflow gate / Telegram output / retry / **`idempotency_key`** / new channels / Mini App / booking / payment / orders changes. Handoff: **[`docs/HANDOFF_B13E_WIRE_PUBLISH_ATTEMPT_AUDIT_TO_NEXT_STEP.md`](HANDOFF_B13E_WIRE_PUBLISH_ATTEMPT_AUDIT_TO_NEXT_STEP.md)**. **Tests:** **`tests/unit/test_supplier_offer_track3_moderation.py`**, **`tests/unit/test_supplier_offer_showcase_publish_attempt.py`**, **`tests/unit/test_showcase_channel_adapter.py`** — all OK. **Next (forward):** **[`docs/B13G_PRODUCTION_PUBLISH_AUDIT_SMOKE_RUNBOOK.md`](B13G_PRODUCTION_PUBLISH_AUDIT_SMOKE_RUNBOOK.md)** (operator-run smoke); **B13H+** manual retry/resend design (docs-first).
- **B13F (implemented):** **Admin read surface for publish attempt history** — **`GET …/review-package`** adds **`showcase_publish_attempts_review`** (**`AdminSupplierOfferShowcasePublishAttemptsReviewRead`** / **`AdminSupplierOfferShowcasePublishAttemptRead`** items, **`SupplierOfferShowcasePublishAttemptService.list_attempts_review_read`**); Telegram admin offer detail appends a **compact** publish-audit block (read-only, up to **5** recent rows, errors trimmed). **MVP:** **no** dedicated **`GET …/showcase-publish-attempts`** (review-package is the main admin read model). **No** migrations, **no** retry/resend, **no** publish or publish-readiness change, **no** new channels, **no** Mini App / booking / payment / orders. Handoff: **[`docs/HANDOFF_B13F_ADMIN_PUBLISH_ATTEMPT_HISTORY_READ_SURFACE_TO_NEXT_STEP.md`](HANDOFF_B13F_ADMIN_PUBLISH_ATTEMPT_HISTORY_READ_SURFACE_TO_NEXT_STEP.md)**. **Tests:** **`tests/unit/test_supplier_offer_showcase_publish_attempt.py`**, **`tests/unit/test_supplier_offer_review_package.py`** (+ **`test_supplier_offer_ai_public_copy_fact_lock`**, **`test_supplier_offer_catalog_conversion_closure`** for review-package regression).
- **B13G (completed, docs / ops):** **Production publish audit smoke runbook** — main doc **[`docs/B13G_PRODUCTION_PUBLISH_AUDIT_SMOKE_RUNBOOK.md`](B13G_PRODUCTION_PUBLISH_AUDIT_SMOKE_RUNBOOK.md)**. Verifies **Railway / Alembic** (**`upgrade head`**, **`current`** includes **`20260531_29`** or later head that revises it), **read-only** **`GET …/review-package`** (**`showcase_publish_attempts_review`**), optional **controlled** publish smoke + post-publish audit checks, **stop** conditions, **non-goals**. **No** code, **no** tests, **no** new migrations, **no** publish executed by doc authoring, **no** mutating admin calls from this docs step, **no** Telegram bot calls from this docs step. **Operator-owned next:** (1) deploy latest commit; (2) run **`python -m alembic upgrade head`** / **`current`** on Railway; (3) run **read-only** checks; (4) only with **explicit** approval, perform **controlled** publish smoke per runbook. Handoff: **[`docs/HANDOFF_B13G_PRODUCTION_PUBLISH_AUDIT_SMOKE_RUNBOOK_TO_NEXT_STEP.md`](HANDOFF_B13G_PRODUCTION_PUBLISH_AUDIT_SMOKE_RUNBOOK_TO_NEXT_STEP.md)**. **Production smoke recorded (2026-05-10):** **[`docs/B13G_PRODUCTION_PUBLISH_AUDIT_SMOKE_RUNBOOK.md`](B13G_PRODUCTION_PUBLISH_AUDIT_SMOKE_RUNBOOK.md)** § Run log — 2026-05-10; **`alembic current`** **`20260531_29 (head)`** on Railway; Offer **#11** published via Admin API; audit attempt **persisted**, **`showcase_message_id` `23`**, **booking_link** still **missing** — **next:** execution link for Offer **11**; **rotate** **`ADMIN_API_TOKEN`** in Railway if exposed during smoke (see handoff **[`docs/HANDOFF_B13G_PRODUCTION_SMOKE_RESULT_TO_NEXT_STEP.md`](HANDOFF_B13G_PRODUCTION_SMOKE_RESULT_TO_NEXT_STEP.md)**).
- **B13G — full conversion smoke (2026-05-10, recorded):** **Offer #12** (*Excursie Timisoara Oradea*) **published** via Admin API; **showcase** post in channel; audit **attempt id 2** **`persisted`** (**`showcase_message_id` 25**). **Offer #11** run above was **publish-audit-only** (past-tour path; no execution link in that record). **Offer #12** **Tour #6** **`B10-SO12-04fb1f`**, **`open_for_sale`**, **`per_seat`**, seats **10**, **`execution_links_review.active_link.id` 5** **`active`**. Mini App: catalog + **`/supplier-offers/12`** show **bookable** / **open linked trip**; exact tour opens with seats/price. **Reservation preparation** still shows **tour is not available for reservation preparation** — **treat as Layer A readiness diagnostic**, **not** a B13 / execution-link failure. **Next:** **Tour #6** reservation-preparation readiness (e.g. **`MiniAppReservationPreparationService`** gates). Handoff: **[`docs/HANDOFF_B13G_FULL_CONVERSION_SMOKE_RESULT_TO_NEXT_STEP.md`](HANDOFF_B13G_FULL_CONVERSION_SMOKE_RESULT_TO_NEXT_STEP.md)**; **rotate** **`ADMIN_API_TOKEN`** in Railway after manual smoke if exposed.
- **B14A (completed, docs / diagnostic only):** **Read-only** trace for B13G follow-ups — **[`docs/B14A_CHANNEL_LINK_AND_RESERVATION_READINESS_DIAGNOSTIC.md`](B14A_CHANNEL_LINK_AND_RESERVATION_READINESS_DIAGNOSTIC.md)**. **Channel Rezervă URL:** **`build_showcase_publication`** → **`mini_app_supplier_offer_url`** (**`/supplier-offers/{id}`** only); **`/tours/{tour_code}`** helper exists but **is not** wired into showcase CTA. **Publish** path does **not** read execution links; **docs** (**runbook** + **conversion panel**) place **execution link after** showcase publish — **no implementation fix** claimed in B14A. **Prep 404:** **`GET /mini-app/tours/.../preparation`** maps **`MiniAppReservationPreparationService.get_preparation`** **`None`** → **`PrivateReservationPreparationService.get_preparable_tour`**; **leading hypothesis** empty **`boarding_points`** on **`per_seat`** Tour **#6** (tour **detail** read is **less strict** than prep). **No** code/tests/migrations/API/publish/link mutations in B14A.
- **B14C (implemented):** **Bridge-created** **new** tours materialize **`boarding_points`** from **`supplier_offers.boarding_places_text`** (**`SupplierOfferTourBridgeService._materialize_boarding_points_for_new_tour`**, **`parse_boarding_places`**) so **`per_seat`** tours can satisfy **`get_preparable_tour`** when offer had usable boarding text. **Not** for **`_link_existing`**; **no** DB migration backfill — **existing** prod **Tour #6** may still need ops boarding rows. Handoff: **[`docs/HANDOFF_B14C_TOUR_BOARDING_AND_PREP_ALIGNMENT_TO_NEXT_STEP.md`](HANDOFF_B14C_TOUR_BOARDING_AND_PREP_ALIGNMENT_TO_NEXT_STEP.md)**.
- **B14D (docs / ops runbook):** **Production-safe** remediation + prep smoke for **existing Tour #6** (**`B10-SO12-04fb1f`**) — read-only checks, **`POST /admin/tours/6/boarding-points`** (preferred), Mini App **`GET …/preparation`**; **no** execution-link / publish / booking mutations in the runbook itself. **[`docs/B14D_TOUR6_BOARDING_REMEDIATION_AND_PREP_SMOKE_RUNBOOK.md`](B14D_TOUR6_BOARDING_REMEDIATION_AND_PREP_SMOKE_RUNBOOK.md)** — **remediation not claimed executed** until operators run it. Handoff: **[`docs/HANDOFF_B14D_TOUR6_BOARDING_REMEDIATION_AND_PREP_SMOKE_TO_NEXT_STEP.md`](HANDOFF_B14D_TOUR6_BOARDING_REMEDIATION_AND_PREP_SMOKE_TO_NEXT_STEP.md)**.
- **B14E (read-only diagnostic, 2026-05-09):** Temporary holds **#52/#53** on **Tour #6** stayed **`active_temporary_hold`** with **`seats_available = 7`** after **`reservation_expires_at`** — **`ReservationExpiryService`** + worker / lazy paths documented; **no fix shipped in B14E** — see **[`docs/B14E_TEMP_HOLD_EXPIRY_RELEASE_DIAGNOSTIC.md`](B14E_TEMP_HOLD_EXPIRY_RELEASE_DIAGNOSTIC.md)** and **[`docs/HANDOFF_B14E_TEMP_HOLD_EXPIRY_RELEASE_DIAGNOSTIC_TO_NEXT_STEP.md`](HANDOFF_B14E_TEMP_HOLD_EXPIRY_RELEASE_DIAGNOSTIC_TO_NEXT_STEP.md)**.
- **B14F (implemented commit `58968a5` — `fix: persist lazy reservation expiry`):** Lazy expiry **persistence** — **`lazy_expire_due_reservations_commit_if_any`** on Mini App catalog/tour detail/bookings, reservation overview, payment-entry (before gate), **`get_preparable_tour`** (incl. private prep). **Admin read** still not an expiry executor. Handoff: **[`docs/HANDOFF_B14F_TEMP_HOLD_EXPIRY_PERSISTENCE_FIX_TO_NEXT_STEP.md`](HANDOFF_B14F_TEMP_HOLD_EXPIRY_PERSISTENCE_FIX_TO_NEXT_STEP.md)**. **B14G (production verification — PASS recorded):** **`GET …/preparation`** for **`B10-SO12-04fb1f`** — **#52/#53** → **`expired_unpaid_hold`**, Tour **#6** seats **7 → 10**; see **[`docs/B14G_RECORD_TEMP_HOLD_EXPIRY_FIX_PRODUCTION_RESULT.md`](B14G_RECORD_TEMP_HOLD_EXPIRY_FIX_PRODUCTION_RESULT.md)**. Handoff **#86** on **#53** still open (support). Handoff: **[`docs/HANDOFF_B14G_TEMP_HOLD_EXPIRY_FIX_PRODUCTION_VERIFICATION_TO_NEXT_STEP.md`](HANDOFF_B14G_TEMP_HOLD_EXPIRY_FIX_PRODUCTION_VERIFICATION_TO_NEXT_STEP.md)**.
- **B15A (completed, docs-only):** **Admin Publishing Console** + **Channel Publication** model — **Supplier Offer Admin** (product, moderation, bridge, catalog, execution link) stays separate from **Publishing Console** (today’s queue, preview/publish/skip/schedule, templates, conversion targets, safety policy). Extends beyond supplier-offer-only channel posts (tour promo, last seats, reminders, transport/RFQ, info) via **configurable templates** + **target registry**; slices **B15B–G**. **[`docs/B15_ADMIN_PUBLISHING_CONSOLE_DESIGN.md`](B15_ADMIN_PUBLISHING_CONSOLE_DESIGN.md)** — next: **[`docs/HANDOFF_B15A_ADMIN_PUBLISHING_CONSOLE_DESIGN_TO_NEXT_STEP.md`](HANDOFF_B15A_ADMIN_PUBLISHING_CONSOLE_DESIGN_TO_NEXT_STEP.md)**.
- **B15B (implemented):** **Read-only** **`GET /admin/publishing-console`** — candidate cards for **supplier offers** (via **`review-package`**) and **tours** (`open_for_sale` promotion slice); **`limit`/`kind`** query; **no** publish, schedule, Telegram send, execution-link/order/payment/reservation mutation, **no** CTA change. Spec: **[`docs/B15B_READ_ONLY_ADMIN_PUBLISHING_CONSOLE.md`](B15B_READ_ONLY_ADMIN_PUBLISHING_CONSOLE.md)**. Tests: **`tests/unit/test_admin_publishing_console.py`** (**8** **passed** **;** **B15D** **scenarios** **included**). Handoff: **[`docs/HANDOFF_B15B_READ_ONLY_ADMIN_PUBLISHING_CONSOLE_TO_NEXT_STEP.md`](HANDOFF_B15B_READ_ONLY_ADMIN_PUBLISHING_CONSOLE_TO_NEXT_STEP.md)**. **B15C (implemented):** supplier-offer showcase **Rezervă** gated on **active execution link** + bridged **catalog-listed** tour; exact **client** route **`/tours/{tour_code}`**; execution link allowed **before** **`published`** when safe — **[`docs/B15C_SUPPLIER_OFFER_EXACT_TOUR_CTA_GATE.md`](B15C_SUPPLIER_OFFER_EXACT_TOUR_CTA_GATE.md)**; handoff **[`docs/HANDOFF_B15C_SUPPLIER_OFFER_EXACT_TOUR_CTA_GATE_TO_NEXT_STEP.md`](HANDOFF_B15C_SUPPLIER_OFFER_EXACT_TOUR_CTA_GATE_TO_NEXT_STEP.md)**. **B15C1 (implemented):** channel **Rezervă** and admin **`showcase-preview`** **`cta_rezerva_href`** prefer **`t.me/{bot}?startapp=tour_{tour_code}`** (Main Mini App) when **`telegram_bot_username`** is set; **`assets/index.html`** applies **`start_param`** → **`/tours/{code}`** via **`history.replaceState`** before Flet loads so **Telegram WebApp** **`initData`** + **`tg_bridge_user_id`** injection still run; HTTPS **`{mini_app}/tours/{code}`** remains fallback when bot username missing. **B15C4 (implemented):** showcase **`sendPhoto`** **sendability** gate — **`telegram_photo:{file_id}`** or HTTPS URLs whose host is **not** a blocked share/page domain (**`share.google`**, Drive/Docs, etc.); **`POST …/media/approve-for-card`** rejects non-sendable refs; **`SupplierOfferModerationService.publish`** preflights **`photo_url`** before Telegram; **`POST …/media/clear-cover-for-text-only`** clears row **`cover_media_reference`** + records **`media_review.status=cover_cleared_for_channel_text_only`**; review-package **`cover_media_quality_review`** adds **`replacement_requested`**, **`supplier_action_needed`**, **`supplier_notice_message_ro`** when replacement is active; operator workflow **`clear_cover_for_text_only_channel`**. **B15C5 (implemented):** optional **`TELEGRAM_MINI_APP_SHORT_NAME`** — channel **Rezervă** **`startapp`** URLs use **`https://t.me/{bot}/{short}?startapp=tour_{code}`** when set; invalid short names are ignored (bare **`?startapp=`**); **`assets/index.html`** **`start_param`** mapping unchanged. **B15C2 (implemented):** Telegram supplier-offer admin (`app/bot/handlers/admin_moderation.py`) — after conversion workflow confirmations (**Leagă tur**, **În catalog**, **Link rezervări**, legacy wizard create/replace/close execution link, packaging approve/generate, media request/**OK poză**, showcase publish) and legacy **Approve / Retract / Reject**, the bot **re-syncs** FSM queue state to the **mutated** `supplier_offer_id` via **`_sync_offer_queue_after_mutation`** / **`_queue_state_after_offer_mutation`** so **Prev/Next** and the refreshed detail card stay on that offer when queue filters change; success copy notes the panel refresh. Tests: **`tests/unit/test_queue_state_after_offer_mutation.py`**. Handoff: **[`docs/HANDOFF_B15C2_ADMIN_NAV_STAY_ON_OFFER_AFTER_BRIDGE_TO_NEXT_STEP.md`](HANDOFF_B15C2_ADMIN_NAV_STAY_ON_OFFER_AFTER_BRIDGE_TO_NEXT_STEP.md)**. **B15C production smoke (PASS — operator-recorded, 2026):** Offer **#13** (*Excursie In Arad*), Tour **#7** / **`B10-SO13-e9f389`**, execution link **#6**, publish attempt **#3**, **`showcase_message_id` 26** captured **plain HTTPS** **Rezervă** (pre-B15C1); see **[`docs/B15C_PRODUCTION_SMOKE_RESULT.md`](B15C_PRODUCTION_SMOKE_RESULT.md)** §**10**. **B15C5 production smoke (PASS — operator-recorded, 2026-05-09):** Offer **#15** (*Test Direct Mini app link*), Tour **#9** **`B10-SO15-460344`**, execution link **#8**, publish attempt **#6**, **`showcase_message_id` 28**, **`cta_rezerva_href`** `https://t.me/tours_tm_bot/banattours?startapp=tour_B10-SO15-460344` — **Rezervă** opens **Mini App directly** (no private-bot launcher); exact tour; **no** identity warning; temporary hold **#55** + payment screen (**Layer A**); see **[`docs/B15C5_DIRECT_MINI_APP_LINK_PRODUCTION_SMOKE_RESULT.md`](B15C5_DIRECT_MINI_APP_LINK_PRODUCTION_SMOKE_RESULT.md)**. Handoffs: **[`docs/HANDOFF_B15C_PRODUCTION_SMOKE_RESULT_TO_NEXT_STEP.md`](HANDOFF_B15C_PRODUCTION_SMOKE_RESULT_TO_NEXT_STEP.md)**, **[`docs/HANDOFF_B15C1_MINI_APP_IDENTITY_FROM_CHANNEL_EXACT_TOUR_ENTRY_TO_NEXT_STEP.md`](HANDOFF_B15C1_MINI_APP_IDENTITY_FROM_CHANNEL_EXACT_TOUR_ENTRY_TO_NEXT_STEP.md)**, **[`docs/HANDOFF_B15C5_DIRECT_MINI_APP_LINK_PRODUCTION_SMOKE_RESULT_TO_NEXT_STEP.md`](HANDOFF_B15C5_DIRECT_MINI_APP_LINK_PRODUCTION_SMOKE_RESULT_TO_NEXT_STEP.md)**. **B15C3** copy polish when prioritized. **B15D** **+** **B15E** **+** **B15F** **on** **`GET /admin/publishing-console`** **are** **implemented** **(** **dedicated** **bullets** **below** **).** **Foundation closure:** **[`docs/B15_PUBLISHING_CONSOLE_FOUNDATION_CLOSURE_CHECKPOINT.md`](B15_PUBLISHING_CONSOLE_FOUNDATION_CLOSURE_CHECKPOINT.md)**. **Forward:** **B15F2** **/** **B15F3** **/** **B15E2** **(** **each** **only** **with** **explicit** **approval** **)** **/** **B15G** **(** **design** **approval** **)**.
- **B15D (implemented):** **Read-only** **`GET /admin/publishing-console`** — additive readiness, blockers, **B15C** exact-tour CTA safety, conversion target, next action/admin paths — **[`docs/B15D_ADMIN_PUBLISHING_CONSOLE_RICH_READ_VIEW.md`](B15D_ADMIN_PUBLISHING_CONSOLE_RICH_READ_VIEW.md)**; **no** mutation/send/scheduler. **Tests:** **`tests/unit/test_admin_publishing_console.py`** — **8** **passed.** **Handoff:** **[`docs/HANDOFF_B15D_ADMIN_PUBLISHING_CONSOLE_RICH_READ_VIEW_TO_NEXT_STEP.md`](HANDOFF_B15D_ADMIN_PUBLISHING_CONSOLE_RICH_READ_VIEW_TO_NEXT_STEP.md)**.
- **B15E (implemented):** **Same** **endpoint** **`actions[]`** — read-only affordances from **`operator_workflow`** **(** **supplier** **)** **and** **console** **/** **future** **hints** **(** **tours** **)** — **[`docs/B15E_ADMIN_PUBLISHING_CONSOLE_ACTION_AFFORDANCES_DESIGN.md`](B15E_ADMIN_PUBLISHING_CONSOLE_ACTION_AFFORDANCES_DESIGN.md)**; **no** **execution** **from** **this** **route.** **Tests:** **`tests/unit/test_admin_publishing_console.py`** — **8** **passed.** **Handoff:** **[`docs/HANDOFF_B15E_ADMIN_PUBLISHING_CONSOLE_ACTION_AFFORDANCES_DESIGN_TO_NEXT_STEP.md`](HANDOFF_B15E_ADMIN_PUBLISHING_CONSOLE_ACTION_AFFORDANCES_DESIGN_TO_NEXT_STEP.md)**.
- **B15F (implemented):** **Same** **endpoint** — additive **source/template/channel/media** metadata and **future-disabled** **`template_actions[]`** / **`channel_actions[]`** — **[`docs/B15F_PUBLISHING_CONSOLE_TEMPLATE_SOURCE_CHANNEL_READ_MODEL.md`](B15F_PUBLISHING_CONSOLE_TEMPLATE_SOURCE_CHANNEL_READ_MODEL.md)**; **read-only** **(** **no** **publish** **/** **send** **/** **template** **edit** **/** **channel** **select** **)**. **Tests:** **`tests/unit/test_admin_publishing_console.py`** — **8** **passed.** **Handoff:** **[`docs/HANDOFF_B15F_PUBLISHING_CONSOLE_TEMPLATE_SOURCE_CHANNEL_READ_MODEL_TO_NEXT_STEP.md`](HANDOFF_B15F_PUBLISHING_CONSOLE_TEMPLATE_SOURCE_CHANNEL_READ_MODEL_TO_NEXT_STEP.md)**. **B15 foundation handoff:** **[`docs/HANDOFF_B15_CLOSE_PUBLISHING_CONSOLE_FOUNDATION_CHECKPOINT_TO_NEXT_STEP.md`](HANDOFF_B15_CLOSE_PUBLISHING_CONSOLE_FOUNDATION_CHECKPOINT_TO_NEXT_STEP.md)**. **Next:** **B15F2** **/** **B15F3** **/** **B15E2** **only** **after** **explicit** **approval**, **or** **B15G** **design** **later**.
- **B16 Step 1** **+** **B16B** **+** **B16C** **(** **implemented** **):** **`GET /admin/ops-dashboard`** — **[`docs/B16_ADMIN_OPS_VISIBILITY_READ_DASHBOARD.md`](B16_ADMIN_OPS_VISIBILITY_READ_DASHBOARD.md)** **;** **tests** **`tests/unit/test_admin_ops_dashboard.py`** **11** **passed** **(** **with** **`test_admin_publishing_console.py`** **—** **19** **)** **.** **B16D1** **(** **implemented** **):** **`GET /admin/supplier-offers/{offer_id}/prepare-conversion-chain/plan`** read-only **prepare_conversion_chain** **plan** **preview** **;** **B16D1.1** **(** **implemented** **):** **`prepare_conversion_chain_plan_path`** **on** **review-package** **,** **plan** **JSON** **,** **publishing** **console** **(** **supplier-offer** **rows** **)**, **ops** **dashboard** **(** **publication** **/** **conversion** **/** **attention** **)** **;** **B16D1.2** **(** **implemented** **):** **`prepare_conversion_chain_plan_status`** **(** **ineligible** **/** **blocked** **/** **partial** **/** **already_prepared** **)**, **`prepare_conversion_chain_recommended_action`** **,** **`prepare_conversion_chain_blockers_count`** **same** **supplier-offer** **surfaces** **+** **plan** **JSON** **;** **tests** **focused** **suite** **31** **passed** **;** **`AdminOpsDashboardService`** **import** **restored** **in** **`admin.py`**. **Handoff** **[`docs/HANDOFF_B16_ADMIN_OPS_VISIBILITY_READ_DASHBOARD_TO_NEXT_STEP.md`](HANDOFF_B16_ADMIN_OPS_VISIBILITY_READ_DASHBOARD_TO_NEXT_STEP.md)** **;** **forward** **B16D2**–**B16E**.
- **B12/B13.3** **—** **`sendMessage`** **with** **`disable_web_page_preview=True`** **for** **text-only** **showcase** **(no** **Flet/Railway** **link** **preview** **card** **).** **B12/B13.4** **—** **read-only** **admin** **`GET /admin/supplier-offers/{id}/showcase-preview`** **(exact** **`caption_html`** **+** **CTA** **URLs** **+** **flags** **;** **no** **Telegram** **I/O** **).** **B12/B13.4A** **`lifecycle_status`** **/** **`can_publish_now`** **/** **`warnings`** **on** **preview** **response** **.** **B12/B13.5–B13.6** **(docs)** **—** **recommended** **`preview` → verify → `publish`** **ops** **procedure** **(not** **code-enforced** **);** **`can_publish_now`** **=** **technical** **readiness** **only** **;** **no** **DB** **preview** **tracking** **/** **`preview_hash`** **in** **MVP** **—** **[`docs/ADMIN_SHOWCASE_PUBLISH_RUNBOOK.md`](ADMIN_SHOWCASE_PUBLISH_RUNBOOK.md)** **.** **B13.6A** **(docs)** **—** **three-layer** **note** **(raw** **input** **/** **AI** **marketing** **draft** **/** **deterministic** **`showcase-preview`** **output** **;** **AI** **not** **auto-trusted** **at** **publish** **)** **in** **same** **runbook** **.**
...
### Next safe step
**BUSINESS line (supplier offer → tour, B1 design accepted, B2 data slice completed):** next implementation track step is **B3 — Supplier dialog upgrade** — see **`docs/SUPPLIER_OFFER_TO_TOUR_BUSINESS_PLAN.md`**, **`docs/SUPPLIER_OFFER_INTAKE_AI_PACKAGING_MODERATION_DESIGN.md`**, §**3** **(dialog)**, and **`docs/HANDOFF_B2_SUPPLIER_OFFER_CONTENT_DATA_UPGRADE.md`**. (This is **independent** of the historical **Y**-numbered execution/ops checkpoints unless explicitly merged in a ticket.)
