...
**Continuity anchor (read this first):** The **live** approved checkpoint is **Phase 7 / Step 5 completed** — **narrow group runtime** (**`app/bot/handlers/group_gating.py`**, **`app/services/group_chat_gating.py`**, **`app/services/group_private_cta.py`**): **group/supergroup text only**; **non-trigger** → **no reply**; **trigger** → **one** reply line: **trigger gating** + **short safe base text** + **narrow category-aware escalation wording** (Step **4**) + **private CTA / deep-link foundation** (**`t.me/...?start=grp_private`** / **`grp_followup`**, Step **5**); **no** handoff DB rows, **no** operator assignment, **no** sensitive booking/payment handling in group; **`TELEGRAM_BOT_USERNAME`** unset → **silent** in groups. **Next:** **Phase 7 / Step 6** — **narrow private `/start` branching for `grp_private` / `grp_followup`** — see **Next Safe Step**. **Phase 6** / **`docs/PHASE_6_REVIEW.md`** — historical admin closure. **Not** **Phase 6 / Step 31**, **not** **admin payment** mutation. **Phase 5** — do **not** resume old **Step 4–5** narratives. Use **`Next Safe Step`** + **Current Status**.

## Current Status
The **latest approved checkpoint** is **Phase 7 / Step 5 completed** (**documentation + code checkpoint aligned**). **Agreed group runtime semantics:** scope is **group / supergroup text** only; **trigger gating** (`evaluate_group_trigger`); if **no** trigger → **no** bot reply; if trigger → **one** reply line: **base text** (default ack **or** Step **4** category line from **`evaluate_handoff_triggers`**) **plus** a consistent **`https://t.me/<bot>?start=`** link — **`grp_private`** for generic triggers, **`grp_followup`** when a handoff category shaped the base line (**`app/services/group_private_cta.py`**); **no** prices, **no** invented facts, **no** promise that a human was already assigned; **`TELEGRAM_BOT_USERNAME`** not configured → bot **stays silent** in groups (mention path inactive). Private **`/start`** does **not** branch on **`grp_*`** in this step (payloads fall through like other non-**`tour_`** args). **Step 1** rules: **`docs/GROUP_ASSISTANT_RULES.md`**. **`create_dispatcher`** includes **`group_gating`** router. **Public** booking / payment / waitlist / Mini App **flows and APIs did not change**.

**Still not implemented (explicit):** **actual** handoff **persistence**; **operator workflow engine**; **full** group assistant behavior; **narrow private `/start` branching** on **`grp_private`** / **`grp_followup`** (Step **6** — see **Next Safe Step**; **not** in Step **5**); **full** generalized deep-link/campaign router; **booking/payment resolution** or other sensitive handling **in group**; **long-form** group replies; **public** booking/payment/waitlist/Mini App **behavior changes**. Steps **4–5** do **not** create handoff rows or assign operators.
...
**Next work:** **Phase 7 / Step 6** — **narrow private `/start` branching for `grp_private` / `grp_followup`** (see **Next Safe Step**). **Later** (not Step **6** by default): i18n group CTAs, rate limits, handoff persistence — product-scoped. **Admin payment mutations** are **not** the default.
...
### Not Implemented Yet
- **Next (Phase 7):** **Phase 7 / Step 6** — **narrow private `/start` branching for `grp_private` / `grp_followup`** — see **Next Safe Step** (i18n, rate limits, handoff persistence remain **later**, separate checkpoints).
- **Group / handoff:** Steps **3–5** = **trigger gating + short safe reply + category-aware wording (Step 4) + private CTA/deep-link foundation (Step 5)**; **real handoff persistence** from group / **operator** engine / **full** assistant — **future** — **`docs/OPEN_QUESTIONS_AND_TECH_DEBT.md`** §**19**.
...
- **Phase 7–9 (beyond Step 6):** optional group assistant at scale, content assistant, analytics — per `docs/IMPLEMENTATION_PLAN.md`

## Next Safe Step

**Primary (single outcome):** **Phase 7 / Step 6** — **narrow private `/start` branching for `grp_private` / `grp_followup`**.

- **Why:** Step **5** emits deep links with these **`start`** payloads; private **`/start`** still treats them like any non-**`tour_`** arg (catalog fallback). The **smallest** next increment is to **recognize** **`grp_private`** and **`grp_followup`** **narrowly** in **`handle_start`** (e.g. one short safe line or routing hint **before** catalog) — **no** booking/payment mutations, **no** handoff DB, **no** operator assignment, **no** Mini App changes unless explicitly scoped.
- **Out of scope for Step 6 unless rescoped:** **i18n** group CTAs, **rate limits**, **handoff persistence** — separate future steps.
- **Baseline (Steps 3–5):** **`app/bot/handlers/group_gating.py`**, **`app/services/group_chat_gating.py`**, **`app/services/group_private_cta.py`**, **`app/schemas/group_private_cta.py`**; private entry **`app/bot/handlers/private_entry.py`** (`CommandStart`) is the **likely** touch point for Step **6**.
...
**Staging (recommended):** after Step **6**: open bot from group link with **`grp_private`** / **`grp_followup`** → **expected** narrow behavior; **`/start tour_*`** unchanged; **public** APIs unchanged.
...
**Completed step references:** `docs/CURSOR_PROMPT_PHASE_6_STEP_1.md` … `docs/CURSOR_PROMPT_PHASE_6_STEP_30.md` (historical); **`docs/CURSOR_PROMPT_PHASE_7_STEP_1.md`** … **`docs/CURSOR_PROMPT_PHASE_7_STEP_5.md`** (historical).  
**Plan:** `docs/IMPLEMENTATION_PLAN.md` — Phase 7.

## Recommended Next Prompt
Implement **Phase 7 / Step 6** — **narrow private `/start` branching for `grp_private` / `grp_followup`** (see **Next Safe Step**): minimal handling in private **`/start`** aligned with **`docs/TELEGRAM_SETUP.md`**; **no** handoff persistence, **no** booking/payment expansion, **no** Mini App runtime changes unless scoped. Sources: **`docs/CHAT_HANDOFF.md`**, **`docs/GROUP_ASSISTANT_RULES.md`**, **`docs/TELEGRAM_SETUP.md`**, `docs/IMPLEMENTATION_PLAN.md`, `docs/OPEN_QUESTIONS_AND_TECH_DEBT.md`, `docs/TESTING_STRATEGY.md`.
...
- **group chat:** Phase **7 / Steps 3–5** — **trigger gating + reply line** with **`t.me`** CTA (`group_gating` handler); Steps **4–5** = category-aware base text + **`grp_private`** / **`grp_followup`** deep link — **not** full group assistant, **not** handoff rows from this path (see **`docs/GROUP_ASSISTANT_RULES.md`**)
...
- **Approved checkpoint:** **Phase 7 / Step 5** complete — **trigger gating** + **short safe reply** + **narrow category-aware escalation wording (Step 4)** + **private CTA/deep-link foundation** (**`grp_private`** / **`grp_followup`**, Step **5**); non-trigger **silent**; **no** handoff DB, **no** operator assignment, **no** booking/payment resolution in group; **private `/start`** does **not** branch on **`grp_*`** yet; **`TELEGRAM_BOT_USERNAME`** required for full gating. **Forward:** **Phase 7 / Step 6** — **narrow private `/start` branching for `grp_private` / `grp_followup`** (see **Next Safe Step**). Phase 6 closure: **`docs/PHASE_6_REVIEW.md`**. **Not** old **Phase 5 / Step 4–5** checkpoints; **not** admin payment mutation or **Phase 6 / Step 31** unless scoped.
...
Default forward path is **Phase 7 / Step 6** — **narrow private `/start` branching** for **`grp_*`** payloads — do not use legacy Phase 5 “next step” prompts for new implementation slices.
