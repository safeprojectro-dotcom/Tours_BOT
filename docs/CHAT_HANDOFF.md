...
**Slice C2B — design accepted (docs-only; implementation = planned C2B1):** next mutating Telegram slice is **packaging-only** — **`approve_packaging_for_publish`** from **`operator_workflow.actions`**, only when **`enabled`**, **mandatory two-step confirmation**, **re-read `GET …/review-package` before execution**, execute **only if still enabled**, **no hidden chains**, **no batch workflow**. Legacy **Aprobă / Respinge** remain moderation (**do not** add **`approve_offer_moderation`** via workflow until consolidation). **Postponed:** **`publish_showcase_channel`**, **`activate_tour_for_catalog`**, **`create_execution_link`**, **`create_tour_bridge`**.
