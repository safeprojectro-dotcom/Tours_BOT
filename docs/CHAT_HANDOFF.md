...
**Continuity anchor (read this first):** The **live** approved checkpoint is **Phase 7 / Step 3 completed** — **minimal group runtime gating** (**`app/bot/handlers/group_gating.py`**, **`app/services/group_chat_gating.py`**): **group/supergroup text only**; **non-trigger** → **no reply**; **mention / approved command / approved trigger phrase** → **one** short safe ack (**no** invented facts, **no** long conversation); **`TELEGRAM_BOT_USERNAME`** unset → **silent** in groups; **`evaluate_handoff_triggers`** **not** wired in runtime yet. **Next:** **Phase 7 / Step 4** — **narrow handoff trigger evaluation / escalation recommendation hookup only** — see **Next Safe Step**. **Phase 6** / **`docs/PHASE_6_REVIEW.md`** — historical admin closure. **Not** **Phase 6 / Step 31**, **not** **admin payment** mutation. **Phase 5** — do **not** resume old **Step 4–5** narratives. Use **`Next Safe Step`** + **Current Status**.

## Current Status
The **latest approved checkpoint** is **Phase 7 / Step 3 completed** (**documentation + code checkpoint aligned**). **Agreed Step 3 runtime semantics:** scope is **group / supergroup text** only; if **group trigger** does **not** fire → **no** bot reply (**ignored**); if it fires → **one** short safe acknowledgment only (**`GROUP_TRIGGER_ACK_REPLY_TEXT`** — private / Mini App CTA, **no** booking/payment **sales** in group, **no** invented facts, **no** long-form chat); **`TELEGRAM_BOT_USERNAME`** not configured → bot **stays silent** in groups (mention path inactive); **`evaluate_handoff_triggers`** is **not** yet in the group runtime path. **Step 1** rules: **`docs/GROUP_ASSISTANT_RULES.md`**. **Step 2** helpers unchanged. **`create_dispatcher`** includes **`group_gating`** router. **Public** booking / payment / waitlist / Mini App **flows and APIs did not change**.

**Still not implemented (explicit):** **full** group assistant; **handoff persistence** from group runtime; **operator workflow engine**; **long-form** group replies; wiring **handoff trigger evaluation** into runtime (**Step 4** — see **Next Safe Step**); **public** customer-flow redesign.
...
**Next work:** **Phase 7 / Step 4** — **narrow runtime hookup for handoff trigger evaluation / escalation recommendation only** (see **Next Safe Step**); **not** operator engine, **not** automatic handoff DB writes unless explicitly scoped. **Admin payment mutations** are **not** the default.
...
### Not Implemented Yet
- **Next (Phase 7):** **Phase 7 / Step 4** — **narrow** runtime use of **`evaluate_handoff_triggers`** (and/or **`evaluate_assistant_trigger_snapshot`**) for **escalation recommendation / structured signal** in the **group** path — **without** handoff row creation, **without** full operator workflow — see **Next Safe Step**; optional later: i18n ack, rate limits (product-scoped).
- **Group / handoff:** Step **3** = **gating + one ack** only; **handoff** persistence / **operator** engine / **full** assistant — **future** — **`docs/OPEN_QUESTIONS_AND_TECH_DEBT.md`** §**19**.
...
- **Phase 7–9 (beyond Step 4):** group assistant at scale, content assistant, analytics — per `docs/IMPLEMENTATION_PLAN.md`

## Next Safe Step

**Primary (single outcome):** **Phase 7 / Step 4** — **narrow runtime hookup for handoff trigger evaluation / escalation recommendation only**.

- **Rationale:** Step **2** added **`evaluate_handoff_triggers`**; Step **3** wired only **`evaluate_group_trigger`**. The **smallest** next increment is to invoke handoff evaluation **in the same narrow group path** (e.g. after trigger fires, or in parallel with gating — **product-scoped**) to surface **structured** **`handoff_required` / `handoff_category`** for logging, reply shaping, or a **single** extra safe line — **without** creating **handoff** DB rows, **without** **operator workflow engine**, **without** long-form replies or booking/payment handling in group.
- **Baseline (Step 3):** **`app/bot/handlers/group_gating.py`**, **`app/services/group_chat_gating.py`**.
- **Still out of scope unless explicitly scoped:** **automatic handoff persistence**, full **CTA** pipeline, **multilingual** ack (can be a **separate** small slice if preferred).
...
**Staging (recommended):** confirm Step **3** behaviors in a test **group** (silence vs ack); then validate Step **4** handoff signal wiring **without** DB side effects.

### Goal
Use **Step 2** handoff helpers **in runtime** for **recommendation / classification** only — **incremental** path toward operator handoff, not a full inbox.
...
**Completed step references:** `docs/CURSOR_PROMPT_PHASE_6_STEP_1.md` … `docs/CURSOR_PROMPT_PHASE_6_STEP_30.md` (historical); **`docs/CURSOR_PROMPT_PHASE_7_STEP_1.md`** … **`docs/CURSOR_PROMPT_PHASE_7_STEP_3.md`** (historical).  
**Plan:** `docs/IMPLEMENTATION_PLAN.md` — Phase 7.

## Recommended Next Prompt
Implement **Phase 7 / Step 4**: **narrow** **group** runtime hookup for **`evaluate_handoff_triggers`** (**escalation recommendation / structured signal** only; **no** handoff DB, **no** operator engine). Keep replies **short** and aligned with **`docs/GROUP_ASSISTANT_RULES.md`**. Sources: **`docs/CHAT_HANDOFF.md`**, **`docs/GROUP_ASSISTANT_RULES.md`**, `docs/IMPLEMENTATION_PLAN.md`, `docs/OPEN_QUESTIONS_AND_TECH_DEBT.md`, `docs/TESTING_STRATEGY.md`, `docs/AI_ASSISTANT_SPEC.md`, `docs/AI_DIALOG_FLOWS.md`, `docs/TELEGRAM_SETUP.md`.
...
- **group chat:** Phase **7 / Step 3** — **trigger gating + one short ack** only (`group_gating` handler); **not** full group assistant (see **`docs/GROUP_ASSISTANT_RULES.md`**)
...
- **Approved checkpoint:** **Phase 7 / Step 3** complete — **minimal group gating** (non-trigger **silent**; trigger → **one** short ack; **`TELEGRAM_BOT_USERNAME`** required for full gating; **`evaluate_handoff_triggers`** **not** in runtime yet). **Forward:** **Phase 7 / Step 4** — **handoff trigger evaluation / escalation recommendation hookup only** (see **Next Safe Step**). Phase 6 closure: **`docs/PHASE_6_REVIEW.md`**. **Not** old **Phase 5 / Step 4–5** checkpoints; **not** admin payment mutation or **Phase 6 / Step 31** unless scoped.
...
Default forward path is **Phase 7 / Step 4** (handoff evaluation in **narrow** group runtime — **recommendation only**, **no** persistence by default) — do not use legacy Phase 5 “next step” prompts for new implementation slices.
