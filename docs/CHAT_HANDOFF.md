...
**B15C exact CTA chain — closure checkpoint (B15C6, docs):** Stable baseline after exact-tour **Rezervă**, execution-link-before-publish, short-name **`startapp`**, cover guard, admin nav, and copy alignment — [`docs/B15C_EXACT_CTA_CHAIN_CLOSURE_CHECKPOINT.md`](B15C_EXACT_CTA_CHAIN_CLOSURE_CHECKPOINT.md). **B15D (implemented):** richer read-only **`GET /admin/publishing-console`** — [`docs/B15D_ADMIN_PUBLISHING_CONSOLE_RICH_READ_VIEW.md`](B15D_ADMIN_PUBLISHING_CONSOLE_RICH_READ_VIEW.md). **B15E (implemented):** same endpoint adds **`actions[]`** affordances — [`docs/B15E_ADMIN_PUBLISHING_CONSOLE_ACTION_AFFORDANCES_DESIGN.md`](B15E_ADMIN_PUBLISHING_CONSOLE_ACTION_AFFORDANCES_DESIGN.md). **B15F (implemented):** same endpoint adds **source/template/channel/media** read metadata and future-disabled **`template_actions`** / **`channel_actions`** hints — [`docs/B15F_PUBLISHING_CONSOLE_TEMPLATE_SOURCE_CHANNEL_READ_MODEL.md`](B15F_PUBLISHING_CONSOLE_TEMPLATE_SOURCE_CHANNEL_READ_MODEL.md); endpoint remains **read-only**. **Tests:** **`tests/unit/test_admin_publishing_console.py`** — **8 passed.** **Next:** **B15F2** / **B15F3** / **B15E2** only after **explicit approval**, or **B15G** design later; [`docs/HANDOFF_B15F_PUBLISHING_CONSOLE_TEMPLATE_SOURCE_CHANNEL_READ_MODEL_TO_NEXT_STEP.md`](HANDOFF_B15F_PUBLISHING_CONSOLE_TEMPLATE_SOURCE_CHANNEL_READ_MODEL_TO_NEXT_STEP.md).
