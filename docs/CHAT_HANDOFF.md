...
**Continuity anchor (read this first):** The **live** approved checkpoint is **Phase 7 / Step 9 completed** — **Phase 7** surface to date: **group trigger** + **handoff trigger** helpers (Step **2**); **minimal group runtime gating** (Step **3**); **category-aware** short escalation wording (Step **4**); **private CTA** deep-link foundation (Step **5**); **narrow private `/start`** for **`grp_private`** / **`grp_followup`** (Step **6** — two **distinct** short intros + catalog; **`tour_*`** and other legacy **`/start`** payloads **unchanged**); **narrow runtime handoff persistence** for **`/start grp_followup`** only (Step **7** — reason **`group_followup_start`**, dedupe-safe for **open** rows, **no** persistence for **`grp_private`**); **focused runtime/bot-flow test coverage** for that **`grp_followup`** chain (Step **8**); **narrow admin read-side visibility** for **`group_followup_start`** (Step **9** — derived **`is_group_followup`**, **`source_label`** on **`GET /admin/handoffs`**, **`GET /admin/handoffs/{id}`**, and **order-detail** handoff summaries; **read-only**; **no** write semantics change). **No** automatic operator assignment **from the Telegram `grp_*` bot paths**, **no** booking/payment initiation from Phase **7** **`grp_*`** paths. **Next:** **Phase 7 / Step 10** — **narrow operator assignment** for **`group_followup_start`** handoffs **only** — see **Next Safe Step**. **Phase 6** / **`docs/PHASE_6_REVIEW.md`** — historical admin closure. **Not** **Phase 6 / Step 31**, **not** **admin payment** mutation. **Phase 5** — do **not** resume old **Step 4–5** narratives. Use **`Next Safe Step`** + **Current Status**.

## Current Status
The **latest approved checkpoint** is **Phase 7 / Step 9 completed** (**documentation + code checkpoint aligned**). **Phase 7 implemented surface (Steps 1–9):** Step **1** — **`docs/GROUP_ASSISTANT_RULES.md`**; Step **2** — **group** + **handoff** trigger helpers (`evaluate_group_trigger`, `evaluate_handoff_triggers`, …); Step **3** — **minimal group runtime gating**; Step **4** — **category-aware** short escalation wording in group replies; Step **5** — **private CTA** **`t.me`** deep-link foundation (`grp_private` / `grp_followup`); Step **6** — **narrow private `/start`** branching: **`grp_private`** → short **`start_grp_private_intro`**; **`grp_followup`** → **different** short **`start_grp_followup_intro`**; then **`_send_catalog_overview`**; **`/start tour_*`** and other legacy **`/start`** behavior **unchanged**; Step **7** — **narrow** **`handoffs`** persistence on **`/start grp_followup`** only (**reason** **`group_followup_start`**; **dedupe** for repeated **`/start grp_followup`** while an **open** matching row exists; **new** row allowed after **closed**; **`grp_private`** — **no** handoff write); Step **8** — **focused runtime/bot-flow tests** for the **`grp_followup`** chain (**`tests/unit/test_private_entry_grp_followup_chain.py`**) — validates **`handle_start`** path, intro, persistence, dedupe, **`grp_private`** no **`group_followup_start`** row, legacy **`tour_*`** unchanged; Step **9** — **narrow admin read-side visibility** for **`group_followup_start`**: **`is_group_followup`**, **`source_label`** on admin handoff **list**, **detail**, and **order-detail** handoff summaries (**`compute_group_followup_visibility`** in **`admin_handoff_queue`**); **read-only**; **no** mutation / booking / payment / public flow changes. **No** automatic operator assignment **from Telegram `grp_*` paths**; **no** booking/payment initiation from Phase **7** **`grp_*`** paths. **Step 1** rules: **`docs/GROUP_ASSISTANT_RULES.md`**. **Public** booking / payment / waitlist / Mini App **flows unchanged** by Step **9**.

**Still not implemented (explicit):** **next** documented slice — **Phase 7 / Step 10** — **narrow operator assignment** for **`group_followup_start`** handoffs **only** (see **Next Safe Step**); **broader** **operator workflow engine**, **internal ops** **claim** expansion, **full** group assistant, **full** private “assistant” flow redesign; **booking/payment resolution** driven by the Phase **7** group/private **`grp_*`** chain beyond existing private/Mini App flows; **long-form** group replies; **public** booking/payment/waitlist/Mini App **behavior changes**; **handoff** rows from **group** chat runtime (Steps **3–4** remain **reply-only**). **Live** Telegram validation **may** still depend on **Railway** / bot deploy health — **product scope** for Phase **7** remains as documented.
...
**Next work:** **Phase 7 / Step 10** — **narrow operator assignment** for **`group_followup_start`** handoffs **only** (see **Next Safe Step**): smallest safe **assign** slice after persistence + tests + read labels — **not** broad workflow or **internal ops** claim — **not** admin payment work. **i18n** / **rate limits** remain **separate** optional slices unless bundled by product. **Admin payment mutations** are **not** the default.
...
### Not Implemented Yet
- **Next (Phase 7):** **Phase 7 / Step 10** — **narrow operator assignment** for **`group_followup_start`** handoffs **only** — see **Next Safe Step**. Optional **later** slices: i18n polish, group **rate limits**.
- **Group / handoff:** Steps **1–9** = **rules + helpers + group gating + escalation wording + deep links + private `/start`** + **narrow** **`handoffs`** write on **`/start grp_followup`** (**Step 7**) + **focused tests** (**Step 8**) + **admin read visibility** for **`group_followup_start`** (**Step 9**); **group** chat path (Steps **3–4**) still **no** DB handoff rows — **`docs/OPEN_QUESTIONS_AND_TECH_DEBT.md`** §**19**.
...
- **Longer-term (beyond Phase 7 / Step 10 checkpoint):** optional group assistant at scale, content assistant, analytics — per `docs/IMPLEMENTATION_PLAN.md`

## Next Safe Step

**Primary (single outcome):** **Phase 7 / Step 10** — **narrow operator assignment** for **`group_followup_start`** handoffs **only** (*smallest safe* path: scoped **admin** assignment behavior or endpoint rules **limited** to **`reason=group_followup_start`** — **not** a full workflow engine, **not** **internal ops** claim expansion).

- **Why:** Step **7** — persistence; Step **8** — runtime chain tests; Step **9** — **read** labels (**`is_group_followup`**, **`source_label`**). The **next smallest increment** is **one narrow assignment slice** tied to this reason — still **no** broad triage redesign.
- **In scope (typical):** explicit, minimal rules for **who may assign** / **how** **`group_followup_start`** rows get **`assigned_operator_id`** (may extend existing **`POST /admin/handoffs/{id}/assign`** behavior **only** for this reason, or add a parallel narrow path — **product/impl choice** in Step **10** prompt); **no** booking/payment mutations; **no** public API expansion beyond admin.
- **Out of scope for Step 10 unless rescoped:** **full** operator workflow engine, **internal ops** **`/internal/ops/handoffs/*`** claim takeover redesign, **admin payment** mutations, **Mini App** / **public** customer flow changes, **broad** refactors.
- **Baseline (Steps 1–9):** as Step **9** plus **`compute_group_followup_visibility`**, admin handoff schemas — **`docs/CHAT_HANDOFF.md`** **Completed Steps**.
...
**Staging (recommended):** smoke **`POST /admin/handoffs/{id}/assign`** (or Step **10**-scoped equivalent) on a **`group_followup_start`** row — **Railway** / deploy health as usual.
...
**Completed step references:** `docs/CURSOR_PROMPT_PHASE_6_STEP_1.md` … `docs/CURSOR_PROMPT_PHASE_6_STEP_30.md` (historical); **`docs/CURSOR_PROMPT_PHASE_7_STEP_1.md`** … **`docs/CURSOR_PROMPT_PHASE_7_STEP_9.md`** (historical).  
**Plan:** `docs/IMPLEMENTATION_PLAN.md` — Phase 7.

## Recommended Next Prompt
Implement **Phase 7 / Step 10** — **narrow operator assignment** for **`group_followup_start`** handoffs **only** (see **Next Safe Step**): smallest safe **assign** slice — **scoped** to **`reason=group_followup_start`**, **no** broad workflow engine, **no** **internal ops** claim redesign, **no** public/Mini App churn unless scoped. Sources: **`docs/CHAT_HANDOFF.md`**, **`docs/GROUP_ASSISTANT_RULES.md`**, **`docs/TELEGRAM_SETUP.md`**, **`docs/TECH_SPEC_TOURS_BOT.md`**, `docs/IMPLEMENTATION_PLAN.md`, `docs/OPEN_QUESTIONS_AND_TECH_DEBT.md`, `docs/TESTING_STRATEGY.md`.
...
- **Approved checkpoint:** **Phase 7 / Step 9** complete — **Phase 7** surface through **Step 9** (Steps **1–8** as before, plus Step **9** **admin read-side** **`is_group_followup`** / **`source_label`** for **`group_followup_start`** on handoff list, detail, and order-detail summaries; **no** write semantics change); **`grp_private`** unchanged; **`tour_*`** / legacy **`/start`** unchanged; **no** automatic assignment from **Telegram `grp_*`** entry; **`TELEGRAM_BOT_USERNAME`** required for full group gating. **Forward:** **Phase 7 / Step 10** — **narrow operator assignment** for **`group_followup_start`** handoffs **only** (see **Next Safe Step**). Phase 6 closure: **`docs/PHASE_6_REVIEW.md`**. **Not** old **Phase 5 / Step 4–5** checkpoints; **not** admin payment mutation or **Phase 6 / Step 31** unless scoped.
...
Default forward path is **Phase 7 / Step 10** — **narrow operator assignment** for **`group_followup_start`** **only** — do not use legacy Phase 5 “next step” prompts for new implementation slices.
