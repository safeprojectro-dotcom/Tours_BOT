...
- **Next safe order:**
  1. **Y38–Y42 (docs) + Y43 (persistence) —** Design gates: **`docs/SUPPLIER_INTERACTION_GATE.md`** … **`docs/SUPPLIER_EXECUTION_PERMISSION_AUDIT_GATE.md`**. **Y43** = **`20260502_21`** + ORM + **`supplier_execution` repository**; **no** **execution** runtime, **no** **API** yet. Next **safe** in-app: **separate** ticket for **start** of execution (entry + permission + Y42) **or** read-only **admin** of execution rows **only** if gated. **no** **messaging** / RFQ / booking / Mini App / links / identity / customer **notifications** unless a slice **explicitly** includes them.
