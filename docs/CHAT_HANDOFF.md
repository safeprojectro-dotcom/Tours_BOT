...
**Continuity anchor (read this first):** The **live** approved checkpoint is **Phase 7 / Step 6 completed** — **Phase 7** surface to date: **group trigger** + **handoff trigger** helpers (Step **2**); **minimal group runtime gating** (Step **3**); **category-aware** short escalation wording (Step **4**); **private CTA** deep-link foundation (Step **5**); **narrow private `/start`** for **`grp_private`** / **`grp_followup`** (Step **6** — intro + catalog, **`tour_*`** legacy payloads unchanged). **No** runtime handoff DB rows from Steps **4–6**, **no** operator assignment, **no** booking/payment initiation from these paths. **Next:** **Phase 7 / Step 7** — **narrow runtime handoff persistence foundation** (payload / intent → persisted handoff row — **not** full operator workflow) — see **Next Safe Step**. **Phase 6** / **`docs/PHASE_6_REVIEW.md`** — historical admin closure. **Not** **Phase 6 / Step 31**, **not** **admin payment** mutation. **Phase 5** — do **not** resume old **Step 4–5** narratives. Use **`Next Safe Step`** + **Current Status**.

## Current Status
The **latest approved checkpoint** is **Phase 7 / Step 6 completed** (**documentation + code checkpoint aligned**). **Phase 7 implemented surface (Steps 1–6):** Step **1** — **`docs/GROUP_ASSISTANT_RULES.md`**; Step **2** — **group** + **handoff** trigger helpers (`evaluate_group_trigger`, `evaluate_handoff_triggers`, …); Step **3** — **minimal group runtime gating**; Step **4** — **category-aware** short escalation wording in group replies; Step **5** — **private CTA** **`t.me`** deep-link foundation (`grp_private` / `grp_followup`); Step **6** — **narrow private `/start`** branching: **`grp_private`** / **`grp_followup`** → distinct short **`start_grp_*_intro`** + **`_send_catalog_overview`**; **`/start tour_*`** and other legacy **`/start`** behavior **unchanged**. **No** new persistence or workflow side effects in Steps **4–6**; **no** operator assignment; **no** booking/payment initiation from these paths. **Step 1** rules: **`docs/GROUP_ASSISTANT_RULES.md`**. **Public** booking / payment / waitlist / Mini App **flows and APIs did not change**.

**Still not implemented (explicit):** **real** handoff **persistence** from the **Phase 7** bot runtime chain (Steps **4–6** do **not** write handoff rows); **operator workflow engine**; **full** group assistant; **full** private “assistant” flow redesign; **booking/payment resolution** or initiation **driven by** the Phase **7** group/private **`grp_*`** chain (beyond existing private/Mini App flows); **long-form** group replies; **public** booking/payment/waitlist/Mini App **behavior changes**. **Live** Telegram validation **may** still depend on **Railway** / bot deploy health — **product scope** for Phase **7** remains as documented.
...
**Next work:** **Phase 7 / Step 7** — **narrow runtime handoff persistence foundation** (see **Next Safe Step**): first persisted **handoff intent** from selected runtime signals (e.g. **`grp_followup`** / category mapping) — **without** full operator inbox; **i18n** / **rate limits** remain **separate** optional slices unless bundled by product. **Admin payment mutations** are **not** the default.
...
- Phase 7 / Step 6 completed
  - **Narrow private `/start` for `grp_private` / `grp_followup`** — **`handle_start`** (**`app/bot/handlers/private_entry.py`**): after language resolved, **`match_group_cta_start_payload`**; intro via **`start_grp_private_intro`** / **`start_grp_followup_intro`** (**`app/bot/messages.py`**); then **`_send_catalog_overview`**; **`tour_*`** path unchanged
...
- **Phase 7 / Step 6 (private `/start` for `grp_*`)** — **`app/bot/handlers/private_entry.py`** + **`match_group_cta_start_payload`**; **tests** `test_group_private_cta`, `test_private_start_grp_messages`
...
### Not Implemented Yet
- **Next (Phase 7):** **Phase 7 / Step 7** — **narrow runtime handoff persistence foundation** (payload / category → **persisted** `handoff` row for operator queue — **minimal** slice, **not** full workflow) — see **Next Safe Step**. Optional **later** slices: i18n polish, group **rate limits** — **not** Step **7** by default.
- **Group / handoff:** Steps **1–6** = **rules + helpers + group gating + escalation wording + deep links + private `/start`** for **`grp_*`**; **runtime persistence** from that chain — **Step 7** — **`docs/OPEN_QUESTIONS_AND_TECH_DEBT.md`** §**19**.
...
- **Phase 7–9 (beyond Step 7):** optional group assistant at scale, content assistant, analytics — per `docs/IMPLEMENTATION_PLAN.md`

## Next Safe Step

**Primary (single outcome):** **Phase 7 / Step 7** — **narrow runtime handoff persistence foundation** (*alias:* **handoff persistence readiness / payload-to-handoff mapping foundation** — same intent).

- **Why:** Steps **1–6** built the **group → private** entry chain (**deep links** + **`grp_*`** `/start` intros). The **next** meaningful increment is to **persist** a **handoff record** for **selected** follow-up situations (e.g. **`grp_followup`** and/or **handoff category** from **`evaluate_handoff_triggers`**) so the **existing** admin handoff **read** API (**Phase 6 / Step 18**) can surface **real** intent — **without** building a **full** operator workflow, **without** automatic assignment, **without** booking/payment side effects unless explicitly scoped.
- **In scope (typical):** narrow **service** + **repository** write path; **one** or **few** explicit triggers; **focused** tests; align with **`handoffs`** table / existing **HandoffEntry** patterns where possible.
- **Out of scope for Step 7 unless rescoped:** **i18n**-only work, **rate limits**, **admin payment** mutations, **Mini App** changes, **full** notification suite, **broad** NLP.
- **Baseline (Steps 1–6):** **`docs/GROUP_ASSISTANT_RULES.md`**; **`group_trigger_evaluation`**, **`handoff_trigger_evaluation`**; **`group_gating`**, **`group_chat_gating`**, **`group_private_cta`**; **`private_entry`** **`grp_*`** branch.
...
**Staging (recommended):** **Railway** (or host) bot deploy healthy → smoke **group** CTA → **`grp_*`** private entry → confirm **Step 6** still OK; after Step **7** code: verify **handoff** row appears **only** when scoped.
...
**Completed step references:** `docs/CURSOR_PROMPT_PHASE_6_STEP_1.md` … `docs/CURSOR_PROMPT_PHASE_6_STEP_30.md` (historical); **`docs/CURSOR_PROMPT_PHASE_7_STEP_1.md`** … **`docs/CURSOR_PROMPT_PHASE_7_STEP_6.md`** (historical).  
**Plan:** `docs/IMPLEMENTATION_PLAN.md` — Phase 7.

## Recommended Next Prompt
Implement **Phase 7 / Step 7** — **narrow runtime handoff persistence foundation** (see **Next Safe Step**): minimal **persisted** `handoff` row from a **defined** runtime signal (e.g. **`grp_followup`** path and/or handoff **category**), **no** full operator engine, **no** public API/Mini App churn unless scoped. Sources: **`docs/CHAT_HANDOFF.md`**, **`docs/GROUP_ASSISTANT_RULES.md`**, **`docs/TELEGRAM_SETUP.md`**, **`docs/TECH_SPEC_TOURS_BOT.md`**, `docs/IMPLEMENTATION_PLAN.md`, `docs/OPEN_QUESTIONS_AND_TECH_DEBT.md`, `docs/TESTING_STRATEGY.md`.
...
- `/start grp_private` / `/start grp_followup` (Phase 7 / Step 6) — short intro then catalog
- guided browsing by:
  - date presets
  - destination keyword
...
- **group chat:** Phase **7 / Steps 3–6** — **trigger gating** + **category-aware** wording + **`t.me`** CTA (`group_gating`); Phase **7 / Step 6** — matching **`grp_*`** private `/start` intros — **not** full group assistant, **not** runtime handoff persistence **until Step 7** (see **`docs/GROUP_ASSISTANT_RULES.md`**)
...
- **Approved checkpoint:** **Phase 7 / Step 6** complete — **Phase 7** surface through **Step 6** (helpers, group gating, escalation wording, deep links, **`grp_*`** private `/start`); **`tour_*`** unchanged; **no** handoff rows from runtime chain **yet**, **no** operator assignment; **`TELEGRAM_BOT_USERNAME`** required for full group gating. **Forward:** **Phase 7 / Step 7** — **narrow runtime handoff persistence foundation** (see **Next Safe Step**). Phase 6 closure: **`docs/PHASE_6_REVIEW.md`**. **Not** old **Phase 5 / Step 4–5** checkpoints; **not** admin payment mutation or **Phase 6 / Step 31** unless scoped.
...
Default forward path is **Phase 7 / Step 7** — **narrow handoff persistence foundation** — do not use legacy Phase 5 “next step” prompts for new implementation slices.
