...
- Y36.1 **Operator assignment** gate is in **`docs/OPERATOR_ASSIGNMENT_GATE.md`**. The implemented and **accepted** slice is **Y36.2** + **Y36.2A** + **Y36.3** + **Y36.4** with **Y36.5** docs acceptance and **Y36.6** travel-date copy polish. **Y37.1** operator **workflow** gate: **`docs/OPERATOR_WORKFLOW_GATE.md`**. **Y37.2** **mark under review** is **implemented/accepted** (see bullet list above). **Y37.3** (docs-only): next operator **decision** layer after **`under_review`** — **`docs/OPERATOR_DECISION_GATE.md`**. **Y36.4** Telegram polish is **shipped/accepted**; **reassign** / **unassign** / **resolve** / **close** / supplier RFQ / bridge / booking from intent remain **out of scope** until their **design gates** (not ad hoc).
- **Next safe order:**
  1. **Implement** the first **operator decision** **runtime** slice per **`docs/OPERATOR_DECISION_GATE.md`** (Y37.4+): **one** narrow `POST .../operator-decision` (or equivalent) with **Y36** actor pattern; **`under_review`** + **assigned to me**; **intent-only** persistence; **no** booking/payment/bridge/execution-link/identity or automatic supplier actions. Wider **menu** of intents and “not serviceable / close” **after** separate gates.
