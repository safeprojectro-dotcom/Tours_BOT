...
- **Next safe order:**
  1. **Y38 + Y39 + Y40 (docs, accepted) —** **`docs/SUPPLIER_INTERACTION_GATE.md`**, **`docs/SUPPLIER_ENTRY_POINTS.md`**, **`docs/SUPPLIER_EXECUTION_FLOW.md`**: intent = **data**; **entry** = start; **flow** = **pipeline** (audit, idempotency, fail-closed). **`operator_workflow_intent`** and **`operator-decision`** are **not** triggers. Before any `app/` supplier work: **ticket** must **name** one Y39 **family** + **surface** and map to **Y40** **stages**; **no** hidden hooks. **no** messaging, RFQ, booking, Mini App, execution links, identity bridge, or notifications in-repo unless a slice **explicitly** includes them. **Do not** auto-notify or auto-contact from intent set alone.
