...
**B16B (done):** **`GET /admin/ops-dashboard`** read-only **filters** / **limits** / **time-window**: **`days_ahead`**, **`recent_days`**, **`orders_limit`**, **`tours_limit`**, **`publications_limit`**, **`conversion_links_limit`**, **`attention_limit`**, **`include_sections`**; response **`filters`** metadata; **excluded** sections return **empty** lists; **invalid** **`include_sections`** → **422**; **no** order/payment/reservation mutation, **no** Telegram send/publish/retry, **no** Layer A / Mini App routing changes. **Tests:** **`tests/unit/test_admin_ops_dashboard.py`** **8** **passed**; with **`test_admin_publishing_console.py`** **16** **passed**. **Next:** **B16C** dashboard-to-detail navigation polish.

**B16D (design only — no implementation):** Guarded OPS automation levels and future **`prepare_conversion_chain`** — **[`docs/B16D_OPS_ACTIONS_GUARDED_AUTOMATION_DESIGN.md`](B16D_OPS_ACTIONS_GUARDED_AUTOMATION_DESIGN.md)**. Public **`publish_showcase_channel`** remains a **separate**, **confirmation-gated** action. **Recommended before execution slices:** **B16C** — handoff **[`docs/HANDOFF_B16D_OPS_ACTIONS_GUARDED_AUTOMATION_DESIGN_TO_NEXT_STEP.md`](HANDOFF_B16D_OPS_ACTIONS_GUARDED_AUTOMATION_DESIGN_TO_NEXT_STEP.md)**.
...
- **B16 Step 1** **+** **B16B** **(** **implemented** **):** **`GET /admin/ops-dashboard`** — read-only OPS dashboard **;** **B16B** **query** **filters** **(** **`days_ahead`**, **`recent_days`**, **limits**, **`include_sections`**, **`filters`** **echo** **)** **;** **tests** **`tests/unit/test_admin_ops_dashboard.py`** **8** **passed** **(** **`test_admin_publishing_console.py`** **included** **—** **16** **passed** **)** **;** **handoff** **[`docs/HANDOFF_B16_ADMIN_OPS_VISIBILITY_READ_DASHBOARD_TO_NEXT_STEP.md`](HANDOFF_B16_ADMIN_OPS_VISIBILITY_READ_DASHBOARD_TO_NEXT_STEP.md)** **;** **forward** **B16C**–**B16E** **per** **handoff**.
