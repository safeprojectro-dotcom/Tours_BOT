...
**Next work:** **Phase 7 / Step 15** — **narrow post-resolution operator visibility / queue refinement** for **`group_followup_start`** (see **Next Safe Step**): **one** small read-side or triage-oriented slice after resolution — **not** a broad workflow engine — **not** admin payment work. **i18n** / **rate limits** remain **separate** optional slices unless bundled by product. **Admin payment mutations** are **not** the default.
...
**Current phase (forward work):** **Phase 7 — Group Assistant And Operator Handoff**. **Phase 7 / Steps 1–14** **completed** (**Combined Steps 13–14** = one approved **narrow resolve + resolved read** block). **Next:** **Phase 7 / Step 15** (see **Next Safe Step**). Phase 6 narrow admin API track **closed** through **Step 30** — see **`docs/PHASE_6_REVIEW.md`**.

**Latest approved checkpoint:** **Combined Phase 7 / Steps 13–14** — adds **`POST /admin/handoffs/{id}/resolve-group-followup`** for **`group_followup_start`** only (**`in_review` → `closed`**, **`closed`** idempotent, **`open`** rejected; **no** migration, **no** notifications) and **read-only** **`group_followup_resolution_label`** on admin handoff reads when that reason is **`closed`**. Steps **1–12** remain as documented (**through** **`mark-in-work`** for **assigned** **`group_followup_start`**). **`grp_private`** unchanged; **`tour_*`** / legacy **`/start`** unchanged; **no** booking/payment handling **in group**; **no** automatic assignment from **Telegram `grp_*`** entry; **public** flows **unchanged**. **Step 1** docs: **`docs/GROUP_ASSISTANT_RULES.md`**. **Prior code baseline** unchanged: **Phase 6 / Step 30** **`move_placement_snapshot`**, **Step 29** move **POST**, admin handoffs/orders as implemented. **Admin payment mutations** remain **intentionally postponed** (see **`docs/OPEN_QUESTIONS_AND_TECH_DEBT.md`** **§1f**). **Still not:** **full** group assistant; **broad** **claim/takeover** engine; **broad** **operator workflow engine**; **booking/payment resolution** **via** the Phase **7** **`grp_*`** chain beyond existing private/Mini App flows; **persisted** move audit/timeline rows; **unassign**; broader **reassignment** redesign; **notifications** from admin handoff actions; **refund / capture / cancel-payment**; **public** customer/Mini App flow redesign; **`per_seat` / `full_bus`** track (separate — **not** in these slices). Internal **ops** JSON stays **separate** from **`/admin/*`**.

**Earlier checkpoints:** Combined Phase 7 / Steps 13–14 (**`resolve-group-followup`** + **`group_followup_resolution_label`**); Phase 7 / Step 12 (**`mark-in-work`** for **assigned** **`group_followup_start`**); Phase 7 / Step 11 (**read-side** **`is_assigned_group_followup`**, **`group_followup_work_label`**); Phase 7 / Step 10 (**`assign-operator`** for **`group_followup_start`** only); Phase 7 / Step 9 (admin **`group_followup_start`** visibility — **`is_group_followup`**, **`source_label`**); Phase 7 / Step 8 (focused **`grp_followup`** chain tests); Phase 7 / Step 7 (narrow **`grp_followup`** handoff persistence); Phase 7 / Step 6 (narrow **`grp_*`** private `/start`); Phase 7 / Step 5 (private CTA deep-link foundation on group replies); Phase 7 / Step 4 (handoff-aware short replies in narrow group path); Phase 7 / Step 3 (narrow group runtime gating + default ack); Phase 7 / Step 2 (trigger helper services + unit tests); Phase 7 / Step 1 ( **`docs/GROUP_ASSISTANT_RULES.md`** — rules only); Phase 6 / Steps 1–7 (foundation through core tour patch); Phase 6 / Steps 8–10 (boarding create / patch / delete); Phase 6 / Steps 11–12 (tour translation upsert/delete); Phase 6 / Steps 13–14 (boarding point translation upsert/delete); Phase 6 / Step 15 (tour archive/unarchive); Phase 6 / Step 16 (order detail payment correction visibility); Phase 6 / Step 17 (order detail action preview); Phase 6 / Step 18 (admin handoff queue read API); Phase 6 / Step 19 (admin handoff mark-in-review); Phase 6 / Step 20 (admin handoff close-only); Phase 6 / Step 21 (admin handoff assign); Phase 6 / Step 22 (admin handoff reopen); Phase 6 / Step 23 (admin mark-cancelled-by-operator); Phase 6 / Step 24 (admin mark-duplicate); Phase 6 / Step 25 (admin mark-no-show); Phase 6 / Step 26 (admin mark-ready-for-departure); Phase 6 / Step 27 (admin lifecycle **`ready_for_departure_paid`** read refinement); Phase 6 / Step 28 (admin order move-readiness read-only fields); Phase 6 / Step 29 (admin order narrow **move** mutation); Phase 6 / Step 30 (admin **`move_placement_snapshot`** read-only); Phase 5 accepted + Phase 5 Step 20 docs (`docs/PHASE_5_ACCEPTANCE_SUMMARY.md`).
...
- **Phase 7 / Step 6 (private `/start` for `grp_*`)** — **`app/bot/handlers/private_entry.py`** + **`match_group_cta_start_payload`**; distinct intros **`start_grp_private_intro`** vs **`start_grp_followup_intro`**; **tests** `test_group_private_cta`, `test_private_start_grp_messages`
...
### Not Implemented Yet
- **Next (Phase 7):** **Phase 7 / Step 15** — **narrow post-resolution operator visibility / queue refinement** for **`group_followup_start`** — see **Next Safe Step** (e.g. clearer triage after **`closed`**, optional **list** ordering/filter hints — **read-side or minimal**, **no** workflow engine). Optional **later** slices: i18n polish, group **rate limits**.
- **Group / handoff:** Steps **1–14** = **rules + helpers + group gating + escalation wording + deep links + private `/start`** + **narrow** **`handoffs`** write on **`/start grp_followup`** (**Step 7**) + **focused tests** (**Step 8**) + **admin read visibility** (**Step 9**) + **narrow `assign-operator`** (**Step 10**) + **read-side work-state labels** (**Step 11**) + **narrow `mark-in-work`** (**Step 12**) + **narrow `resolve-group-followup`** + **`group_followup_resolution_label`** (**Steps 13–14**); **group** chat path (Steps **3–4**) still **no** DB handoff rows — **`docs/OPEN_QUESTIONS_AND_TECH_DEBT.md`** §**19**.
...
- **Longer-term (beyond Phase 7 / Step 15 checkpoint):** optional group assistant at scale, content assistant, analytics — per `docs/IMPLEMENTATION_PLAN.md`

## Next Safe Step

**Primary (single outcome):** **Phase 7 / Step 15** — **narrow post-resolution operator visibility / queue refinement** for **`group_followup_start`** (*one* small **read-side** or **triage** slice so **resolved** (**`closed`**) group-follow-up handoffs are easier to **scan** or **deprioritize** in **`GET /admin/handoffs`** / related admin reads — e.g. derived flag, sort hint, or **narrow** default ordering — **without** a full operator workflow engine, **without** notifications, **without** changing booking/payment/public/Mini App flows).

- **Why:** Steps **7–14** — persistence, chain tests, visibility, assign, in-work, **resolve**, and **resolved** read label exist. The **next smallest increment** is **operator-facing queue clarity** *after* resolution — still **no** broad claim engine, **no** notifications, **no** new persistence unless an explicit future slice requires it.
- **In scope (typical):** **read-side** or **minimal list-behavior** refinement **only** for **`group_followup_start`** triage (exact shape is a **Step 15** design choice); **no** general handoff workflow redesign.
- **Out of scope for Step 15 unless rescoped:** **`per_seat` / `full_bus`**, **broad** takeover/reassign, **customer** notifications, **admin payment** mutations, **Mini App** / **public** flow changes, **reopen** redesign.
- **Baseline (Steps 1–14):** **`assign-operator`**, **`mark-in-work`**, **`resolve-group-followup`**, **`group_followup_resolution_label`**, **`compute_group_followup_*`**, admin handoff read/write as in **`docs/CHAT_HANDOFF.md`** **Completed Steps**.
...
**Staging (recommended):** smoke **`GET /admin/handoffs`** (and handoff detail if touched) after **`resolve-group-followup`** on a **`group_followup_start`** row — **Railway** / deploy health as usual.
...
**Completed step references:** `docs/CURSOR_PROMPT_PHASE_6_STEP_1.md` … `docs/CURSOR_PROMPT_PHASE_6_STEP_30.md` (historical); **`docs/CURSOR_PROMPT_PHASE_7_STEP_1.md`** … **`docs/CURSOR_PROMPT_PHASE_7_STEP_12.md`** + **`docs/CURSOR_PROMPT_PHASE_7_STEP_13_14_COMBINED.md`** (historical).  
**Plan:** `docs/IMPLEMENTATION_PLAN.md` — Phase 7.

## Recommended Next Prompt
Implement **Phase 7 / Step 15** — **narrow post-resolution operator visibility / queue refinement** for **`group_followup_start`** (see **Next Safe Step**): **one** small **read-side** or **triage** slice after **`resolve-group-followup`** — **not** a broad workflow engine, **not** notifications, **not** public/Mini App churn unless scoped, **not** **`per_seat` / `full_bus`**. Sources: **`docs/CHAT_HANDOFF.md`**, **`docs/GROUP_ASSISTANT_RULES.md`**, **`docs/TELEGRAM_SETUP.md`**, **`docs/TECH_SPEC_TOURS_BOT.md`**, `docs/IMPLEMENTATION_PLAN.md`, `docs/OPEN_QUESTIONS_AND_TECH_DEBT.md`, `docs/TESTING_STRATEGY.md`.
...
- **Approved checkpoint:** **Combined Phase 7 / Steps 13–14** complete — **Phase 7** surface through **Step 14** (Steps **1–12** as before, plus **`POST /admin/handoffs/{id}/resolve-group-followup`** for **`group_followup_start`** — **`in_review` → `closed`**, **`closed`** idempotent, **`open`** rejected; plus **`group_followup_resolution_label`** on admin reads when that reason is **`closed`**; **no** migration, **no** notifications); **`grp_private`** unchanged; **`tour_*`** / legacy **`/start`** unchanged; **no** automatic assignment from **Telegram `grp_*`** entry; **`TELEGRAM_BOT_USERNAME`** required for full group gating. **Forward:** **Phase 7 / Step 15** — **narrow post-resolution operator visibility / queue refinement** for **`group_followup_start`** (see **Next Safe Step**). Phase 6 closure: **`docs/PHASE_6_REVIEW.md`**. **Not** old **Phase 5 / Step 4–5** checkpoints; **not** admin payment mutation or **Phase 6 / Step 31** unless scoped.
...
Default forward path is **Phase 7 / Step 15** — **narrow post-resolution operator visibility / queue refinement** for **`group_followup_start`** — do not use legacy Phase 5 “next step” prompts for new implementation slices.
