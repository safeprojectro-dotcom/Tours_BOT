...
**Slice C2B5 (implemented):** **`GET …/review-package`** exposes **`cover_media_quality_review`** plus matching strings in aggregated **`warnings`** and **`operator_workflow.warnings`** (deterministic cover/B7.1 **`media_review`** hints — missing or non-sendable photo ref, **`showcase_photo_url`** vs **`cover_media_reference`**, snapshot drift, negative **`media_review`** states, **`approve-for-card`** reminder when a photo would send but B7.1 approval for that reference is missing)**;** read-only — **no** publish gate **.**
