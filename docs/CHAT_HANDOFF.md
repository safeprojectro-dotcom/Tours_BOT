...
**A1 (done / docs design gate):** Admin Automation Cockpit & Controlled Operations Design Gate — **[`docs/A1_ADMIN_AUTOMATION_COCKPIT_DESIGN_GATE.md`](A1_ADMIN_AUTOMATION_COCKPIT_DESIGN_GATE.md)**. Defines cockpit queues, generic card model, action taxonomy, next-best-action rules, fact-lock, AI role boundaries, supplier clarification, marketing review, catalog/conversion, departure ops, customer handoff, RFQ handling, safety boundaries, and manual UAT vision. **Docs-only:** no cockpit UI, buttons, endpoints, migrations, scheduler, Telegram publish, supplier notification send, QR tokens, Layer A changes, or B11 changes. **Next recommended read-only block:** **A1-Block 1 — Cockpit Read-Only Foundation** (queue contracts, generic cockpit card model, summary dashboard, supplier intake, missing info, offer readiness, risk/conflict). A1A-A1I remain optional planning labels only, not the required execution order.
