...
- **Completed checkpoint in this sub-track:** **Steps 1–4** — **Step 1** admin/source-of-truth **`tour.sales_mode`**; **Step 2** backend **`TourSalesModePolicyService`** / **`TourSalesModePolicyRead`**; **Step 3** Mini App read-side adaptation to policy; **Step 4** private bot read-side adaptation to policy. Summary: **Completed Steps** → **Phase 7.1 / Sales Mode / Steps 1–4 completed**. Verification and production smoke: **Verified** (below) and **Production stabilization after Phase 7.1 deploy** (below).
- **Next work in this sub-track:** **Phase 7.1 / Sales Mode / Step 5** — operator-assisted full-bus path — **Next Safe Step**. Operational deploy/migration gate: **`COMMIT_PUSH_DEPLOY.md`** (repo root).
...
### Not Implemented Yet
- **Next (forward, active sub-track):** **Phase 7.1 / Sales Mode / Step 5** — **operator-assisted full-bus path** — **Next Safe Step**; **separate** from Phase **7** handoff/operator chain expansion. Optional **later** slices: i18n polish, group **rate limits** — product-scoped.
- **Phase 7.1 — explicitly postponed (after Steps 1–4):** **`full_bus_price`**; **`bus_capacity`**; **direct whole-bus reservation flow**; **whole-bus payment flow**; **dedicated operator-assisted full-bus workflow** (beyond **Step 5** when scoped); **pricing expansion**; **core reservation engine rewrite**; **old Phase 7** **`grp_followup_*`** chain expansion; broader **payment** / **availability** / **seat-semantics** changes driven by **`sales_mode`** without an explicit slice.
...
### Next safe step
**Phase 7.1 / Sales Mode / Step 5** — **operator-assisted full-bus path**

**Goal:**
- Make the **full-bus assistance** path **more explicit** and **operationally structured**.
- **Preserve tour context** through the **assistance / handoff** path.
- Keep it **separate** from **direct whole-bus self-service** booking.

**Must not expand into:**
- **Direct whole-bus reservation**
- **Whole-bus payment**
- **Broad refactor** of **old Phase 7** **`grp_followup_*`** chain
- **Pricing-model expansion** unless **explicitly scheduled**

**Baseline (shipped):** **Phase 7.1 / Steps 1–4** — **`tour.sales_mode`**, **`TourSalesModePolicyService`**, Mini App + private bot **read-side** policy reflection; **`COMMIT_PUSH_DEPLOY.md`** migration gate for **`20260416_06`**.
...
- **Approved checkpoint:** **Phase 7 closed** — **`docs/PHASE_7_REVIEW.md`** (**review / closure accepted**); **implementation** through Steps **1–17** + **final consolidation** (private **`grp_followup`** + admin **`group_followup_*`** read labels; **no** operator chat, **no** handoff push notifications in this track); **`grp_private`** unchanged; **`tour_*`** / legacy **`/start`** unchanged; **no** automatic assignment from **Telegram `grp_*`** entry; **`TELEGRAM_BOT_USERNAME`** required for full group gating. **Active sub-track:** **Phase 7.1 — tour sales mode** — **Steps 1–4 completed** (admin **`tour.sales_mode`**, **`TourSalesModePolicyService`**, Mini App + private bot read-side); **Railway production** stabilized after **`tours.sales_mode`** migration; **Forward:** **Step 5** (operator-assisted full-bus path — **Next Safe Step**) — **separate** from **Phase 7** **`grp_followup_*`** expansion. **Deploy-critical:** **`tours.sales_mode`** DDL (**`20260416_06`**) must be applied on target DB before Phase **7.1** app deploy — **`COMMIT_PUSH_DEPLOY.md`**. Phase 6 closure: **`docs/PHASE_6_REVIEW.md`**. **Not** old **Phase 5 / Step 4–5** checkpoints; **not** admin payment mutation or **Phase 6 / Step 31** unless scoped.
...
Default forward path is **Phase 7.1 — tour sales mode** — **Step 5** (operator-assisted full-bus path) — do not use legacy Phase 5 “next step” prompts; **do not** assume further Phase **7** **`grp_followup`** implementation without **explicit** product rescoping.
