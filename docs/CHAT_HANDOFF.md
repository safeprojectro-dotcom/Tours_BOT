...
- **Next safe order:**
  1. **Y38 (docs-first) —** **`docs/SUPPLIER_INTERACTION_GATE.md`**. Post–Y38 **next** is defined **only** in that file (section *Post–Y38: explicit next step*): Layer C is **stabilized**; **in-code** supplier/RFQ automation is **not** next. Before any `app/` change for supplier interaction: **new** design gate + ticket; **`operator_workflow_intent`** is **read-only** w.r.t. supplier execution, never a **direct** trigger. **Do not** auto-notify or auto-contact suppliers from intent set alone.
