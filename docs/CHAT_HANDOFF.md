...
**Admin operator workflow — Slice C1 + C1.1 (implemented):** Telegram **private admin offer detail** (moderation workspace) appends **read-only** **`operator_workflow`** text from **`SupplierOfferReviewPackageService.review_package()`** — display only (**C1.1:** compact block for mobile; full detail stays on **`GET …/review-package`**); formatter performs **no** POSTs **;** **no** publish **/** bridge **/** activate-for-catalog **/** execution-link buttons. **Possible Slice C2:** inline buttons driven by **`operator_workflow.actions`**, lowest-risk first **+** explicit confirmation **— not started automatically.**
