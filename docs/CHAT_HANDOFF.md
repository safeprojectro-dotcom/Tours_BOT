...
**B17 (design gate):** **Channel / Template Editor** product/technical boundaries — **[`docs/B17_CHANNEL_TEMPLATE_EDITOR_DESIGN_GATE.md`](B17_CHANNEL_TEMPLATE_EDITOR_DESIGN_GATE.md)** (**B17** markdown is **design-only**; it does not ship code). **B17A (done):** read-only **`GET /admin/publishing-console/supplier-offers/{offer_id}/editor`** — **`AdminPublishingConsoleEditorDetailRead`** + editor **section** DTOs; aggregates existing B15/B15P data into **`channel_section`**, **`template_section`**, **`preview_section`**, **`cta_section`**, **`media_section`**, **`readiness_section`**, **`safety_section`**, **`future_actions`**, **`source_snapshot`** (**no** selection/draft persistence, **no** Telegram I/O, **no** publish, **no** **`prepare_conversion_chain`** from this **GET**). **Handoff:** **[`docs/HANDOFF_B17A_READ_ONLY_EDITOR_DETAIL_VIEW.md`](HANDOFF_B17A_READ_ONLY_EDITOR_DETAIL_VIEW.md)**. **Next (when chartered):** **B17B** selection metadata only — **not** Telegram **publish** without **explicit** go/no-go (**B17E/F**).
...
**B15O (done / pending commit):** **Publishing Console Foundation Closure** — formal docs checkpoint **[`docs/B15O_PUBLISHING_CONSOLE_FOUNDATION_CLOSURE.md`](docs/B15O_PUBLISHING_CONSOLE_FOUNDATION_CLOSURE.md)**. **B15 foundation is closed** as **read-model + guarded internal prepare-chain** (no public automation). **Read surfaces:** **`GET /admin/publishing-console`**, **`GET /admin/publishing-console?kind=supplier_offer_initial`**, **`GET /admin/publishing-console/supplier-offers/{offer_id}`**, **`GET /admin/publishing-console/supplier-offers/{offer_id}/editor`** (**B17A**), **`GET /admin/supplier-offers/{offer_id}/review-package`**, **`GET /admin/supplier-offers/{offer_id}/prepare-conversion-chain/plan`**. **Guarded prepare-chain POST:** **`POST /admin/supplier-offers/{offer_id}/prepare-conversion-chain`**, **`POST /admin/publishing-console/supplier-offers/{offer_id}/prepare-conversion-chain`**. **Safety:** **No** Telegram I/O, **no** scheduler, **no** auto-publish, **no** publish attempts from read surfaces, **no** Layer A mutation, **no** Mini App/B11 routing changes, **no** migration on this foundation line.
