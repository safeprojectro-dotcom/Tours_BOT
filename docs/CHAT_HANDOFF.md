...
- **Next safe order:**
  1. **Y38 + Y39 + Y40 + Y41 (docs, accepted) —** **`docs/SUPPLIER_INTERACTION_GATE.md`**, **`docs/SUPPLIER_ENTRY_POINTS.md`**, **`docs/SUPPLIER_EXECUTION_FLOW.md`**, **`docs/SUPPLIER_EXECUTION_DATA_CONTRACT.md`**: intent = **data**; **entry** = start; **flow** = **pipeline**; **persistence** = **logical** contract (request/attempt/audit, **status** boundaries, **intent snapshot** not trigger). **`operator_workflow_intent`** and **`operator-decision`** are **not** triggers. Before any `app/` supplier work: **ticket** must **name** Y39 **family** + **surface**, map to **Y40** **stages** and **Y41** **fields**; **no** hidden hooks. **no** messaging, RFQ, booking, Mini App, execution links, identity bridge, or notifications in-repo unless a slice **explicitly** includes them. **Do not** auto-notify or auto-contact from intent set alone. **Migrations** only when a slice implements DB — not part of Y41 acceptance.
