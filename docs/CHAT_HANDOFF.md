...
**Next work:** **Phase 7 review / closure checkpoint** (see **Next Safe Step**): reconcile **`docs/CHAT_HANDOFF.md`**, **`docs/GROUP_ASSISTANT_RULES.md`**, **`docs/IMPLEMENTATION_PLAN.md`**, **`docs/TECH_SPEC_TOURS_BOT.md`** with shipped Phase **7** chain; staging smoke (**`/health`**, **`grp_followup`** path, admin handoff reads); **explicit product decision** on the **next** track (**e.g.** **`per_seat` / `full_bus`**, content assistant, notifications — **separate** scopes) — **not** defaulting to more **`grp_followup`** micro-implementation unless product re-opens it. **Admin payment mutations** remain **not** the default.
...
**Current phase (forward work):** **Phase 7 — Group Assistant And Operator Handoff** — **implementation track consolidated** (Steps **1–17** + **final followup/operator wording alignment**). **Next:** **Phase 7 review / closure checkpoint** (see **Next Safe Step**) — then **new product/domain track** by explicit scope (**not** more **`grp_followup`** micro-steps by default). Phase 6 narrow admin API track **closed** through **Step 30** — see **`docs/PHASE_6_REVIEW.md`**.

**Latest approved checkpoint:** **Phase 7 final followup/operator consolidation block** — core chain (**Steps 1–17**) plus **read-side polish**: private **`grp_followup`** messages and admin **`compute_group_followup_assignment_visibility`** / **`compute_group_followup_resolution_label`** strings share one **queued / assigned / in progress / closed** model with **`group_followup_queue_state`**; **`HandoffEntryService.group_followup_private_intro_key`** unchanged in behavior; **`should_show_group_followup_resolved_confirmation`** retained for tests; **no** new workflow capabilities, **no** API schema churn, **no** public/Mini App changes. **`grp_private`** unchanged. **Step 1** docs: **`docs/GROUP_ASSISTANT_RULES.md`**. **Prior code baseline** unchanged: **Phase 6 / Step 30** **`move_placement_snapshot`**, **Step 29** move **POST**, admin handoffs/orders as implemented. **Admin payment mutations** remain **intentionally postponed** (see **`docs/OPEN_QUESTIONS_AND_TECH_DEBT.md`** **§1f**). **Still not:** **full** group assistant; **operator↔customer chat**; **handoff push notifications**; **broad** **claim/takeover** engine; **broad** **operator workflow engine**; **booking/payment resolution** **via** the Phase **7** **`grp_*`** chain beyond existing private/Mini App flows; **persisted** move audit/timeline rows; **unassign**; broader **reassignment** redesign; **refund / capture / cancel-payment**; **public** customer/Mini App flow redesign; **`per_seat` / `full_bus`** track (separate — **next** architecture/product line, **not** implied by Phase **7** closure). Internal **ops** JSON stays **separate** from **`/admin/*`**.
...
- **Longer-term (post Phase 7 review):** **`per_seat` / `full_bus`**, optional group assistant at scale, content assistant, handoff notifications, analytics — **explicit** product scope per `docs/IMPLEMENTATION_PLAN.md` — **not** bundled into Phase **7** closure by default.

## Next Safe Step

**Primary (single outcome):** **Phase 7 review / closure checkpoint** — **read-only / process** work: confirm **`docs/CHAT_HANDOFF.md`**, **`docs/GROUP_ASSISTANT_RULES.md`**, **`docs/IMPLEMENTATION_PLAN.md`**, **`docs/TECH_SPEC_TOURS_BOT.md`** describe the **shipped** narrow **`group_followup_start`** chain end-to-end; run **staging smoke** (**`/health`**, **`/start grp_followup`** states, admin handoff list/detail); record **explicit next track** (e.g. **`per_seat` / `full_bus`**, notifications, content) — **avoid** opening new **`grp_followup`** micro-implementation unless product re-scopes.

- **Why:** Core Phase **7** handoff/operator **chain** for **`group_followup_start`** is **implemented and consolidated**; further micro-steps in the same thread are **diminishing returns** vs **review + next domain slice**.
- **In scope (typical):** documentation accuracy, acceptance notes, **`docs/PHASE_6_REVIEW.md`**-style closure narrative for Phase **7** if desired, test/ops checklist — **no** required code churn.
- **Out of scope unless explicitly re-scoped:** new **`grp_followup`** features, **operator chat**, **handoff notification** delivery, **`per_seat` / `full_bus`** implementation, **admin payment** mutations, **public** / **Mini App** flow changes.
- **Baseline (shipped):** Steps **1–17** + **final consolidation** — **`docs/CHAT_HANDOFF.md`** **Completed Steps** + **`docs/CURSOR_PROMPT_PHASE_7_FINAL_CONSOLIDATION_BLOCK.md`** (historical prompt, if present).
...
**Staging (recommended):** final smoke on **`grp_followup`** + admin **`group_followup_queue_state`** visibility — **Railway** / deploy health as usual.
...
**Completed step references:** `docs/CURSOR_PROMPT_PHASE_6_STEP_1.md` … `docs/CURSOR_PROMPT_PHASE_6_STEP_30.md` (historical); **`docs/CURSOR_PROMPT_PHASE_7_STEP_1.md`** … **`docs/CURSOR_PROMPT_PHASE_7_STEP_12.md`** + **`docs/CURSOR_PROMPT_PHASE_7_STEP_13_14_COMBINED.md`** + **`docs/CURSOR_PROMPT_PHASE_7_STEP_15.md`** + **`docs/CURSOR_PROMPT_PHASE_7_STEP_16.md`** + **`docs/CURSOR_PROMPT_PHASE_7_STEP_17.md`** + **`docs/CURSOR_PROMPT_PHASE_7_FINAL_CONSOLIDATION_BLOCK.md`** (historical).  
**Plan:** `docs/IMPLEMENTATION_PLAN.md` — Phase 7.

## Recommended Next Prompt
Run **Phase 7 review / closure checkpoint** (see **Next Safe Step**): align narrative docs with the **shipped** **`group_followup_start`** chain (**Steps 1–17** + **final consolidation**); staging smoke; decide and document the **next explicit product track** (**e.g.** **`per_seat` / `full_bus`**, notifications) — **do not** default to new **`grp_followup`** code slices unless product re-opens scope. Sources: **`docs/CHAT_HANDOFF.md`**, **`docs/GROUP_ASSISTANT_RULES.md`**, **`docs/TELEGRAM_SETUP.md`**, **`docs/TECH_SPEC_TOURS_BOT.md`**, `docs/IMPLEMENTATION_PLAN.md`, `docs/OPEN_QUESTIONS_AND_TECH_DEBT.md`, `docs/TESTING_STRATEGY.md`, `docs/PHASE_6_REVIEW.md` (closure pattern reference).
...
- **Approved checkpoint:** **Phase 7 final followup/operator consolidation block** complete — Steps **1–17** + **wording alignment** (private **`grp_followup`** + admin **`group_followup_*`** read labels; **no** new workflow, **no** migration, **no** handoff push notifications); **`grp_private`** unchanged; **`tour_*`** / legacy **`/start`** unchanged; **no** automatic assignment from **Telegram `grp_*`** entry; **`TELEGRAM_BOT_USERNAME`** required for full group gating. **Forward:** **Phase 7 review / closure checkpoint** (see **Next Safe Step**), then **explicit next product track** — **not** default **`grp_followup`** micro-code. Phase 6 closure: **`docs/PHASE_6_REVIEW.md`**. **Not** old **Phase 5 / Step 4–5** checkpoints; **not** admin payment mutation or **Phase 6 / Step 31** unless scoped.
...
Default forward path is **Phase 7 review / closure** — do not use legacy Phase 5 “next step” prompts; **do not** assume further Phase **7** **`grp_followup`** implementation without **explicit** product scope.
