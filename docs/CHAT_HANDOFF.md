...
- **B14F (implemented):** Lazy expiry **persistence** — **`lazy_expire_due_reservations_commit_if_any`** commits when expired holds are processed on Mini App catalog/tour detail/bookings, reservation overview, payment-entry (before gate), and **`get_preparable_tour`** (incl. private bot prep). **Admin read** still not an expiry executor. Handoff: **[`docs/HANDOFF_B14F_TEMP_HOLD_EXPIRY_PERSISTENCE_FIX_TO_NEXT_STEP.md`](HANDOFF_B14F_TEMP_HOLD_EXPIRY_PERSISTENCE_FIX_TO_NEXT_STEP.md)**.
