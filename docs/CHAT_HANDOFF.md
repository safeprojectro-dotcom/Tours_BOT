...
- **B13B (implemented):** **Showcase channel adapter interface + Telegram wrapper** — **`app/services/showcase_channel_adapter.py`** (**`ShowcaseChannelPublishRequest`**, **`ShowcaseChannelPublishResult`**, **`ShowcaseChannelAdapter`**, **`TelegramShowcaseChannelAdapter`**, **`default_telegram_showcase_adapter`**); **`SupplierOfferModerationService.publish`** calls the adapter, which delegates to **`send_showcase_publication`**; **behavior-preserving** refactor — same Telegram output/persistence, **no** publish readiness change, **no** outbox / publish-attempt table / migrations / new channels, **no** Mini App / booking / payment / orders. Handoff: **[`docs/HANDOFF_B13B_CHANNEL_ADAPTER_INTERFACE_TELEGRAM_WRAPPER_TO_NEXT_STEP.md`](HANDOFF_B13B_CHANNEL_ADAPTER_INTERFACE_TELEGRAM_WRAPPER_TO_NEXT_STEP.md)**. Tests: adapter + track3 moderation + review_package + catalog_conversion_closure + full **`test_telegram_admin_moderation_y281`** (**89** passed). **Next likely:** **B13C** publish attempt/audit design **or** **B13D** channel preview payload read model.
