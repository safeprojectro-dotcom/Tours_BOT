...
**Plan:** `docs/IMPLEMENTATION_PLAN.md` — Phase 7 **closed**; **active sub-track:** **Phase 7.1 — tour sales mode** (**Steps 1–5** done; **Step 6** next). **V2 supplier marketplace:** `docs/IMPLEMENTATION_PLAN_V2_SUPPLIER_MARKETPLACE.md` — **Track 0**–**3** complete (**Track 3** = moderated channel publication + `supoffer_` private deep link — implementation + stabilization review); **Track 4** next (request marketplace — when scheduled).
