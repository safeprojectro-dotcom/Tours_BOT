...
- Y36.2 + Y36.2A + Y36.3 (operator assignment runtime, ORM fix, docs stabilization) truth:
  ...
  - **Migration `20260429_18`:** adds **`assigned_operator_id`**, **`assigned_by_user_id`**, **`assigned_at`** on **`custom_marketplace_requests`**. **Production (Railway):** applied over SSH with **`python -m alembic upgrade head`**, verified with **`python -m alembic current`**.
  ...
  - **Y36.3 (this checkpoint):** production smoke: bot recovered after deploy; **`/admin_requests`**; request detail shows **Owner**; **Assign to me** updates line to **Assigned to you**. Tests (as recorded in stabilization): **`python -m compileall app tests/unit/test_api_admin.py tests/unit/test_telegram_admin_moderation_y281.py`**; **`pytest tests/unit/test_api_admin.py -k "assign"`** — 21 passed; **`pytest tests/unit/test_telegram_admin_moderation_y281.py -k "admin_ops"`** — 4 passed.
  - **Forward:** **Y36.4** may add operator-assignment **UI polish / list filtering**; **reassign** / **unassign** remain postponed unless separately gated.
...
- Y36.1 **Operator assignment** gate is in **`docs/OPERATOR_ASSIGNMENT_GATE.md`**. **Y36.2** implements assign-to-me (migration **`20260429_18`**), **Y36.2A** fixes SQLAlchemy symmetric relationships, **Y36.3** is the post-production docs stabilization (this file + **`docs/OPEN_QUESTIONS_AND_TECH_DEBT.md`**). **Y36.4** (UI polish / filtering) continues only after the Y36.3 docs checkpoint is **committed, pushed, and deployed**; **reassign** / **unassign** are not in that slice unless separately gated.
- **Next safe order:**
  1. **Y36.4** (optional): operator assignment **UI polish** or **list filtering** on admin/Telegram custom-request surfaces; **not** reassign/unassign without a new gate.
