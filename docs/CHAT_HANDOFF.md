...
**A1 (done / docs design gate):** Admin Automation Cockpit & Controlled Operations Design Gate — **[`docs/A1_ADMIN_AUTOMATION_COCKPIT_DESIGN_GATE.md`](A1_ADMIN_AUTOMATION_COCKPIT_DESIGN_GATE.md)**. Defines cockpit queues, generic card model, action taxonomy, next-best-action rules, fact-lock, AI role boundaries, supplier clarification, marketing review, catalog/conversion, departure ops, customer handoff, RFQ handling, safety boundaries, and manual UAT vision. **A1-Block 1 (done):** **`GET /admin/automation-cockpit`** — **[`docs/HANDOFF_A1_BLOCK1_COCKPIT_READ_ONLY_FOUNDATION.md`](HANDOFF_A1_BLOCK1_COCKPIT_READ_ONLY_FOUNDATION.md)** — core operational queues. **A1-Block 2 (done):** same **`GET`** — additive **marketing_review**, **publishing_queue**, **catalog_conversion** lanes + **`commercial_context`** / fact-lock copy — **[`docs/HANDOFF_A1_BLOCK2_COMMERCIAL_MARKETING_CONVERSION_QUEUES.md`](HANDOFF_A1_BLOCK2_COMMERCIAL_MARKETING_CONVERSION_QUEUES.md)**; still **no** writes, **no** Telegram/send, **no** scheduler, **no** prepare-chain execution from this GET. **Next recommended block:** **A1-Block 3 — Operations / Handoff / RFQ Queues** (see A1 §17). A1A-A1I remain optional planning labels only, not the required execution order.
