...
- narrow admin write slices for tours, boarding points, and single-language translation upsert exist
...
FIRST NARROW TOUR TRANSLATION DELETE-ONLY SLICE

Что именно сделать:
1. Добавить только один protected admin mutation endpoint:
   - DELETE /admin/tours/{tour_id}/translations/{language_code}

2. Реализовать только delete of a single translation row for one tour and one language.
...
- do not introduce fallback language policy changes here
...
Требования к тестам:
- focused tests only
- покрыть protected access for DELETE /admin/tours/{tour_id}/translations/{language_code}
- покрыть successful delete
- покрыть not-found behavior for missing tour
