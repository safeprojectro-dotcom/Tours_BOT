...
**Цепочка конверсии (как в коде и смоках, C2B9B):** оффер **→** AI/admin packaging (`generate`, `approve` packaging) **→** модерация + готовность к publish (`review-package`, `conversion_closure`) **→** публикация showcase (**`POST …/publish`** **или** Telegram **Publică / Publish** при **`publish_showcase_channel.enabled`** — **C2B8B**, с двойным перечитыванием **`review-package`**) **→** явный мост **`Tour`** (**`POST …/tour-bridge`** **или** Telegram **Link tour** при **`create_tour_bridge.enabled`** — **C2B10T-A**) **→** активация каталога при допустимости (**`POST …/tours/{id}/activate-for-catalog`**, **`activate_tour_for_catalog`**, guard **B8.3** **или** Telegram **List for sale / În catalog** при **`activate_tour_for_catalog.enabled`** — **C2B10T-B**, тот же паттерн двойного **`review-package`**) **→** создание/активация **execution link** (**`POST …/execution-link`** **или** Telegram **Booking link / Link rezervări** при **`create_execution_link.enabled`** — **C2B10T-C**; мастер ссылок **/** HTTP для **`published`** — иные входы) **→** **B11** `/start supoffer_<id>` ведёт в **точный** тур Mini App **только** при активной ссылке **`+`** **`OPEN_FOR_SALE`** **`+`** видимости в каталоге **→** бронирование/оплата остаются **Layer A**. **Примечание:** в таблице §3 шаги **5–7** часто делают **до** канального **publish** (каталог может быть зелёным раньше ссылки); **execution link** после **`published`** (см. шаг **10**).
...
| 6 | **`linked_tour_catalog.can_activate_for_catalog`**, конфликт **B8.3** если есть | **`POST /admin/tours/{tour_id}/activate-for-catalog`** **или** Telegram **List for sale / În catalog** (**C2B10T-B**, если **`activate_tour_for_catalog.enabled`**) | **`status: open_for_sale`**, код тура | Каталог Mini App ≙ **`OPEN_FOR_SALE`**, не автомат из модерации |
...
| 10 | **`execution_links_review.can_create_execution_link`**, причина предпроверки | **`POST …/execution-link`** с **`{ "tour_id": … }`** **или** Telegram **Booking link / Link rezervări** (**C2B10T-C** при **`create_execution_link.enabled`**) | **`GET …/execution-links`**, **`conversion_closure`** | Только при **`published`**; не подменяет Layer A |
...
**Slice C2A (Telegram — только безопасные кнопки):** inline-кнопки могут появиться только для кодов **`review_package_refresh`** и **`get_showcase_preview`** (из **`operator_workflow.actions`**, если **`enabled`**). Колбэки **не выполняют POST**: карточка перезагружается свежим **`review-package`**; превью showcase через **`SupplierOfferModerationService.showcase_preview`** (**ничего не отправляет в канал**). **C2A не включает** мутации packaging/модерации/showcase — они в **C2B1+**; **C2B8B** (**publish showcase**), **C2B10T-A** (**bridge**), **C2B10T-B** (**activate-for-catalog**) и **C2B10T-C** (**execution link**) — отдельные действия с подтверждением и повторным **`review-package`**.

**Политика UX кнопок Telegram (принята, docs-only):** карточка админа **не** консоль разработчика; подписи **1–2 слова**, по действию/результату; **не** snake_case, endpoints, enum, HTTP в тексте кнопок. Отключённые действия **`operator_workflow`** — **скрывать**, не показывать как disabled. Безопасное чтение — без подтверждения; безопасная мутация — подтверждение и повторное **`GET …/review-package`** перед выполнением; **публикация showcase** (**`publish_showcase_channel`**) — **два** перечитывания **`review-package`** (**C2B8B**); **`create_tour_bridge`** — **C2B10T-A** (**Link tour / Leagă tur**); **`activate_tour_for_catalog`** — **C2B10T-B** (**List for sale / În catalog**); **`create_execution_link`** — **C2B10T-C** (**Booking link / Link rezervări**), тот же паттерн перечитывания **`review-package`**. Ориентир подписей RO/EN: **`review_package_refresh`** → **Actualizează / Refresh**; **`get_showcase_preview`** → **Preview**; **`generate_packaging_draft`** → **Generează text** или **Pregătește**; **`approve_packaging_for_publish`** → **Aprobă text / Aprobă pachet** (не голое «Aprobă»); legacy модерация → **Aprobă oferta**, **Respinge oferta**; **мост** → **Link tour / Leagă tur**; **каталог** → **List for sale / În catalog**; showcase publish → **Publică / Publish**; **ссылка исполнения** → **Booking link / Link rezervări**. Порядок кнопок: наблюдение/чтение → packaging → **Link tour** (если включено) → **List for sale** (если включено) → legacy модерация → ops/publish → **Booking link** (если включено) → навигация. **HTTP** **`POST …/execution-link`** и мастер ссылок для **`published`** остаются для иных сценариев.

**Slice C2B3 (Telegram):** реализованы только короткие подписи RO/EN и логический порядок кнопок в карточке оффера (**observe/read → packaging → Link tour (если действие включено) → List for sale (если действие включено) → legacy модерация → showcase publish (если действие включено) → Booking link (если действие включено) → Orders/Requests → навигация**); мутационные колбэки — слайсы **C2B1–C2B8B**, **C2B10T-A**, **C2B10T-B**, **C2B10T-C**, **C2B6**.
...
| Поле | Значение |
|------|----------|
| **Тип** | Playbook Slice A + Slice B (**`operator_workflow`**) + Slice C1 / C1.1 / **C2A** / **C2B1** / **C2B2** / **C2B3** / **C2B4** / **C2B5** / **C2B6** / **C2B8B** / **C2B10T-A** / **C2B10T-B** / **C2B10T-C** (Telegram) + **C2B7.1** (**HTTP** **`PUT …/cover`**) + UX policy (подписи/порядок — см. §3) |
| **Язык** | Русский |
