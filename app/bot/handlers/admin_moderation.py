...
def _operator_workflow_c2b1_packaging_propose_callback(
    offer_id: int,
    ow: AdminSupplierOfferOperatorWorkflowRead,
) -> str | None:
    """Single propose callback when packaging approve action is enabled (Slice C2B1)."""
    for act in ow.actions:
        if act.code == OPERATOR_WORKFLOW_C2B1_PACKAGING_APPROVE_CODE and act.enabled:
            return f"{ADMIN_OPS_OW_PKG_APPROVE_PROPOSE_PREFIX}{offer_id}"
    return None


def _accept_warnings_for_packaging_approve(ps: SupplierOfferPackagingStatus | str) -> bool:
    if isinstance(ps, str):
        try:
            ps = SupplierOfferPackagingStatus(ps)
        except ValueError:
            return False
    return ps is SupplierOfferPackagingStatus.NEEDS_ADMIN_REVIEW


def _packaging_status_translation_key(ps: SupplierOfferPackagingStatus | str) -> str:
    v = ps.value if isinstance(ps, SupplierOfferPackagingStatus) else str(ps)
    return f"supplier_offer_packaging_status_{v}"


def _packaging_approve_confirm_prompt_text(
    language_code: str | None,
    pkg: AdminSupplierOfferReviewPackageRead,
) -> str:
    oid = str(pkg.offer.id)
    ps = pkg.offer.packaging_status
    ps_label = translate(language_code, _packaging_status_translation_key(ps))
    lines = [
        translate(language_code, "admin_offer_ow_pkg_confirm_title", offer_id=oid),
        translate(language_code, "admin_offer_ow_pkg_confirm_packaging_status", status=ps_label),
    ]
    if ps == SupplierOfferPackagingStatus.NEEDS_ADMIN_REVIEW:
        lines.append(translate(language_code, "admin_offer_ow_pkg_confirm_warning_note"))
    lines.append(translate(language_code, "admin_offer_ow_pkg_confirm_question"))
    return "\n".join(lines)


def _packaging_approve_confirmation_keyboard(language_code: str | None, *, offer_id: int) -> InlineKeyboardBuilder:
    kb = InlineKeyboardBuilder()
    kb.button(
        text=translate(language_code, "admin_offer_ow_pkg_confirm_ok"),
        callback_data=f"{ADMIN_OPS_OW_PKG_APPROVE_CONFIRM_PREFIX}{offer_id}",
    )
    kb.button(
        text=translate(language_code, "admin_offer_ow_pkg_confirm_cancel"),
        callback_data=f"{ADMIN_OPS_OW_PKG_APPROVE_CANCEL_PREFIX}{offer_id}",
    )
    kb.adjust(2)
    return kb


def _operator_workflow_c2b2_generate_propose_callback(
    offer_id: int,
    ow: AdminSupplierOfferOperatorWorkflowRead,
) -> str | None:
    """Single propose callback when generate_packaging_draft is enabled (Slice C2B2)."""
    for act in ow.actions:
        if act.code == OPERATOR_WORKFLOW_C2B2_GENERATE_CODE and act.enabled:
            return f"{ADMIN_OPS_OW_PKG_GEN_PROPOSE_PREFIX}{offer_id}"
    return None


def _generate_packaging_draft_confirm_prompt_text(
    language_code: str | None,
    pkg: AdminSupplierOfferReviewPackageRead,
) -> str:
    oid = str(pkg.offer.id)
    ...
    ps_label = translate(language_code, _packaging_status_translation_key(ps))
    lines = [
        translate(language_code, "admin_offer_ow_pkg_gen_confirm_title", offer_id=oid),
        translate(language_code, "admin_offer_ow_pkg_confirm_packaging_status", status=ps_label),
        translate(language_code, "admin_offer_ow_pkg_gen_confirm_question"),
    ]
    return "\n".join(lines)


def _generate_packaging_draft_confirmation_keyboard(language_code: str | None, *, offer_id: int) -> InlineKeyboardBuilder:
    kb = InlineKeyboardBuilder()
    kb.button(
        text=translate(language_code, "admin_offer_ow_pkg_gen_confirm_ok"),
        callback_data=f"{ADMIN_OPS_OW_PKG_GEN_CONFIRM_PREFIX}{offer_id}",
    )
    kb.button(
        text=translate(language_code, "admin_offer_ow_pkg_confirm_cancel"),
...
def _operator_workflow_c2b6_media_req_propose_callback(
    offer_id: int,
    ow: AdminSupplierOfferOperatorWorkflowRead,
) -> str | None:
    """Slice C2B6: propose callback when request_cover_photo_replacement is enabled."""
    for act in ow.actions:
        if act.code == OPERATOR_WORKFLOW_C2B6_REQUEST_PHOTO_CODE and act.enabled:
            return f"{ADMIN_OPS_OW_MEDIA_REQ_PROPOSE_PREFIX}{offer_id}"
    return None


def _media_req_confirm_prompt_text(language_code: str | None, *, offer_id: int) -> str:
    oid = str(offer_id)
    return _telegram_plain_trim(
        "\n".join(
            [
                translate(language_code, "admin_offer_ow_media_req_confirm_title", offer_id=oid),
                translate(language_code, "admin_offer_ow_media_req_confirm_question"),
            ],
        ),
    )


def _media_req_confirmation_keyboard(language_code: str | None, *, offer_id: int) -> InlineKeyboardBuilder:
    kb = InlineKeyboardBuilder()
    kb.button(
        text=translate(language_code, "admin_offer_ow_media_req_confirm_ok"),
        callback_data=f"{ADMIN_OPS_OW_MEDIA_REQ_CONFIRM_PREFIX}{offer_id}",
    )
    kb.button(
        text=translate(language_code, "admin_offer_ow_pkg_confirm_cancel"),
...
@router.callback_query(
    ...
    | F.data.startswith(ADMIN_OPS_OW_PKG_APPROVE_CANCEL_PREFIX)
)
async def admin_ops_operator_workflow_c2b1_packaging(query: CallbackQuery, state: FSMContext) -> None:
    """Slice C2B1: approve_packaging_for_publish with mandatory confirmation."""
    if query.from_user is None or query.data is None or query.message is None:
        return
    with SessionLocal() as session:
        lg = _user_service().resolve_language(
    ...
    if data.startswith(ADMIN_OPS_OW_PKG_APPROVE_CANCEL_PREFIX):
        raw_id = data.removeprefix(ADMIN_OPS_OW_PKG_APPROVE_CANCEL_PREFIX)
        offer_id = int(raw_id) if raw_id.isdigit() else 0
        if offer_id <= 0:
            await query.answer(translate(lg, "admin_offer_no_current"), show_alert=True)
            return
    ...
    if data.startswith(ADMIN_OPS_OW_PKG_APPROVE_PROPOSE_PREFIX):
        raw_id = data.removeprefix(ADMIN_OPS_OW_PKG_APPROVE_PROPOSE_PREFIX)
        offer_id = int(raw_id) if raw_id.isdigit() else 0
        if offer_id <= 0:
            await query.answer(translate(lg, "admin_offer_no_current"), show_alert=True)
            return
        try:
            with SessionLocal() as session:
                pkg = SupplierOfferReviewPackageService().review_package(session, offer_id=offer_id)
                approve_ok = any(
                    a.code == OPERATOR_WORKFLOW_C2B1_PACKAGING_APPROVE_CODE and a.enabled
                    for a in pkg.operator_workflow.actions
                )
                if not approve_ok:
                    await query.answer(translate(lg, "admin_offer_ow_action_unavailable"), show_alert=True)
                    return
                prompt = _telegram_plain_trim(_packaging_approve_confirm_prompt_text(lg, pkg))
                markup = _packaging_approve_confirmation_keyboard(lg, offer_id=offer_id).as_markup()
    ...
    raw_id = data.removeprefix(ADMIN_OPS_OW_PKG_APPROVE_CONFIRM_PREFIX)
    offer_id = int(raw_id) if raw_id.isdigit() else 0
    if offer_id <= 0:
        await query.answer(translate(lg, "admin_offer_no_current"), show_alert=True)
        return
    ...
    try:
        with SessionLocal() as session:
            ...
            if not approve_ok:
                await query.answer(translate(lg, "admin_offer_ow_action_unavailable"), show_alert=True)
                return
            accept_warnings = _accept_warnings_for_packaging_approve(pkg.offer.packaging_status)
            ...
            try:
                ...
                session.rollback()
                await query.message.answer(
                    translate(lg, "admin_offer_ow_pkg_failed", detail=exc.message),
                )
                await query.answer()
                return
...
@router.callback_query(
    F.data.startswith(ADMIN_OPS_OW_PKG_GEN_PROPOSE_PREFIX)
    | F.data.startswith(ADMIN_OPS_OW_PKG_GEN_CONFIRM_PREFIX)
    | F.data.startswith(ADMIN_OPS_OW_PKG_GEN_CANCEL_PREFIX)
)
async def admin_ops_operator_workflow_c2b2_generate_packaging(query: CallbackQuery, state: FSMContext) -> None:
    """Slice C2B2: generate_packaging_draft with mandatory confirmation."""
    if query.from_user is None or query.data is None or query.message is None:
        return
    with SessionLocal() as session:
        lg = _user_service().resolve_language(
    ...
    if data.startswith(ADMIN_OPS_OW_PKG_GEN_CANCEL_PREFIX):
        raw_id = data.removeprefix(ADMIN_OPS_OW_PKG_GEN_CANCEL_PREFIX)
        offer_id = int(raw_id) if raw_id.isdigit() else 0
        if offer_id <= 0:
            await query.answer(translate(lg, "admin_offer_no_current"), show_alert=True)
            return
    ...
    if data.startswith(ADMIN_OPS_OW_PKG_GEN_PROPOSE_PREFIX):
        raw_id = data.removeprefix(ADMIN_OPS_OW_PKG_GEN_PROPOSE_PREFIX)
        offer_id = int(raw_id) if raw_id.isdigit() else 0
        if offer_id <= 0:
            await query.answer(translate(lg, "admin_offer_no_current"), show_alert=True)
            return
        try:
            with SessionLocal() as session:
                pkg = SupplierOfferReviewPackageService().review_package(session, offer_id=offer_id)
                gen_ok = any(
                    a.code == OPERATOR_WORKFLOW_C2B2_GENERATE_CODE and a.enabled
                    for a in pkg.operator_workflow.actions
                )
                if not gen_ok:
                    await query.answer(translate(lg, "admin_offer_ow_action_unavailable"), show_alert=True)
                    return
                prompt = _telegram_plain_trim(_generate_packaging_draft_confirm_prompt_text(lg, pkg))
                markup = _generate_packaging_draft_confirmation_keyboard(lg, offer_id=offer_id).as_markup()
            await query.message.answer(prompt, reply_markup=markup)
            await query.answer()
        except SupplierOfferReviewPackageNotFoundError:
            await query.answer(translate(lg, "admin_offer_no_current"), show_alert=True)
    ...
    raw_id = data.removeprefix(ADMIN_OPS_OW_PKG_GEN_CONFIRM_PREFIX)
    offer_id = int(raw_id) if raw_id.isdigit() else 0
    if offer_id <= 0:
        await query.answer(translate(lg, "admin_offer_no_current"), show_alert=True)
        return
    ...
    try:
        with SessionLocal() as session:
            ...
            if not gen_ok:
                ...
                return
            try:
                svc = SupplierOfferPackagingService()
                updated = svc.generate_and_persist(session, offer_id=offer_id)
            except SupplierOfferPackagingNotFoundError:
                await query.answer(translate(lg, "admin_offer_no_current"), show_alert=True)
                return
            except Exception as exc:
                logger.warning("generate_packaging_draft telegram failed offer_id=%s: %s", offer_id, exc)
                session.rollback()
                await query.message.answer(
                    translate(lg, "admin_offer_ow_pkg_gen_failed", detail=str(exc)),
                )
                await query.answer()
                return
...
@router.callback_query(
    F.data.startswith(ADMIN_OPS_OW_MEDIA_REQ_PROPOSE_PREFIX)
    | F.data.startswith(ADMIN_OPS_OW_MEDIA_REQ_CONFIRM_PREFIX)
    | F.data.startswith(ADMIN_OPS_OW_MEDIA_REQ_CANCEL_PREFIX)
)
async def admin_ops_operator_workflow_c2b6_media_request_photo(query: CallbackQuery, state: FSMContext) -> None:
    """Slice C2B6: request_cover_photo_replacement → media_review replacement_requested (confirmation required)."""
    if query.from_user is None or query.data is None or query.message is None:
        return
    with SessionLocal() as session:
        lg = _user_service().resolve_language(
    ...
    if data.startswith(ADMIN_OPS_OW_MEDIA_REQ_CANCEL_PREFIX):
        raw_id = data.removeprefix(ADMIN_OPS_OW_MEDIA_REQ_CANCEL_PREFIX)
        offer_id = int(raw_id) if raw_id.isdigit() else 0
        if offer_id <= 0:
            await query.answer(translate(lg, "admin_offer_no_current"), show_alert=True)
            return
    ...
    if data.startswith(ADMIN_OPS_OW_MEDIA_REQ_PROPOSE_PREFIX):
        raw_id = data.removeprefix(ADMIN_OPS_OW_MEDIA_REQ_PROPOSE_PREFIX)
        offer_id = int(raw_id) if raw_id.isdigit() else 0
        if offer_id <= 0:
            await query.answer(translate(lg, "admin_offer_no_current"), show_alert=True)
            return
        try:
            with SessionLocal() as session:
                pkg = SupplierOfferReviewPackageService().review_package(session, offer_id=offer_id)
                req_ok = any(
                    a.code == OPERATOR_WORKFLOW_C2B6_REQUEST_PHOTO_CODE and a.enabled
                    for a in pkg.operator_workflow.actions
                )
                if not req_ok:
                    await query.answer(translate(lg, "admin_offer_ow_action_unavailable"), show_alert=True)
                    return
                prompt = _media_req_confirm_prompt_text(lg, offer_id=offer_id)
                markup = _media_req_confirmation_keyboard(lg, offer_id=offer_id).as_markup()
            await query.message.answer(prompt, reply_markup=markup)
            await query.answer()
        except SupplierOfferReviewPackageNotFoundError:
            await query.answer(translate(lg, "admin_offer_no_current"), show_alert=True)
    ...
    raw_id = data.removeprefix(ADMIN_OPS_OW_MEDIA_REQ_CONFIRM_PREFIX)
    offer_id = int(raw_id) if raw_id.isdigit() else 0
    if offer_id <= 0:
        await query.answer(translate(lg, "admin_offer_no_current"), show_alert=True)
        return
    ...
    try:
        with SessionLocal() as session:
            ...
            try:
                updated = msvc.request_replacement(
                    session,
                    offer_id=offer_id,
                    reason=DEFAULT_TELEGRAM_PREVIEW_COVER_REPLACEMENT_REASON,
                    reviewed_by=reviewed_by,
                )
                session.commit()
                ...
                logger.warning("media request_replacement telegram failed offer_id=%s: %s", offer_id, exc)
                session.rollback()
                await query.message.answer(
                    translate(lg, "admin_offer_ow_media_req_failed", detail=str(exc)),
                )
                await query.answer()
                return
